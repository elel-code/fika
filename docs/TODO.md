...
- [x] Main-view tile virtualization.
  ...
  - Current: ordinary icon-view rows use the Rust render plan for centered media and centered titles below the icon, while recursive-search rows with group/location metadata keep the horizontal media + text layout.
...
- [~] Spike Dolphin-style self-managed main viewport.
  ...
  - Current: visible tile local `x/y` comes from the reusable `for item[index]` loop and pane row metrics, while tile width/height, media/text rectangles, group/title/location line positions, and tile sizing/font display tokens are projected by the Rust item-view render plan before the virtual slice reaches Slint. Ordinary icon-view rows keep centered media plus centered titles below the icon; recursive-search rows with group/location metadata keep the horizontal media + text layout. Pane row data carries Rust-projected rows/cell/content/scroll metrics, so `SplitPaneView` no longer computes scrollbar extent, zoom-derived tile metrics, or per-item text/media layout inside the Slint main-view layer, and overlapping entries do not get row-data churn just because slice-local coordinates shifted.
