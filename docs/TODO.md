...
- [x] Main-view tile virtualization.
  ...
  - Current: ordinary rows use the Rust render plan for Dolphin-style compact horizontal tiles: icon on the left, filename on the right, and the same item height as the column-first row layout. Following Dolphin's `updateCompactLayoutTextCache()` separation, Rust projects stable title line y/height plus text width, so large zoom levels cannot collapse names to zero. `SplitPaneView` now has one unconditional compact base loop that always paints `item.name`; Trash and recursive-search rows only add a `show-location` metadata overlay for group/location text.
