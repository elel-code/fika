...
- [~] Spike Dolphin-style self-managed main viewport.
  ...
  - Current: `ItemViewPaintEntry` has been narrowed to title text plus Rust-projected paint geometry; directory/file identity stays in Rust row tokens and folder/file fallback sidecars, so the Slint title paint model no longer carries `is_dir` just for a fallback branch that has already moved out of that loop. Paint and highlight rows also no longer carry Slint-facing `slice_index`; paint uses the model row for drop-target indexing and highlight rows draw directly from projected `x/y/width`.
  ...
  - Current: file/folder fallback media rendering has moved out of Slint and into `src/app/item_view_renderer.rs`, then down into pane-level `PaneView` cache. Successful thumbnail images have moved out of `ItemViewEntry` into pane-local sparse `ItemViewMediaEntry` overlay rows; `SplitPaneView` draws pane-level folder/file fallback through separate Rust-projected sparse `ItemViewFallbackMediaEntry` folder/file sidecars with fixed image sources, then overlays only loaded thumbnails. The sparse media overlay now reuses its pane-local `VecModel` through a Rust-only `slice_index/media_token` sidecar and carries its own item x/y, so thumbnail refreshes and cached relayouts do not replace the whole media model, compare `Image` rows, or look up `root.bounds[media.slice_index]`. Fallback file/folder media is cached per pane/theme as one fixed 72px source image, inherited by split-pane snapshots, and reused across icon zoom levels so first zoom does not swap the Slint fallback image source or run per-row `is_dir` source branching.
  ...
  - Current: metadata overlay rendering now uses a pane-local sparse `ItemViewMetadataEntry` model. Rust projects only non-empty group/location rows into that model and attaches item x/y from the current bounds sidecar, `SplitPaneView` draws one metadata `Text` loop from `root.metadata`, and ordinary empty metadata rows no longer instantiate hidden `Text` primitives when `show-location` is active. Non-empty metadata refreshes reuse the same pane-local `VecModel` with sparse row updates instead of replacing the whole overlay model.
  - Current: virtual slice row reuse now uses pane-local `ItemViewRowToken` sidecars owned by `src/app/model_update.rs`, including a lightweight `media_token` for thumbnails/fallback media. Loaded thumbnail images live in a pane-local sparse `ItemViewMediaEntry` overlay, so reused rows no longer need `VecModel::row_data()` clones just to compare visible `ItemViewEntry` values. The Rust-projected `ItemViewItemBounds`, Slint-facing `ItemViewPaintEntry`, and folder/file `ItemViewFallbackMediaEntry` sidecars reuse their pane-local `VecModel`s during range slides and cached relayouts, the sparse media overlay reuses its pane-local model through `ItemViewMediaToken`, and the sparse `ItemViewMetadataEntry` sidecar reuses its own pane-local model during show-location updates. `PaneViewData` no longer exposes `ItemViewEntry` or `bounds` to Slint, and visible primitive loops no longer cross-index bounds for highlight/media/metadata/drop overlays; they read only their own paint/overlay rows.
  ...
  - Next: continue replacing the remaining visible primitive loop with the renderer/reuse side of the Dolphin-style item view layer, starting with text/tile frame reuse or a `SharedPixelBuffer`/`Image` self-rendered tile-frame spike.
