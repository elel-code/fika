...
- [x] Main-view tile virtualization.
  - Current: Rust keeps the visible `virtual_entries` slice pane-locally for controller, hit-test, DnD, thumbnail token state, and bounds projection; Slint receives `entry_count` plus pane-local raster/slot/metadata drawing data, not the full filtered file model or full bounds model.
  ...
  - Current: ordinary rows use the Rust render plan for Dolphin-style compact horizontal tiles: icon on the left, filename on the right, and the same item height as the column-first row layout. Following Dolphin's `updateCompactLayoutTextCache()` separation, Rust projects the ordinary title `Text` frame as the whole tile height so large zoom levels cannot clip names away; Trash and recursive-search rows still use Rust-projected title line y/height plus slotized `show-location` group/location fields. The Rust-native `ItemViewEntry` hot row now carries only identity, directory flag, thumbnail state, and media token; successful thumbnail images, group/location strings, fallback file/folder images, and media/title geometry are pane-level or slot data instead of per-row hot fields. Base icon/name drawing now consumes a pane-local `ItemViewSlotEntry` slot pool projected from row tokens plus bounds, so the Slint title loop no longer pairs `ItemViewEntry` and bounds rows and stable items keep their slot across local insert/delete.
  - Current: `PaneViewData` carries Rust-projected item-view layout metrics (`rows_per_column`, cell size, padding, content width, virtual slice width, and scroll max), viewport, empty state, a tile raster base layer (`item_view_raster_layer` plus width/height), and pane-local `ItemViewSlotEntry` slot model. The business `ItemViewEntry` slice is Rust-native and stays in `PaneView.virtual_entries`, with bounds in a Rust-only `virtual_bounds_entries` sidecar; selection/drop raster inputs, fallback media kind, thumbnail sources, and metadata sources are not separate Slint-facing pane row data. The old selection revision hot field, old paint sidecar, old thumbnail model, old metadata model, old per-item metadata hot fields, old entry/bounds Slint row models, and old Slint-generated `ItemViewEntry` struct have been removed; selection/drop changes regenerate only the visible tile raster layer, visible thumbnail completion patches affected slot rows, and show-location changes patch metadata fields on active slot rows.
...
- [x] Ctrl+wheel zoom for main file tiles.
  - Current: zoom follows Dolphin's `DolphinItemListView::setZoomLevel()` / `updateGridSize()` / `KFileItemListView::triggerIconSizeUpdate()` flow: `icon_zoom_layout_changed()` synchronously applies the new compact style/layout to visible panes through `apply_visible_pane_zoom_style_options()` and `prepare_virtual_view_snapshot_update()`, while `IconSizeUpdateScheduler` keeps the roles updater pending for the 300ms Dolphin `LongInterval`. Scroll/layout changes during that pending interval still rebuild the visible slice, but pass `schedule_thumbnails=false`, matching Dolphin's suppression of visible-index role updates while the icon-size timer is active. `VirtualViewCache` keeps only the current `Arc<ItemViewLayoutEngine>`, `RawFileEntry` caches filename width units at the filesystem source boundary and `PaneEntryModel` stores trait-object entries for Dolphin-style per-column longest-name widths, compact title font metrics stay stable, and icon/media size uses Dolphin's `ZoomLevelInfo` 0..16 mapping.
...
- [x] Add thumbnail model fields.
  - Acceptance: Rust-native `ItemViewEntry` can represent thumbnail state without breaking existing icon fallback.
...
- [x] Dolphin-style thumbnail/preview role scheduling.
  - Acceptance: thumbnail/preview role scheduling follows Dolphin `KFileItemModelRolesUpdater::indexesToResolve()` ordering while still generating viewport thumbnails before offscreen items.
  ...
  - Current: thumbnail/preview roles are ordered in `src/app/file_item_roles_updater.rs` from pane-local row tokens: visible files, visible dirs, forward read-ahead, reverse backward read-ahead, last page, first page, then capped fill. This mirrors Dolphin's model layer instead of keeping ad-hoc priority logic in `main.rs`.
  - Current: `src/app/thumbnail_pipeline.rs` owns duplicate suppression, pending-state marking, candidate/cache/failure checks, and the per-view-sync job cap; `main.rs` no longer owns prioritized thumbnail scheduling.
...
- [x] Split virtual main-view preparation out of `main.rs`.
  ...
  - Current: `src/app/virtual_view.rs` prepares clamped viewport state and the current virtual business slice, then `main.rs` updates Rust-side `ItemViewEntry` rows, maintains the Rust bounds sidecar, projects Slint-facing slot/overlay sidecars, and delegates thumbnail/preview role scheduling to `src/app/file_item_roles_updater.rs`.
...
- [~] Spike Dolphin-style self-managed main viewport.
  ...
  - Current: visible tile `x/y/text_width` is projected from pane-local Rust `ItemViewItemBounds` into Slint-facing `ItemViewSlotEntry` rows, while full tile width stays in the Rust bounds sidecar for raster, hit-test, and layout. Tile height, media/text rectangles, title line position, tile sizing/font display tokens, selection backgrounds, fallback folder/file glyphs, loaded thumbnail tokens/images, and group/location metadata are kept out of per-row `ItemViewEntry` fields. Ordinary rows use Dolphin-style compact horizontal tiles with left media and right text; ordinary title `Text` receives Dolphin's whole-item compact text-frame shape from Rust to avoid large-zoom clipping, while recursive-search rows with group/location metadata keep the Rust-projected multi-line text positions on the same stable slot. Base `ItemViewEntry` rows no longer carry group/location strings, fallback images, metadata/primitive geometry, thumbnail images, or Slint-facing base paint data. Pane row data carries Rust-projected rows/cell/content/scroll metrics, stable item slots, and the current tile raster layer, so `SplitPaneView` no longer computes scrollbar extent, zoom-derived tile metrics, title metadata branching, per-item text/media layout, per-row fallback/thumbnail branching, or per-loop row modulo inside the Slint main-view layer, and overlapping entries do not get hot-row churn just because slice-local coordinates or absolute indices shifted.
  - Current: `ItemViewSlotEntry` carries only active/name/media kind, loaded-thumbnail presence/image, metadata group/location fields, plus the Slint-read slot geometry x/y/text width; path, absolute index, thumbnail token, directory/file identity, and thumbnail state stay in Rust-only projection/token sidecars. The Slint title slot model no longer carries `is_dir`, `thumbnail_state`, `path`, `absolute_index`, `thumbnail_token`, `slice_index`, or full tile `width`; selection/current drop-target geometry feeds the Rust-generated tile raster base layer, while fallback icons, loaded thumbnails, and metadata text are stable slot loops keyed by the same slot pool. Media/metadata identity stays in Rust-only source/token sidecars until it is published into slot fields.
  - Current: Dolphin compact visual metrics now live in `src/app/item_view_metrics.rs`. `geometry.rs` owns viewport/visible-range layout, `item_view.rs` owns input/controller/hit-test, and `item_view_renderer.rs` owns render plan, metadata projection, fallback glyph drawing, and the tile raster base layer. They consume the shared media/font/line/cell/row source instead of owning duplicate zoom formulae; zoom updates tile geometry and current visible raster immediately, while only thumbnail/preview role scheduling is deferred through the 300ms icon-size timer instead of swapping Slint fallback image sources.
  ...
  - Current: file/folder fallback media and successful thumbnail media are no longer separate Slint-facing models. Folder/file fallback geometry is derived from `ItemViewTileFrameBatch` slot plans and rendered by stable slot `Image` loops using pane-cached icon images; successful thumbnail images have moved out of `ItemViewEntry` and the old `ItemViewThumbnailEntry` model into `ItemViewSlotEntry.has_thumbnail` / `thumbnail`, while `thumbnail_token` stays in the Rust-only slot token. `ItemViewMediaSource` is now a Rust-only `slice_index + image` input that `model_update.rs` attaches to the matching active slot using the renderer's media-token lookup. Same key + same thumbnail token reuses the existing slot image without comparing or replacing `Image`; token changes patch only the affected slot. There is no Slint-facing folder/file fallback image model, thumbnail model, fallback sidecar model, or per-row `is_dir` source branch left.
  - Current: metadata overlay rendering is slotized. Rust projects only non-empty group/location text through Rust-only `ItemViewMetadataOverlaySource`, then `model_update.rs` attaches those values to matching active `ItemViewSlotEntry` fields using the source `slice_index`. `SplitPaneView` draws group/location through two slot `Text` loops gated by `has_metadata_group` and `has_metadata_location`, so the current text backend stays Slint-native while the renderer owns the swappable text projection boundary. Ordinary empty metadata items do not instantiate hidden overlay rows or a separate Slint model, and non-empty metadata refreshes reuse the same pane-local slot pool.
  - Current: `item_view_renderer.rs` now exposes `ItemViewFrameEntry`, `ItemViewTileFrameSource`, `ItemViewTileFramePlan`, and `ItemViewTileFrameBatch` as the Rust-side per-tile frame boundary for the future self-rendered path. Both Rust-native `ItemViewEntry` snapshots and pane-local cached `ItemViewRowToken` rows can feed the same frame source; `ItemViewTileFramePlan` then keeps only visible, bounds-backed text/fallback/highlight primitives, while token-only thumbnail updates still read from the source through the batch media-token lookup API. Batch construction, slot-row projection, selection/drop-target raster geometry, fallback media kind, and source-index media-token lookup now live in `item_view_renderer.rs`; `model_update.rs` consumes batch APIs to reuse/publish pane-local models instead of recombining geometry rules separately for each Slint primitive model.
  - Current: virtual slice row reuse now uses pane-local `ItemViewRowToken` sidecars, Rust-only `ItemViewSlotProjection` / `ItemViewSlotToken`, and a stable Slint-facing `ItemViewSlotEntry` draw row pool owned by `src/app/model_update.rs`. Loaded thumbnail images live on the stable slot row, and same key + same Rust-side token preserves the existing `Image`, so reused rows no longer need `VecModel::row_data()` clones or image comparisons just to compare visible `ItemViewEntry` values. The Rust-projected `ItemViewItemBounds` and Slint-facing slot entries are reused during range slides, local insert/delete, and virtual slice refreshes; selection/drop geometry is derived on demand by `ItemViewTileFrameBatch`, loaded thumbnail sources are Rust-only `ItemViewMediaSource` rows until they patch slots, and metadata updates use Rust-only `ItemViewMetadataOverlaySource` before publishing compact Slint-facing rows. `PaneViewData` no longer exposes `ItemViewEntry`, `bounds`, `thumbnails`, slot identity, or thumbnail token to Slint, and visible primitive loops no longer cross-index bounds for selection/fallback/media/metadata/drop overlays; selection/drop target are in the raster base layer, while file/folder fallback icons and loaded thumbnails use stable slot image loops. `PaneView` keeps a pane-local tile raster cache keyed by raster input plus pane-local raster revision; row token, bounds, selection, and drop target changes bump the revision, while thumbnail token changes patch only the affected slot row.
  - Current: split-pane virtual snapshots clone the Rust-side visible entry/bounds slices, row-token sidecar, and stable slot state, keeping pane views independent while preserving the current visible slice.
  ...
  - Current: Slint-facing layout metrics, virtual range planning, virtual slice geometry, range-hint alignment, cache signature comparison, hit-test, item bounds, rectangle-selection candidate logic, layout mode, scroll axis, and logical→physical item projection are now exposed through `ItemViewLayouter`, with `CompactItemViewLayout` as the first implementation. Runtime view/cache state now stores `ItemViewLayoutEngine` rather than `CompactItemViewLayout` directly, and the engine delegates to the compact layouter through the same trait boundary. `split_view.rs`, `virtual_view.rs`, `pane.rs`, `item_view.rs`, and `main.rs` import the layouter trait instead of calling concrete public compact methods directly. Compact layout construction consumes precomputed filename width units through `ItemViewModelEntry`, so Dolphin-style compact size hints remain per-column longest-name driven while zoom and virtual refresh avoid hot-path filename cloning/measurement; background cache-miss layout now streams those width units directly from entries/visible-index caches instead of materializing a separate width Vec, and cache-hit signature checks use shared `Arc` identity before falling back to full layout-array comparison. `ItemViewModelEntry` now exposes the complete entry field surface used by filtering, navigation/status, metadata projection, Slint `FileEntry` results, and item-view row projection; `RawFileEntry` and `FileEntry` implement the same trait, `&T` and `Arc<T>` forward through the same interface, and no concrete row type owns compatibility projection methods. `PaneEntryModel` wraps pane/cache/virtual entry storage behind `Arc<dyn ItemViewModelEntry + Send + Sync>` and exposes model-trait iterators/accessors instead of naked backing containers. `item_view_model.rs` also owns the shared count/folder/file/location/path summary helper consumed by selection and status UI. Selection/search filtering and virtual snapshot filtering now use the same `item_view_model.rs` predicates instead of duplicating query/kind/modified/size/chooser logic against concrete row fields. This is still compact-only at runtime, but the state boundary now matches Dolphin's layouter/model responsibility split more closely.
  ...
  - Remaining: **Self-rendered Tile Frame** — Dolphin 的 `kstandarditemlistwidget` 用 `QPainter` 在 widget 上一次性绘制整个可见区域（图标+文字+高亮+拖放反馈）并复用 widget 实例。Fika 已有 Rust-side `ItemViewTileFrameSource`、bounds-backed `ItemViewTileFramePlan` 和 `ItemViewTileFrameBatch` 作为统一 tile frame 输入/绘制批次，并已增加 `ItemViewTileFrameRasterInput` / `ItemViewTileFrameRaster`，可从 batch 产出一张 `SharedPixelBuffer`/`Image` raster layer。当前 raster layer 已接入 `SplitPaneView`，覆盖选中背景、已加载 thumbnail 和当前 drop target；基础 file/folder fallback icon 改为 PaneView 按尺寸/主题缓存的 image，并由 Slint stable slot loop 贴图。`PaneView` 现在按 raster input + pane-local raster revision 保存 cache，view invalidate/clear 时释放，并通过 `ItemViewSlotEntry` pool 复用 Slint tile rows。主视图仍使用 Slint 原生 `Text` primitive 绘制标题/metadata。剩余迁移重点是稳定 title/metadata text backend 输入边界和必要的字体/text stack 评估；文字 raster 只作为可选 backend，必须在实测快于 Slint 原生 `Text` 后再接入。
   - Remaining: **Controller Signal Bus** — Dolphin 的 `kitemlistcontroller` 通过 Qt signal/slot 与 view 通信，controller 只发射类型化信号，view 自己处理。Fika 的功能等价物已经完整：所有输入手势（item press / activate / right-click / blank press-move-release）都通过 `item_view.rs` controller helpers 返回 `ItemViewControllerAction` 枚举（`SelectPath` / `ActivatePath` / `RequestContextMenu` / `SelectRect` / `ClearSelection`），再经过唯一 dispatch 函数 `apply_item_view_controller_action()` 分发到 `select_path_for_slot()` / `open_path_for_slot()` / `clear_selection` / `select_rect` / 菜单路由。`RequestContextMenu` 只携带 `ItemViewHitEntry`（path/name/size/modified/is-dir），不再暴露完整 `FileEntry`。Dolphin 的 Qt signal/slot 在这里等价于 `enum ItemViewControllerAction` + `match` dispatch —— 每个手势产生一个类型化意图，每个意图有确定的处理分支。唯一剩余差距是组织性的：`apply_item_view_controller_action()` 还在 `main.rs` 中，移入 `PaneController` struct 即完成。
  - Remaining: **Two-phase View Refresh** — Dolphin 显式保留旧 visible state → 新数据就绪 → 原子替换。Fika 的 async virtual refresh in-flight/pending bookkeeping 已收敛进 `VirtualViewRefreshState`，不再是 `PaneView` 上的两个裸字段；过滤/search filter 刷新现在不再先 `clear_virtual_view()`，而是只失效 active cache range、保留当前可见 view rows/layout，强制用空 `VirtualViewCache` 构造新 filter model 的后台 prepare，等 virtual result 返回后再原子提交，避免 filter 变更期间暴露空白或误用旧 visible range。仍缺覆盖目录监控刷新、递归搜索结果、目录切换恢复等所有 view refresh 类型的统一 pending→commit 状态机。
...
- [~] Treat COSMIC Files as the primary Rust/design reference outside the main-pane layout.
  ...
  - Current: external-project parity work should inspect the local source trees directly instead of maintaining separate reference-note documents.
...
- [x] Move thumbnail caching closer to the freedesktop model used by COSMIC.
  ...
  - Current: thumbnail dispatch is now bounded per virtual-view sync; Dolphin-style role ordering lives in `file_item_roles_updater.rs`, while candidate/cache/failure/pending batch logic stays in `thumbnail_pipeline.rs` instead of inline in `main.rs`.
