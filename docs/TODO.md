...
- [x] Main-view tile virtualization.
  - Current: Rust keeps the visible `virtual_entries` slice pane-locally for controller, hit-test, DnD, and thumbnail token state; Slint receives `entry_count` plus pane-local paint/bounds/highlight/media/metadata sidecars, not the full filtered file model.
...
- [x] Split virtual main-view preparation out of `main.rs`.
  ...
  - Current: `src/app/virtual_view.rs` prepares clamped viewport state and the current virtual business slice, then `main.rs` updates Rust-side `ItemViewEntry` rows, projects Slint-facing paint/bounds/overlay sidecars, and schedules visible thumbnails.
...
- [~] Spike Dolphin-style self-managed main viewport.
  ...
  - Current: selected backgrounds and drag/drop target feedback are now separate sparse overlays. Selection state is kept out of `ItemViewEntry` row data; `src/app/model_update.rs` updates pane-local `ItemViewRowToken` sidecars and projects selected rows into pane-local cached `ItemViewHighlightEntry` models with item x/y/width already attached, while Slint draws only those highlight rectangles plus one `drag-target-slice-index` rectangle instead of re-evaluating selected/drop state inside every visible item. The former `selection_revision` field/property path is deleted rather than kept as a compatibility trigger.
  - Current: file/folder fallback media rendering has moved out of Slint and into `src/app/item_view_renderer.rs`, then down into pane-level `PaneView` cache. Successful thumbnail images have moved out of `ItemViewEntry` into pane-local sparse `ItemViewMediaEntry` overlay rows; `SplitPaneView` always draws pane-level folder/file fallback in the base media loop and overlays only loaded thumbnails. The sparse media overlay now reuses its pane-local `VecModel` through a Rust-only `slice_index/media_token` sidecar and carries its own item x/y, so thumbnail refreshes and cached relayouts do not replace the whole media model, compare `Image` rows, or look up `root.bounds[media.slice_index]`. Fallback file/folder media is cached per pane/theme as one fixed 72px source image, inherited by split-pane snapshots, and reused across icon zoom levels so first zoom does not swap the Slint fallback image source.
  ...
  - Current: virtual slice row reuse now uses pane-local `ItemViewRowToken` sidecars owned by `src/app/model_update.rs`, including a lightweight `media_token` for thumbnails/fallback media. Loaded thumbnail images live in a pane-local sparse `ItemViewMediaEntry` overlay, so reused rows no longer need `VecModel::row_data()` clones just to compare visible `ItemViewEntry` values. The Rust-projected `ItemViewBoundsEntry` and `ItemViewPaintEntry` sidecars reuse their pane-local `VecModel`s during range slides and cached relayouts, the sparse media overlay reuses its pane-local model through `ItemViewMediaToken`, and the sparse `ItemViewMetadataEntry` sidecar reuses its own pane-local model during show-location updates. `PaneViewData` no longer exposes `ItemViewEntry` to Slint, and visible primitive loops no longer cross-index bounds for highlight/media/metadata overlays; they read only their own paint/bounds/overlay sidecars.
