...
- [~] Spike Dolphin-style self-managed main viewport.
  ...
  - Current: metadata overlay rendering has been flattened from per-item transparent `Rectangle` wrappers into two direct `Text` loops for group/location. This removes one wrapper layer, but `visible:` still means ordinary entries can instantiate hidden metadata `Text` primitives when `show-location` is active.
  ...
  - Next: first replace the show-location overlay with a pane-local sparse metadata model that only contains non-empty group/location rows, so ordinary entries do not create hidden metadata `Text` primitives. Then continue replacing the remaining visible primitive loop with the renderer/reuse side of the Dolphin-style item view layer, starting with text/tile frame reuse or a `SharedPixelBuffer`/`Image` self-rendered tile-frame spike.
