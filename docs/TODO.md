...
- [x] Ctrl+wheel zoom for main file tiles.
  - Current: zoom now follows Dolphin's direct `setStyleOption()` / `doLayout()` model: `icon_zoom_layout_changed()` synchronously rebuilds the current pane virtual layout and visible raster through `prepare_virtual_view_snapshot_update()`, while only thumbnail/preview role scheduling is delayed by the 300ms Dolphin `LongInterval`-style timer. `VirtualViewCache` keeps only the current `Arc<ItemViewLayoutEngine>`, `PaneEntrySnapshot` still caches filename width units for Dolphin-style per-column longest-name widths, and compact title font metrics stay stable while icon/media size changes with zoom.
