...
- [x] Main-view tile virtualization.
  - Current: Rust keeps the visible `virtual_entries` slice pane-locally for controller, hit-test, DnD, thumbnail token state, and bounds projection; Slint receives `entry_count` plus pane-local raster/paint/media/metadata drawing data, not the full filtered file model or full bounds model.
  ...
  - Current: `PaneViewData` carries Rust-projected item-view layout metrics (`rows_per_column`, cell size, padding, content width, virtual slice width, and scroll max), viewport, empty state, a tile raster base layer (`item_view_raster_layer` plus width/height), and pane-local `ItemViewPaintEntry`, `ItemViewMediaEntry`, and `ItemViewMetadataEntry` overlay models. The business `ItemViewEntry` model stays Rust-side in `PaneView.virtual_entries`; selected/fallback sidecars are Rust-only inputs for raster generation, not Slint-facing pane row data. The old selection revision hot field and old per-item metadata hot fields have been removed; selection/fallback changes regenerate only the visible tile raster layer, visible thumbnail completions publish only sparse media rows, and show-location changes publish only the sparse metadata model.
...
- [~] Spike Dolphin-style self-managed main viewport.
  ...
  - Current: visible tile `x/y/width/text_width` is maintained in a pane-local Rust `ItemViewItemBounds` sidecar and projected into Slint-facing `ItemViewPaintEntry` / sparse overlay rows, while tile height, media/text rectangles, title line position, tile sizing/font display tokens, selection backgrounds, and fallback folder/file glyphs are pane-level/raster data rather than per-row `ItemViewEntry` fields. Ordinary rows use Dolphin-style compact horizontal tiles with left media and right text; ordinary title `Text` receives Dolphin's whole-item compact text-frame shape from Rust to avoid large-zoom clipping, while recursive-search rows with group/location metadata keep the Rust-projected multi-line text positions through a separate sparse metadata model. Base `ItemViewEntry` rows no longer carry group/location strings, fallback images, metadata/primitive geometry, or Slint-facing base paint data. Pane row data carries Rust-projected rows/cell/content/scroll metrics and the current tile raster layer, so `SplitPaneView` no longer computes scrollbar extent, zoom-derived tile metrics, title metadata branching, per-item text/media layout, per-row fallback branching, or per-loop row modulo inside the Slint main-view layer, and overlapping entries do not get hot-row churn just because slice-local coordinates shifted.
  - Current: `ItemViewPaintEntry` has been narrowed to title text plus Rust-projected paint geometry; directory/file identity stays in Rust row tokens and Rust-only folder/file fallback sidecars, so the Slint title paint model no longer carries `is_dir` for fallback branching. Paint, thumbnail media, and metadata rows no longer carry Slint-facing `slice_index`; paint uses the model row for drop-target indexing, while selection and folder/file fallback geometry feed the Rust-generated tile raster base layer. Media/metadata identity stays in Rust-only source/token sidecars.
  ...
  - Current: selected backgrounds are now painted into the tile raster base layer. Selection state is kept out of `ItemViewEntry` row data; `src/app/model_update.rs` updates pane-local `ItemViewRowToken` sidecars and Rust-only `ItemViewHighlightEntry` geometry, then `PaneView` regenerates the current visible raster slice. Slint keeps only one live drop-target rectangle that reuses the matching `ItemViewPaintEntry` geometry instead of re-evaluating selected/drop state inside every visible item. The former `selection_revision` field/property path is deleted rather than kept as a compatibility trigger.
  - Current: file/folder fallback media rendering has moved out of Slint and into `src/app/item_view_renderer.rs` as part of the tile raster base layer. Successful thumbnail images have moved out of `ItemViewEntry` into Rust-only `ItemViewMediaSource` rows that are projected into pane-local sparse `ItemViewMediaEntry` overlay rows; `SplitPaneView` draws the raster base layer first, then overlays only loaded thumbnails. The sparse media overlay now reuses its pane-local `VecModel` through a Rust-only `slice_index/media_token` sidecar and carries only image/x/y on the Slint-facing row, so thumbnail refreshes and cached relayouts do not replace the whole media model, compare `Image` rows, look up `root.bounds[media.slice_index]`, or keep `slice_index` in the paint loop. There is no Slint-facing folder/file fallback image model or per-row `is_dir` source branch left.
  - Current: folder/file fallback media sidecars now update as Rust-only sparse row models instead of pretending to be continuous virtual-range models, avoiding avoidable remove/insert churn when a mixed directory scroll shifts past items of the other kind. Their rows carry only the geometry needed by the raster generator; `slice_index` stays out of Slint because the two sidecars already encode folder/file identity and the glyphs are painted into the tile raster base layer.
  ...
  - Current: virtual slice row reuse now uses pane-local `ItemViewRowToken` sidecars owned by `src/app/model_update.rs`, including a lightweight `media_token` for thumbnails/fallback media. Loaded thumbnail images live in a pane-local sparse `ItemViewMediaEntry` overlay, so reused rows no longer need `VecModel::row_data()` clones just to compare visible `ItemViewEntry` values. The Rust-projected `ItemViewItemBounds`, Slint-facing `ItemViewPaintEntry`, and Rust-only highlight/fallback sidecars are reused during range slides and cached relayouts; the sparse media overlay reuses its pane-local model through Rust-only `ItemViewMediaToken`, and metadata updates use Rust-only `ItemViewMetadataOverlaySource` before publishing compact Slint-facing rows. `PaneViewData` no longer exposes `ItemViewEntry` or `bounds` to Slint, and visible primitive loops no longer cross-index bounds for selection/fallback/media/metadata/drop overlays; selection/fallback are already in the raster base layer, and the remaining Slint overlays read only their own paint/overlay rows.
