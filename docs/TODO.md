...
- [~] Spike Dolphin-style self-managed main viewport.
  ...
  - Current: item-view press and blank-release gestures now return `ItemViewControllerAction` from `src/app/item_view.rs`; `main.rs` executes those actions instead of directly mutating `ItemViewInputState` internals for drag-source setup or matching blank-release details. This is still callback-driven, but it moves another step toward Dolphin's controller signal/action split.
  ...
  - Remaining: **Controller Signal Bus** — Dolphin 的 `kitemlistcontroller` 是独立输入层，通过信号与 view 通信。Fika 已把 item press / blank release 收敛成 `ItemViewControllerAction`，但 controller 仍不是完整事件总线：entry lookup、activation、context menu、selection 执行和 UI/async routing 仍由 `main.rs` 回调编排。
