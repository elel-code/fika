...
- [x] Ctrl+wheel zoom for main file tiles.
  - Current: zoom changes first try a Dolphin-style cached relayout of the current virtual slice for already cached or prewarmed layouts; un-cached layouts are only built synchronously for very small directories, while ordinary `/etc`-scale and larger directories now skip synchronous current-slice style refresh and go straight to background virtual prepare. Cache-hit zoom relayout immediately calls `sync_pane_view_ui()` so current visible fallback/icon/text/raster follows Dolphin's visible widget style update without artificial delay; only thumbnail/preview role scheduling uses the 300ms Dolphin `LongInterval`-style timer. This keeps full compact layout construction out of the zoom input event without delaying visible icons. The UI thread no longer synchronously computes every icon zoom level during directory/scroll rebuilds; `VirtualViewCache` keeps the pane-local layout as `Arc<ItemViewLayoutEngine>` plus a small recent-layout history, and ordinary virtual result / zoom fast path now schedule all zoom layout prewarm levels by distance in `spawn_blocking` with pane-local current-generation single-flight and generation-checked incremental result application. Prewarm commits each zoom level as soon as it is built, so adjacent `+1/-1` layouts enter history before farther levels finish. Snapshot requests, snapshot results, pane surface metrics, hit-test, rectangle selection, pane-cache writeback, and prewarm results share the column width/offset arrays instead of cloning them. Cache-hit layout matching first checks `Arc::ptr_eq` before falling back to full signature comparison, so scroll/viewport cache hits do not rescan the entire layout width/offset arrays. `CompactItemViewLayout` also stores item widths, column widths, and column offsets as `Arc<[f32]>`, and derives per-item text width instead of allocating a duplicate text-width array. Background virtual prepare and zoom cached relayout reuse the current or recent cached layout when the layout signature and entry count match; thumbnail size only decides whether current visible raster/preview rows must rebuild, not whether the compact layout can be reused. Cache-miss layout now streams `name_width_units` from entries or visible-index caches straight into the layouter instead of allocating a full `visible_name_width_units` intermediate Vec. Content and filter changes clear the cached layout history. `PaneEntrySnapshot` caches filename width units when entries enter the pane, so virtual snapshot and cached zoom relayout keep Dolphin-style per-column longest-name widths without repeatedly cloning names or rescanning characters during zoom/directory switches. Thumbnail/preview role scheduling after icon-size changes still uses a latest-only timer, but the ordering and ownership now live in `src/app/file_item_roles_updater.rs`, matching Dolphin `KFileItemModelRolesUpdater::indexesToResolve()` over pane-local row tokens: visible files, visible dirs, forward read-ahead, reverse backward read-ahead, last page, first page, then capped fill. Compact text font metrics stay stable across icon zoom levels so each directory's first zoom does not force a new Slint text font size cold path. Fallback glyphs are regenerated by current visible raster commits, so zoom changes target media geometry and the base layer rather than swapping folder/file fallback `Image` sources. Ordinary window/sidebar/split layout changes still flush immediately. `persist_ui_state()` no longer rebuilds virtual views or synchronously writes `settings.tsv` on the UI thread; interactive settings saves are coalesced and written in the background, while close/navigation keeps a synchronous latest save.
