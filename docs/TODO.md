...
- [x] 接入 pane-local navigation history。
  - 验收：Back/Forward 通过 `PaneId` 路由，切换 focused pane 不会改变历史事件目标；鼠标侧键按 Dolphin `BackButton` / `ForwardButton` 语义绑定到鼠标所在 pane，而不是全局焦点 pane；文件区 viewport、item 冒泡路径、横向 scrollbar 区域和 Places 侧栏都显式拦截 Navigate Back/Forward，避免子控件 mouse occlusion 后侧键失效。
- [~] 建立 chooser shell。
...
- [x] 实现 pane-local zoom（缩放）。
  ...
  - 已完成：新增 `docs/ZOOM_REFERENCE.md`，记录 Dolphin `ZoomLevelInfo`、`DolphinViewActionHandler::zoomIn/zoomOut/zoomReset`、`DolphinView::setZoomLevel` 和 `DolphinItemListView::setZoomLevel`；`ViewState` 改为存储 pane-local `zoom_level`，通过 Dolphin 式 0..16 level 映射到 16px..256px icon size；Ctrl+Plus/Ctrl+Minus/Ctrl+0 和 Ctrl+mouse-wheel 按目标/focused `PaneId` 路由；compact layout 的 icon size、基础列宽和行高由 zoom level 派生；zoom 后只清理目标 pane 的 `CompactColumnMetrics` cache，不触发 directory reload；split pane 继承源 pane `ViewState`，因此继承 zoom。
- [x] 实现状态栏（Status Bar）。
...
- [~] 实现 Places 侧栏右键菜单。
  ...
  - 剩余验收：补设备 place 的 Unmount/Eject/Safely Remove。
- [x] 实现 Trash 视图右键菜单。
...
- [~] 实现 pane item 拖拽目标（drop target）。
  ...
  - 已完成：pane file grid 已接收内部 item drag；拖到目录 item 时目标目录写入 pane-local transient `ItemDropTarget`，目录 tile 根据当前 drop action 使用 Copy 绿色、Move 琥珀色、Link 紫色高亮，和蓝色 selected 高亮区分；拖到非目录 item 或 pane 空白区域时 target 回到当前 pane 目录并可执行 drop；drop 执行复用 core file transfer primitives，支持 Copy/Move/Link mode 和受影响目录 refresh；每次 drag move 和 drop 都从 `Window::modifiers()` 重新计算 `FileTransferMode`，无修饰键或 secondary/Ctrl 为 Copy，Shift 为 Move，Shift+secondary/Ctrl 或 Alt 为 Link；GPUI 平台层 `ExternalPaths` 外部文件 drop 已接入 pane/目录 drop target，并使用同一 action 选择；breadcrumb path segment 已支持内部 item drag 和外部 `ExternalPaths` drop，drop 前会把目标 pane 导航到对应目录；drop target 设置会刷新 pane-local stale generation，若超过 3s 没有任何 drag-move 刷新则清理 pane/Places 高亮，避免 drag cancel/leave 后高亮残留；drop、chooser 拒绝、operation busy、pane lifecycle clear 都会清理 drag/drop transient state；新增 drop target state 和 modifier-to-mode 测试。
  ...
  - 剩余验收：补 cursor action 图标；若 GPUI 后续提供内部 drag leave 事件，可把当前 stale-timeout 清理升级为立即 leave 清理；底层 Wayland clipboard/data-offer 若不转换为 GPUI `ExternalPaths`，还需要 GPUI backend 多 MIME 支持。
...
- [~] 实现拖拽过程中的视觉反馈。
  ...
  - 剩余验收：补 cursor action 图标；若 GPUI 后续暴露内部 drag leave，再改为离开时立即清理。
...
- [ ] Devices 设备识别（U 盘等）。
  ...
  - 已完成：新增 `docs/DEVICES_REFERENCE.md`，记录 Dolphin `KFilePlacesModel`/Solid Places 集成和 cosmic-files `gio::VolumeMonitor` mounter 模型；新增 `src/core/devices.rs`，提供 `MountInfoEntry`、`DeviceInfo`、`DeviceEvent`，解析 `/proc/self/mountinfo`，解码 mountinfo octal escape，过滤 pseudo filesystem/loop system image，生成包含 device path、mount point、filesystem type、best-effort label、capacity placeholder 和 best-effort removable flag 的 core 设备快照；UDisks2 初始发现已通过 shared system bus 调用 `org.freedesktop.DBus.ObjectManager.GetManagedObjects()`，把 `org.freedesktop.UDisks2.Block` / `.Drive` / `.Filesystem` 属性合并为 `Udisks2Snapshot`，再与 mountinfo mount point 合并成 `DeviceInfo`；UDisks2 label、filesystem type、size、removable/ejectable 优先，mountinfo 作为 mounted fallback，`Block.HintIgnore=true` 会阻止 mountinfo 把该设备重新加入；新增 `device_events_between()`，把 old/new 设备快照转换成 deterministic Added/Removed/Changed 事件；Places 启动时读取 mountinfo 的 mounted removable devices 并投影为 `Removable Devices` 动态 section，设备条目不写入 `user-places.xbel`，`replace_removable_device_places()` 只替换动态设备 section、保留用户 bookmark 顺序、跳过与内置/用户 place 重复的 mount point，并使用 drive icon；已用单元测试覆盖 escaped mount point、pseudo filesystem 过滤、media mount removable 推导、非法 escape、UDisks2 block/drive/filesystem 合并、HintIgnore 优先级、设备事件 diff、Removable Devices section 生成/排序/去重和动态替换不污染用户 bookmarks。
