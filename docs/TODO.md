...
- [~] Spike Dolphin-style self-managed main viewport.
  ...
  - Current: virtual slice start/width calculation is now owned by `CompactItemViewLayout::virtual_slice_geometry()`, and virtual-view cache compatibility is owned by `CompactItemViewLayout::matches_layout_signature()`. `split_view.rs` no longer reads column offsets directly when publishing pane view metrics, and `pane.rs` no longer compares compact column/item vectors directly for cache reuse. This is still compact-only, but it moves more layout details behind the layouter boundary needed for Dolphin-style polymorphic layout modes.
  ...
  - Remaining: **Polymorphic Layouter** — Dolphin 的 `kitemlistviewlayouter` 通过 `setItemLayout()` 支持 Compact / Details / Icons 三种模式，内部按逻辑坐标计算后用 `itemRect()` 转置到物理坐标。Fika `CompactItemViewLayout`（`geometry.rs`）仍仅实现 compact 列优先，虽已收拢 virtual slice geometry 和 cache signature 查询，但还没有 layouter trait、逻辑→物理转置层或 Details/Icons 模式。
