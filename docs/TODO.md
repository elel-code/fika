...
- [x] Main-view tile virtualization.
  ...
  - Current: ordinary rows use the Rust render plan for Dolphin-style compact horizontal tiles: icon on the left, filename on the right, and the same item height as the column-first row layout. Following Dolphin's `updateCompactLayoutTextCache()` separation, ordinary title `Text` uses the whole tile height as its compact text frame so large zoom levels cannot clip names away; Trash and recursive-search rows still use Rust-projected title line y/height plus sparse `show-location` metadata overlays for group/location text.
  - Current: `PaneViewData` carries Rust-projected item-view layout metrics (`rows_per_column`, cell size, padding, content width, virtual slice width, and scroll max), viewport, and empty state; pane-local `ItemViewEntry`, cached `ItemViewHighlightEntry`, and `ItemViewMetadataEntry` models are exposed as separate top-level slot models so visible item rows and sparse overlays are not nested inside pane row data. The old selection revision hot field has been removed; selection changes now publish only the sparse highlight model.
...
- [~] Spike Dolphin-style self-managed main viewport.
  ...
  - Current: visible tile local `x/y` comes from the reusable `for item[index]` loop and pane row metrics, while tile width/height, media/text rectangles, group/title/location line positions, and tile sizing/font display tokens are projected by the Rust item-view render plan before the virtual slice reaches Slint. Ordinary rows use Dolphin-style compact horizontal tiles with left media and right text; ordinary title `Text` uses Dolphin's whole-item compact text-frame shape to avoid large-zoom clipping, while recursive-search rows with group/location metadata keep the Rust-projected multi-line text positions and add extra lines. Pane row data carries Rust-projected rows/cell/content/scroll metrics, so `SplitPaneView` no longer computes scrollbar extent, zoom-derived tile metrics, or per-item text/media layout inside the Slint main-view layer, and overlapping entries do not get row-data churn just because slice-local coordinates shifted.
