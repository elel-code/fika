...
- [~] Spike Dolphin-style self-managed main viewport.
  ...
  - Current: virtual slice start/width calculation is now owned by `CompactItemViewLayout::virtual_slice_geometry()`, range-hint alignment by `CompactItemViewLayout::expand_virtual_range_to_hint()`, and virtual-view cache compatibility by `CompactItemViewLayout::matches_layout_signature()`. `split_view.rs` no longer reads column offsets directly when publishing pane view metrics, `virtual_view.rs` no longer applies rows-per-column hint alignment itself, and `pane.rs` no longer compares compact column/item vectors directly for cache reuse. This is still compact-only, but it moves more layout details behind the layouter boundary needed for Dolphin-style polymorphic layout modes.
