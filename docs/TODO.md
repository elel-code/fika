...
- [~] Spike Dolphin-style self-managed main viewport.
  ...
  - Current: `ItemViewPaintEntry` has been narrowed to title text plus Rust-projected paint geometry; directory/file identity stays in Rust row tokens and folder/file fallback sidecars, so the Slint title paint model no longer carries `is_dir` just for a fallback branch that has already moved out of that loop.
