...
- [~] Spike Dolphin-style self-managed main viewport.
  ...
  - Current: virtual slice row reuse now uses pane-local `ItemViewRowToken` sidecars owned by `src/app/model_update.rs`, including a lightweight `media_token` for thumbnails/fallback media. Loaded thumbnail images live in a pane-local sparse `ItemViewMediaEntry` overlay, so reused rows no longer need `VecModel::row_data()` clones just to compare visible `ItemViewEntry` values. The Rust-projected `ItemViewBoundsEntry` sidecar also reuses its pane-local `VecModel` during range slides and cached relayouts, so bounds updates do not replace the Slint model that every visible primitive loop reads.
