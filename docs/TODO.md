...
- [x] Main-view tile virtualization.
  ...
  - Current: ordinary rows use the Rust render plan for Dolphin-style compact horizontal tiles: icon on the left, filename on the right, and the same item height as the column-first row layout. Following Dolphin's `updateCompactLayoutTextCache()` separation, Rust projects the ordinary title `Text` frame as the whole tile height so large zoom levels cannot clip names away; Trash and recursive-search rows still use Rust-projected title line y/height plus sparse `show-location` metadata overlays for group/location text. The base `ItemViewEntry` hot row now carries only identity, directory flag, thumbnail state, successful thumbnail media, and media token; group/location strings, fallback file/folder images, and media/title geometry are pane-level or sparse metadata data instead of per-row hot fields. The Slint title loop only consumes pane-level projected frames and does not branch on group/location.
  - Current: `PaneViewData` carries Rust-projected item-view layout metrics (`rows_per_column`, cell size, padding, content width, virtual slice width, and scroll max), viewport, and empty state; pane-local `ItemViewEntry`, cached `ItemViewHighlightEntry`, and `ItemViewMetadataEntry` models are exposed as separate top-level slot models so visible item rows and sparse overlays are not nested inside pane row data. The old selection revision hot field and old per-item metadata hot fields have been removed; selection changes now publish only the sparse highlight model, and show-location changes publish only the sparse metadata model.
...
- [~] Spike Dolphin-style self-managed main viewport.
  ...
  - Current: file/folder fallback media rendering has moved out of Slint and into `src/app/item_view_renderer.rs`, then down into pane-level `PaneView` cache. `ItemViewEntry.media` only carries successful thumbnails; `SplitPaneView` consumes one media `Image` primitive and chooses between the thumbnail image and `PaneViewData`'s cached folder/file fallback images by `thumbnail_state`. Fallback file/folder media is cached per pane, prewarmed for all icon zoom levels during directory/zoom relayout, inherited by split-pane snapshots, and reused for the same `ItemViewRendererKey`, which includes theme plus tile/media/text/font metrics.
  ...
  - Current: virtual slice row reuse now uses pane-local `ItemViewRowToken` sidecars owned by `src/app/model_update.rs`, including a lightweight `media_token` for thumbnails/fallback media. Overlapping rows are not re-published just because the generated `Image` object differs while the media identity is unchanged, and reused rows no longer need `VecModel::row_data()` clones just to compare visible `ItemViewEntry` values.
