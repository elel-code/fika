...
- [~] 后续重构必须持续拆分 `.rs` 文件和目录，禁止继续把右键菜单、滚动条、拖拽、缩略图等大块行为堆进 `src/main.rs`。新增或重构功能优先落到 `src/core/*`、`src/ui/*` 或新的职责明确模块；需要继续扩展的 UI 模块优先采用现代 Rust 目录式模块（`feature.rs` 作为入口，`feature/*.rs` 承接子职责），`src/main.rs` 只保留 app 状态编排和跨模块路由。
  - 当前拆分进展：`src/ui/scrollbar.rs` 已承接横向滚动条测量/绘制/drag capture；`src/ui/context_menu.rs` 已承接右键菜单 target/action/item/icon model、root/submenu/nested submenu 的 action 生成、service/Open With/Ark fallback 分组、级联定位、viewport clamp/flip、open-state 数据结构、icon snapshot 收集和 GPUI overlay/row 渲染；`src/ui/application_chooser.rs` 已承接 Other Application 弹窗状态、overlay/row 渲染、`uniform_list` 虚拟列表和可见行 icon snapshot 解析；`src/ui/properties_dialog.rs` 已承接 Properties dialog 状态和 overlay/row 渲染；`src/ui/place_draft.rs` 已承接 Places Add/Edit draft 状态和 overlay/field 渲染；`src/ui/rename.rs` 和 `src/ui/rename/draft.rs` 已承接 pane-local rename draft 状态；`src/ui/clipboard.rs` 和 `src/ui/clipboard/state.rs` 已承接 copy/cut mode、GPUI `ClipboardItem` 往返、primary/clipboard 选择导入和 `ClipboardState`；`src/ui/shortcuts.rs` 已承接 pane keyboard shortcut、Ctrl/secondary+wheel zoom 和 rename/location/place/filter 文本输入分类；`src/ui/location_bar.rs` 已作为地址栏 UI 入口，`src/ui/location_bar/draft.rs` 已承接 editable location draft、caret 和水平 scroll snapshot，`src/ui/location_bar/metrics.rs` 已承接 editable 地址栏测量、水平滚动和 caret hit-test metrics；`src/ui/status_bar.rs` 已作为状态栏 UI 入口，`src/ui/status_bar/state.rs` 已承接状态栏 snapshot、space info cache、summary cache、progress handle 和 delayed-progress/df parsing helper；`src/ui/pane.rs` 已作为 pane 外壳 UI 入口，`src/ui/pane/snapshot.rs` 已承接 `PaneSnapshot`；`src/ui/file_grid.rs` 已作为文件网格 UI 入口，`src/ui/file_grid/snapshot.rs` 已承接 `VisibleItemSnapshot`，`src/ui/file_grid/slots.rs` 已承接 pane-local visible item slot pool 和 recycled slot 上限；`src/ui/places.rs` 已作为 Places UI 入口，`src/ui/places/snapshot.rs` 已承接 `PlaceIcon` 和 `PlaceSnapshot`；`src/ui/rubber_band.rs` 已作为框选 UI 状态入口，`src/ui/rubber_band/state.rs` 已承接 rubber-band drag payload、selection rect 和 viewport 投影；`src/ui/drag_drop.rs` 已作为 DnD/drop target UI 入口，`src/ui/drag_drop/state.rs` 已承接 `FileTransferMode`、item/place drag payload、drop target state、modifier-to-mode、cursor-style 和 target matching/reject helper；`src/ui/filter_bar.rs` 已作为过滤栏 UI 入口，`src/ui/filter_bar/state.rs` 已承接 filter snapshot、pane-local filter state 和 filtered model cache key/entry；`src/ui/icons.rs` 已作为文件/命名图标入口，`src/ui/icons/cache.rs` 已承接 `FileIconCache`、`FileIconSnapshot`、MIME icon candidate、系统 icon theme 解析、继承和 size fallback；`src/core/location.rs` 已承接地址栏/启动参数共用的 `~` 展开、绝对/相对路径解析、startup directory normalization、breadcrumb segment model 和 filesystem completion；`src/main.rs` 只保留 context menu action 执行路由、show/choose/open/default/properties 数据生成、place/location draft 提交写回、filter 状态写回和已分类输入 action 分发等 app 状态路由。
