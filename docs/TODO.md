...
- [x] 实现 Dolphin compact file view。
  - 验收：core compact layout、model-index hit-test、selection rect、rubber-band overlay、GPUI item rendering 使用 `src/core/view.rs` 的布局结果；文件网格已抽到 `src/ui/file_grid.rs`，compact layout cache/model projection 已拆到 `src/ui/file_grid/layout.rs`，横向滚动条状态/绘制/拖动命中已拆到 `src/ui/scrollbar.rs`；普通滚轮驱动 pane-local 横向 scroll state；条目按列优先 `index / rows_per_column`、`index % rows_per_column` 布局；每列宽度由该列 item 的最长 name 单独决定，并通过 pane-local `CompactColumnMetrics` cache 复用；列宽 cache 对齐 Dolphin `KItemListSizeHintResolver` 的稳定 geometry 思路，当前 layout key 下会一次性解析全部列宽，滚动期间 content width 不再随新列进入 viewport 改变，避免水平 scrollbar handle 抖动；snapshot 只投影 `CompactLayout::visible_items()` 返回的可见条目，不再 clone 全目录 entries；visible range 计算按可见列/行生成，不按 model 总数扫描；GPUI tile identity 使用 pane-local reusable slot pool，离屏 slot 回收后给新进入 viewport 的 item 复用，core `ItemId` 仍只作为文件模型身份；GPUI element id 使用 pane id + slot id，避免分屏 key 冲突；recycled slot pool 上限为 100，对齐 Dolphin `KItemListCreatorBase::pushRecycleableWidget()`；目录单击只选择，双击才进入；item 本体不画边框，hover 只显示非选中浅底色，不复用 selected 高亮；item 的 selection/highlight/hitbox 使用自身 `visual_rect`，宽度由 icon + name text 决定，item visual rect 是 mouse occlusion hitbox，左键 press 直接处理单选/Ctrl toggle/Shift range 并阻止冒泡到 viewport；点击空白区域清空 selection；空白左键 press 会先清 selection 再进入 rubber-band 状态，rubber-band 只从空白区域开始，item 上 drag 进入 item drag source；rubber-band 产生的选择会保留 selection origin，随后右键只有落在已选 item visual/core 内才打开多选菜单，右键落在空白或未选中 item 上只取消该框选选择；window 坐标路径缺少 viewport origin 或落在 viewport 外时不再按空白内容处理，避免滚动条、pane chrome 或未测量区域触发清选/框选/空白菜单；rubber-band overlay 渲染时从 content 坐标扣除当前 scroll offset，横向滚动后框选框仍与实际选择区域重合；文件/目录图标按目录、扩展名和图标尺寸缓存，候选来自 shared-mime-info 与系统主题路径，读取 KDE/GTK 图标主题配置、支持主题继承，并在文本 fallback 前优先使用主题 `unknown`/`application-octet-stream`；横向 scrollbar/handle visual 与 pane-local scroll state 同步，scrollbar 根控件使用 pane 当前实际布局宽度，drag 按 GPUI 事件中的控件 bounds 重新计算 track 映射并更新 pane-local scroll；scrollbar track/reserve 是正常 mouse occlusion hitbox，不再用 scroll-only mouse blocker 吞掉 left drag；scrollbar reserve 和条体左键按下即使轨道 bounds 缓存缺失也会停止传播，避免滚动条下方 item 或框选起点被触发；scrollbar 拖拽保留初始抓取点偏移，left down 在 GPUI capture phase 命中轨道并建立 active drag，scrollbar 模块保留 live canvas bounds 和 measured-track fallback 两条启动路径；move/up 改为 scrollbar canvas/drag overlay 在 paint callback 内注册的 capture-phase window mouse tracking 且不依赖 `MouseMoveEvent::dragging()`，active drag 按启动时 track bounds 从 window 坐标更新 scroll，full-pane mouse capture 层只作为拖拽期间的临时 hitbox 兜底，不再依赖 bubble 阶段、滚动条 canvas 重绘后仍是当前 hitbox，或鼠标持续悬停在 12px 高的滚动条元素上。
  - 当前滚动条实现更新：`src/ui/scrollbar.rs` 独立持有 `ActiveScrollBarDrag`、`HorizontalScrollBarTrack`、track bounds 测量、canvas 绘制、paint-phase window capture 和 drag 数学；`src/ui/file_grid.rs` 只组合 item viewport 与 scrollbar slot。生产拖拽优先来自 handle canvas 的 live GPUI paint bounds，scrollbar reserve 左键按下会调用同一模块的 measured-track fallback 并停止传播，避免 GPUI 命中顺序导致滚动条变成只遮挡不拖动的死区。active drag 持有启动时的 window-space track rect 和 handle grab offset，move/up 由 scrollbar handle canvas 或拖拽中的 overlay canvas 在 paint callback 内注册的 capture listener 处理，pane 内 full-pane capture layer 只保留为拖拽时的 hitbox 兜底；不再在 app render/layout 阶段调用 `Window::on_mouse_event`。
- [~] 实现滚动条平滑插值滚动。
...
- [!] 滚动条拖拽诊断：鼠标拖动 handle 困难，疑似双路径事件冲突和 occlusion 生命周期问题。
  - 疑点一（双路径拖拽启动冲突）：`src/ui/scrollbar.rs:536-561` 在 handle canvas 的 paint 回调中用 `window.on_mouse_event(capture, MouseDown)` 启动拖拽（Path A）；`src/ui/file_grid.rs:428-439` 在 reserve div 上用声明式 `on_mouse_down(MouseButton::Left)` 启动拖拽（Path B，bubble 阶段）。若 GPUI 的 `stop_propagation` 在 window-level capture 中不能完全阻止 element-level bubble handler，Path B 会在 Path A 之后执行并覆写 `ActiveScrollBarDrag`。两条路径分别使用 paint 回调的 live `bounds`（Path A）和 prepaint 存储的 `horizontal_scrollbar_tracks`（Path B）来推导 `handle_grab_x`，任何坐标舍入偏差都会导致拖拽起始点跳动或不跟手。
  - 疑点二（occlusion 清理监听器）：拖拽开始后 `scrollbar_drag_active = true`，`src/ui/file_grid.rs:449-451` 追加 `.absolute().inset_0()` 的 `scrollbar_drag_capture_overlay`，完全覆盖 file_grid（含滚动条区域）。handle canvas 被遮挡后 GPUI 通过 `.occlude()` 优化可能停止向其派发 paint，导致之前在 paint 回调中注册的 `window.on_mouse_event(capture, MouseMove/MouseUp)` 被清理。Overlay 自身虽有 bubble `on_mouse_move` 和内部 canvas 的 capture listener 作为兜底，但若兜底路径的 paint 注册也存在边界情况，拖拽中 move 事件即丢失。
  - 验证手段：在 `begin_horizontal_scrollbar_drag_from_track_point`（`src/ui/scrollbar.rs:425`）入口处增加 `if self.active_scrollbar_drag.is_some() { return false; }` guard，阻止 Path B 覆写 Path A。若拖拽体验显著改善，即确认为双路径冲突。
...
- [~] 实现 keyboard shortcuts。
  - 已完成：方向键、Shift+方向键、Ctrl/secondary+A、Ctrl/secondary+C/X/V、Ctrl/secondary+L、Ctrl/secondary+Plus/Minus/0、Ctrl/secondary+Shift+N、F2 rename、F6 editable location、Escape、F5、F3、Backspace、Alt+Left、Alt+Right、Alt+D、Delete、Ctrl/secondary+W 和 Ctrl/secondary+Z 都按 focused `PaneId` 路由到 pane-local action；快捷键分类、Ctrl/secondary+wheel zoom delta 映射和 rename/location/place/filter 文本输入分类已拆到 `src/ui/shortcuts.rs`，`src/main.rs` 只把分类后的 action 分发到 pane-local app 方法。
...
- [~] 实现 pane item 拖拽源（drag source）。
  - 已完成：item `visual_rect` 拥有独立 GPUI drag source 和基础拖拽预览，item 上拖拽不再触发空白 rubber-band；拖拽已选中 item 时预览显示当前 pane selection count，语义对齐 Dolphin “拖拽已选中 item = 拖拽选中集合”，且只向 tile 传递轻量 count，不复制完整 selected path 列表。
  - 参考：Dolphin `DolphinView::startDrag()` 创建 `QDrag`，设置 pixmap、mime data（`text/uri-list`），支持 `MoveAction` / `CopyAction` / `LinkAction`。
  - 剩余验收：补向外部应用发布的 `text/uri-list` MIME data。
- [~] 实现 pane item 拖拽目标（drop target）。
  - 参考：Dolphin `DolphinView::dropEvent()` 判断 drop action，对目录执行 move/copy/link 操作。
  - 已完成：pane file grid 已接收内部 item drag；拖到目录 item 时目标目录写入 pane-local transient `ItemDropTarget`，目录 tile 根据当前 drop action 使用 Copy 绿色、Move 琥珀色、Link 紫色高亮，和蓝色 selected 高亮区分；拖到非目录 item 或 pane 空白区域时 target 回到当前 pane 目录并可执行 drop；drop 执行复用 core file transfer primitives，支持 Copy/Move/Link mode 和受影响目录 refresh；每次 drag move 和 drop 都从 `Window::modifiers()` 重新计算 `FileTransferMode`，无修饰键或 secondary/Ctrl 为 Copy，Shift 为 Move，Shift+secondary/Ctrl 或 Alt 为 Link；drag-move 时同步刷新 GPUI active drag cursor，Copy 使用 `DragCopy`，Link 使用 `DragLink`，Move 因 GPUI 当前没有 `DragMove` cursor 变体暂用默认 `Arrow`；GPUI 平台层 `ExternalPaths` 外部文件 drop 已接入 pane/目录 drop target，并使用同一 action 选择；breadcrumb path segment 已支持内部 item drag 和外部 `ExternalPaths` drop，drop 前会把目标 pane 导航到对应目录；drop target 设置会刷新 pane-local stale generation，若超过 3s 没有任何 drag-move 刷新则清理 pane/Places 高亮，避免 drag cancel/leave 后高亮残留；drop、chooser 拒绝、operation busy、pane lifecycle clear 都会清理 drag/drop transient state；新增 drop target state、modifier-to-mode 和 cursor-style 映射测试。
  ...
  - 剩余验收：Move 专用 drag cursor/icon 仍需 GPUI/backend 暴露对应 cursor style 后补齐；若 GPUI 后续提供内部 drag leave 事件，可把当前 stale-timeout 清理升级为立即 leave 清理；底层 Wayland clipboard/data-offer 若不转换为 GPUI `ExternalPaths`，还需要 GPUI backend 多 MIME 支持。
- [~] 实现 Places 侧栏 item 拖拽源。
  - 参考：Dolphin Places panel 允许拖出 bookmark 到外部。
  - 验收：侧栏中的 places 条目可拖拽到 pane 中打开；侧栏条目拖拽到外部应用时传递文件路径（如果对应目录存在）。
  - 已完成：Places 行作为 GPUI 内部 `PlaceDrag` source，拖拽预览显示 label/path；file grid 接收 `PlaceDrag`，拖到 pane 空白或 item 上会导航目标 pane 到对应 place path，并清理 pane/Places drop target；已补目标 pane 导航和状态清理测试。
  - 剩余验收：底层 GPUI/Wayland backend 还未提供从 app 内部 drag source 向外部应用发布 `text/uri-list` 的路径，因此拖出到外部应用的 MIME 传递仍待后续 backend 支持。
...
- [~] 实现 pane 到侧栏、侧栏到 pane 的互相拖拽。
  ...
  - 剩余验收：补 Places drag source 的外部 `text/uri-list` MIME 发布。
- [~] 实现拖拽过程中的视觉反馈。
...
- [~] 优化 inline rename 交互体验。
  ...
  - 已完成：`src/ui/rename.rs` 和 `src/ui/rename/draft.rs` 已承接 pane-local rename draft 状态；基础 inline rename 文本输入和 Enter/Escape 提交/取消已实现。
