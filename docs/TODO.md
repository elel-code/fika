...
- [~] Spike Dolphin-style self-managed main viewport.
  ...
  - Current: `item_view_renderer.rs` now exposes `ItemViewFrameEntry` plus `ItemViewTileFrameSource` as the Rust-side per-tile frame input for the future self-rendered path. Both Slint-facing `ItemViewEntry` snapshots and pane-local cached `ItemViewRowToken` rows can feed the same frame source, and `model_update.rs` derives paint rows, selected highlight rows, fallback media rows, and loaded-media tokens from that shared frame source instead of recombining entry/bounds/token state separately for each Slint primitive model.
