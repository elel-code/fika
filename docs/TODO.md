...
- [x] Drag external folder into Places.
  ...
  - Current: winit fallback source/mime constants, disable-env parsing, startup diagnostics, and DnD trace formatting now live in `src/app/dnd.rs`; `main.rs` only handles the actual winit event bridge.
...
- [x] Add `zbus` XDP portal backend prototype.
  ...
  - Current: recognized `wayland:` `parent_window` handles are preserved and forwarded to `fika --chooser --chooser-parent-window`; empty, malformed, or unknown handles are dropped. `FIKA_DEBUG_PORTAL=1` logs the backend parse decision and the chooser-side received handle, and both diagnostics explicitly report that native transient binding is still disabled. Native transient parent binding remains Wayland platform/window-backend work.
  - Current: `FIKA_DEBUG_PORTAL=1` also prints one request summary per OpenFile / SaveFile / SaveFiles call, including request handle, start directory, selection/save flags, filter and choice counts, initial filter index, parent-window forwarding state, and `native_transient=false`.
