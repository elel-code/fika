...
- [x] Main-view tile virtualization.
  ...
  - Current: `SplitPaneView` owns a self-managed clipped viewport and scrollbar; rendered tiles live in a local slice layer anchored by the first virtualized column plus the first row offset and shifted by `viewport_x`, so large directories avoid both scrollbar-width churn and huge per-tile global coordinates while unaligned cached slices can still render after zoom.
...
- [~] Spike Dolphin-style self-managed main viewport.
  ...
  - Current: split-pane virtual snapshots copy visible rows into an independent `VecModel` and clone the matching row-token sidecar, keeping pane models independent while preserving the current visible slice.
   ...
   - Remaining: **Smooth Scroller** — Dolphin 的 `kitemlistsmoothscroller.cpp` 在 layouter 和 view 之间做插值动画; Fika 当前 `viewport_x` 直接写入，滚轮、导航恢复、resize clamp 均为瞬时跳变，无动画过渡层。
   - Remaining: **Polymorphic Layouter** — Dolphin 的 `kitemlistviewlayouter` 通过 `setItemLayout()` 支持 Compact / Details / Icons 三种模式，内部按逻辑坐标计算后用 `itemRect()` 转置到物理坐标。Fika `CompactItemViewLayout`（`geometry.rs`）仅实现 compact 列优先，无 layouter trait、无逻辑→物理转置层、无 Details/Icons 模式。
