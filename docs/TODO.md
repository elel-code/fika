...
- [~] Spike Dolphin-style self-managed main viewport.
  ...
  - Remaining: **Controller Signal Bus** — Dolphin 的 `kitemlistcontroller` 是独立输入层，通过信号与 view 通信。Fika `ItemViewInputState`（`item_view.rs`）仍直接依赖 `AppState`/`AppWindow` 读取 pane 状态，无独立的 controller 事件总线，手势结果通过 `main.rs` 回调分发。
