...
- [x] Add `zbus` XDP portal backend prototype.
  ...
  - Current: `FIKA_DEBUG_PORTAL=1` also prints one request summary per OpenFile / SaveFile / SaveFiles call, including request handle, start directory, selection/save flags, filter and choice counts, initial filter index, parent-window forwarding state, and `native_transient=false`. When the chooser future finishes, the same debug stream now records whether the request selected paths, was cancelled by the user, produced empty output, was closed by the portal request, or failed with an error.
