...
- [x] Main-view tile virtualization.
  ...
  - Current: viewport-only state publication patches only the affected `PaneSlotData.viewport_x` row field, so clamp and restored-scroll updates no longer rebuild the full pane row model for a single float change.
  ...
  - Current: ordinary icon-view rows use the Rust render plan for centered media and a bottom-anchored title area, with `SplitPaneView` keeping a minimum title font size and tile-width title rect so cached/old rows cannot collapse names to zero; recursive-search rows with group/location metadata keep the horizontal media + text layout.
...
- [~] Spike Dolphin-style self-managed main viewport.
  ...
  - Current: visible tile local `x/y` comes from the reusable `for item[index]` loop and pane row metrics, while tile width/height, media/text rectangles, group/title/location line positions, and tile sizing/font display tokens are projected by the Rust item-view render plan before the virtual slice reaches Slint. Ordinary icon-view rows keep centered media plus a bottom-anchored title area that uses the tile width and a minimum title font size in `SplitPaneView`; recursive-search rows with group/location metadata keep the horizontal media + text layout and Rust-projected text rects. Pane row data carries Rust-projected rows/cell/content/scroll metrics, so `SplitPaneView` no longer computes scrollbar extent, zoom-derived tile metrics, or per-item text/media layout inside the Slint main-view layer, and overlapping entries do not get row-data churn just because slice-local coordinates shifted.
  - Current: the old standalone tile component has been deleted; `SplitPaneView` now owns the visible tile primitive loop directly, and that loop no longer uses per-item `HorizontalLayout` / `VerticalLayout` containers.
