...
- [~] Spike Dolphin-style self-managed main viewport.
  ...
  - Remaining: **Self-rendered Tile Frame** — Dolphin 的 `kstandarditemlistwidget` 用 `QPainter` 在 widget 上一次性绘制整个可见区域（图标+文字+高亮+拖放反馈）并复用 widget 实例。Fika 已有 Rust-side `ItemViewTileFrameSource`、bounds-backed `ItemViewTileFramePlan` 和 `ItemViewTileFrameBatch` 作为统一 tile frame 输入/绘制批次，并已增加 `ItemViewTileFrameRasterInput` / `ItemViewTileFrameRaster`，可从 batch 产出一张 `SharedPixelBuffer`/`Image` raster layer。当前 raster layer 已接入 `SplitPaneView`，覆盖选中背景、已加载 thumbnail 和当前 drop target；基础 file/folder fallback icon 改为 PaneView 按尺寸/主题缓存的 image，并由 Slint paint loop 贴图。`PaneView` 现在按 raster input + pane-local raster revision 保存 cache，view invalidate/clear 时释放。主视图仍使用 Slint 原生 `Text` primitive 绘制标题/metadata。剩余迁移重点是稳定 title/metadata text backend 输入边界、tile reuse 和必要的字体/text stack 评估；文字 raster 只作为可选 backend，必须在实测快于 Slint 原生 `Text` 后再接入。tile 复用仍停留在 `VecModel` 滑动更新（`update_sliding_vec_model`），缺少 Dolphin 式的 tile 实例复用池。
   - Remaining: **Model Store Polymorphism** — Dolphin 的 `kfileitemmodel` 是抽象基类，layouter/controller/view 通过接口消费，数据源切换（本地目录/搜索/回收站/设备）不需要改任何消费方。Fika 的 trait 层已完整：`ItemViewModelEntry` 定义 11 个字段方法 + `model_to_file_entry`/`model_to_item_view_entry`/`model_metadata_source` 默认投影，`PaneEntrySnapshot`、`FileEntry`、`RawFileEntry` 三个类型实现该 trait，`&T` blanket impl 支持 iterator adapter 直接转发；`item_view_entry_matches_filters`（含 query/kind/modified/size/chooser 五个子断言）统一接受 `impl ItemViewModelEntry`，不再各自复制字段逻辑；`ItemViewModelEntrySummary` 统一 count/folders/files/location/path 汇总；selection（`visible_paths`/`filtered_entry_at`/`select_rect_candidates`）通过 `model_path_string`/`model_to_file_entry` 消费 trait；virtual_view 通过 `model_location`/`model_name` 消费 trait；thumbnail range checks 通过 `model_path` 消费 trait。`PaneEntryModel` 已经成为 pane/cache/virtual 入口的 store wrapper，renderer 输入侧另有 `ItemViewFrameEntry` trait（2 实现：`ItemViewEntry` + `ItemViewRowToken`），controller 输出侧有 `ItemViewHitEntry` 窄 struct。唯一剩余差距：`PaneEntryModel` 内部 backing 仍为 `Arc<[PaneEntrySnapshot]>`，无法注入搜索结果/回收站/设备的不同条目类型。改成 store-level enum、泛型 `<M: ItemViewModelEntry>`，或 trait object backing 即完成。
