...
- [~] Spike Dolphin-style self-managed main viewport.
  ...
  - Current: item-view press, activation, and blank-area gesture lifecycle now route through `src/app/item_view.rs`; item press hit-test plus drag-source setup are handled by `press_entry_at_pane_point()`, activation hit-test returns an `ActivatePath` action from `activate_entry_at_pane_point()`, blank press/move/release/cancel use slot-level controller helpers, and `main.rs` executes returned `ItemViewControllerAction` values instead of directly mutating `ItemViewInputState` internals or matching gesture details. This is still callback-driven, but it moves another step toward Dolphin's controller signal/action split.
  ...
  - Remaining: **Controller Signal Bus** — Dolphin 的 `kitemlistcontroller` 是独立输入层，通过信号与 view 通信。Fika 已把 item press hit-test/drag-source setup、activation hit-test 和 blank gesture lifecycle 收敛进 `item_view.rs` controller helpers，并通过 `ItemViewControllerAction` 返回执行意图，但 controller 仍不是完整事件总线：context menu、selection 执行和 UI/async routing 仍由 `main.rs` 回调编排。
