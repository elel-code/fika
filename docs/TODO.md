...
- [~] Spike Dolphin-style self-managed main viewport.
  ...
  - Current: file/folder fallback media rendering has moved out of Slint and into `src/app/item_view_renderer.rs`, then down into pane-level `PaneView` cache. Successful thumbnail images have moved out of `ItemViewEntry` into pane-local sparse `ItemViewMediaEntry` overlay rows; `SplitPaneView` always draws pane-level folder/file fallback in the base media loop and overlays only loaded thumbnails. Fallback file/folder media is cached per pane/theme as one fixed 72px source image, inherited by split-pane snapshots, and reused across icon zoom levels so first zoom does not swap the Slint fallback image source.
