...
- [~] Evolve file operation progress toward COSMIC's controller split.
  ...
  - Current: Undo completion now flows through `FileUndoCompletionSummary`; affected-directory derivation, failed-Undo restoration, status text, and old overwrite-backup cleanup decisions live in `operation_controller.rs`, while `main.rs` refreshes pane ids, syncs Undo UI when requested, and performs backup cleanup after releasing the mutable `AppState` borrow.
  - Current: Undo start/take decisions now flow through `FileUndoStartDecision`, including empty-state copy, affected-pane routing, and start status text; `main.rs` syncs Undo UI after releasing the mutable `AppState` borrow and then dispatches the async task.
