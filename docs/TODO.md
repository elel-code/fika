...
- [~] Spike Dolphin-style self-managed main viewport.
  ...
  - Current: virtual slice row reuse now uses a lightweight `media_token` for thumbnails/fallback media, so overlapping rows are not re-published just because the generated `Image` object differs while the media identity is unchanged.
