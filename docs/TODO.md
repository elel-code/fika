...
- [~] Phase 11：Details row 已开始进入 retained paint slot；当前 P11a 只把行内容/几何/视觉状态投影到 retained model，渲染仍保留 GPUI row subtree。背景/图标/文字是否继续转入 custom paint layer 需要 perf 证明不劣于 GPUI built-in 路径。
