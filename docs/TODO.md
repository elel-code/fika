...
- [ ] Add split view / dual-pane browsing.
  ...
  - Current: Undo, privileged operation completion, and protected external-edit save-back also refresh panes through affected-directory pane-id routing, so inactive split previews are not left stale after those non-directory async completions; Undo start/completion/failure, privileged completion, and admin write-back save status now write to the same affected panes.
