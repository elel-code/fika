...
- [~] Phase 11：Details row 已进入 retained paint slot，背景/图标/文字已转入 content-level custom visual layer；当前仍保留 row shell 作为交互/drag 边界。继续移除 row shell 或扩大自绘前必须用 perf 证明不劣于 GPUI built-in 路径。
