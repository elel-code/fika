...
- [~] Spike Dolphin-style self-managed main viewport.
  ...
  - Current: Slint-facing layout metrics, virtual range planning, virtual slice geometry, range-hint alignment, cache signature comparison, hit-test, item bounds, and rectangle-selection candidate logic are now exposed through `ItemViewLayouter`, with `CompactItemViewLayout` as the first implementation. `split_view.rs`, `virtual_view.rs`, `pane.rs`, `item_view.rs`, and `main.rs` import the layouter trait instead of calling concrete public compact methods directly. This is still compact-only, but the first trait boundary needed for Dolphin-style polymorphic layout modes now exists.
  ...
  - Remaining: **Polymorphic Layouter** — Dolphin 的 `kitemlistviewlayouter` 通过 `setItemLayout()` 支持 Compact / Details / Icons 三种模式，内部按逻辑坐标计算后用 `itemRect()` 转置到物理坐标。Fika 已有 `ItemViewLayouter` trait 并由 `CompactItemViewLayout` 实现，但仍只有 compact 列优先实现，还缺 layout-mode enum/trait object 边界、逻辑→物理转置层和 Details/Icons 模式。
