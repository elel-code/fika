...
- [x] 直接 crates.io 依赖不使用 `*`。版本声明保持最新稳定大版本范围（例如 `tokio = "1"`、`zbus = "5"`），不锁到 patch/minor，实际解析版本由 `Cargo.lock` 和 `cargo update` 维护；Fika 自身 D-Bus 路径不直接引入或依赖 `async-io` runtime。
...
- [~] 后续重构必须持续拆分 `.rs` 文件和目录，禁止继续把右键菜单、滚动条、拖拽、缩略图等大块行为堆进 `src/main.rs`。新增或重构功能优先落到 `src/core/*`、`src/ui/*` 或新的职责明确模块，`src/main.rs` 只保留 app 状态编排和跨模块路由。
  - 当前拆分进展：`src/ui/scrollbar.rs` 已承接横向滚动条测量/绘制/drag capture；`src/ui/context_menu.rs` 已承接右键菜单 target/action/item/icon model、root/submenu/nested submenu 的 action 生成、service/Open With/Ark fallback 分组、级联定位、viewport clamp/flip 和 open-state 数据结构；`src/ui/application_chooser.rs` 已承接 Other Application 弹窗状态、overlay/row 渲染、`uniform_list` 虚拟列表和可见行 icon snapshot 解析；`src/ui/properties_dialog.rs` 已承接 Properties dialog 状态和 overlay/row 渲染；`src/ui/place_draft.rs` 已承接 Places Add/Edit draft 状态和 overlay/field 渲染；`src/main.rs` 只保留 context menu overlay 渲染、action 执行路由、show/choose/open/default/properties 数据生成/place draft 提交写回等 app 状态路由。
