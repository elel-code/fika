...
- [x] Main-view tile virtualization.
  ...
  - Current: `PaneViewData` carries Rust-projected item-view layout metrics (`rows_per_column`, cell size, padding, content width, virtual slice width, and scroll max), viewport, selection revision, and empty state; pane-local `ItemViewEntry`, cached `ItemViewHighlightEntry`, and `ItemViewMetadataEntry` models are exposed as separate top-level slot models so visible item rows and sparse overlays are not nested inside pane row data.
