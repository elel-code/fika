...
- [x] Main-view tile virtualization.
  ...
  - Current: ordinary icon-view rows use the Rust render plan for centered media and a title rect inside the same item-size height used by row layout, so cached/old rows cannot collapse names to zero; recursive-search rows with group/location metadata keep the horizontal media + text layout.
