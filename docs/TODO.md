...
- [x] Main-view tile virtualization.
  - Current: Rust keeps the visible `virtual_entries` slice pane-locally for controller, hit-test, DnD, thumbnail token state, and bounds projection; Slint receives `entry_count` plus pane-local paint/highlight/media/metadata sidecars, not the full filtered file model or full bounds model.
  ...
  - Current: viewport-only state publication patches only the affected `PaneViewData.viewport_x` row field, so clamp and restored-scroll updates no longer rebuild the full pane row model for a single float change.
  ...
  - Current: `PaneViewData` carries Rust-projected item-view layout metrics (`rows_per_column`, cell size, padding, content width, virtual slice width, and scroll max), viewport, empty state, and pane-local `ItemViewPaintEntry`, `ItemViewHighlightEntry`, `ItemViewMediaEntry`, and `ItemViewMetadataEntry` models. The business `ItemViewEntry` model stays Rust-side in `PaneView.virtual_entries`; visible item rows and sparse overlays are not nested inside pane row data. The old selection revision hot field and old per-item metadata hot fields have been removed; selection changes now publish only the sparse highlight model, visible thumbnail completions publish only sparse media rows, and show-location changes publish only the sparse metadata model.
...
- [x] Ctrl+wheel zoom for main file tiles.
  - Current: zoom changes refresh visible pane layout immediately, first trying a Dolphin-style cached relayout of the current virtual slice; cache misses now start a background virtual prepare instead of doing a synchronous snapshot rebuild inside the zoom input event. Directory/scroll rebuilds prewarm a pane-local virtual range hint for all icon zoom levels' true visible ranges, and those ranges are aligned to each target zoom's `rows_per_column`, while the zoom fast path only requires the current slice to cover the new true visible range and keeps the cached window even when its start is not aligned to the new row count. Thumbnail scheduling after icon-size changes still uses a latest-only timer and now reads lightweight pane-local row tokens instead of touching the Slint row model. Compact text font metrics stay stable across icon zoom levels so each directory's first zoom does not force a new Slint text font size cold path. Pane-level fallback media is cached per theme as a fixed high-resolution source image, so zoom only changes target media geometry instead of swapping folder/file fallback `Image` sources. Ordinary window/sidebar/split layout changes still flush immediately. `persist_ui_state()` no longer rebuilds virtual views or synchronously writes `settings.tsv` on the UI thread; interactive settings saves are coalesced and written in the background, while close/navigation keeps a synchronous latest save.
...
- [x] Search filters.
  ...
  - Current: the `Filter` popup is slot-routed and anchored near the button/chip; it is now a single-page widget-menu-style selector panel for Type / Modified / Size, not a pane-expanding strip or target-routed subpage.
  - Current: active Type / Modified / Size filters appear as removable chips in the second row; clicking the chip body opens the shared selector panel, while the trailing remove control clears only that restriction.
...
- [~] Spike Dolphin-style self-managed main viewport.
  ...
  - Current: visible tile `x/y/width/text_width` is maintained in a pane-local Rust `ItemViewItemBounds` sidecar and projected into Slint-facing `ItemViewPaintEntry` / sparse overlay rows, while tile height, media/text rectangles, title line position, tile sizing/font display tokens, and fallback folder/file images are pane-level view data rather than per-row `ItemViewEntry` fields. Ordinary rows use Dolphin-style compact horizontal tiles with left media and right text; ordinary title `Text` receives Dolphin's whole-item compact text-frame shape from Rust to avoid large-zoom clipping, while recursive-search rows with group/location metadata keep the Rust-projected multi-line text positions through a separate sparse metadata model. Base `ItemViewEntry` rows no longer carry group/location strings, fallback images, metadata/primitive geometry, or Slint-facing base paint data. Pane row data carries Rust-projected rows/cell/content/scroll metrics, so `SplitPaneView` no longer computes scrollbar extent, zoom-derived tile metrics, title metadata branching, per-item text/media layout, or per-loop row modulo inside the Slint main-view layer, and overlapping entries do not get hot-row churn just because slice-local coordinates shifted.
  - Current: `ItemViewPaintEntry` has been narrowed to title text plus Rust-projected paint geometry; directory/file identity stays in Rust row tokens and folder/file fallback sidecars, so the Slint title paint model no longer carries `is_dir` just for a fallback branch that has already moved out of that loop. Paint, highlight, thumbnail media, and metadata rows also no longer carry Slint-facing `slice_index`; paint uses the model row for drop-target indexing, highlight rows draw directly from projected `x/y/width`, and media/metadata identity stays in Rust-only source/token sidecars.
  - Current: Dolphin compact visual metrics now live in `src/app/item_view_metrics.rs`. `geometry.rs` owns viewport/visible-range layout, `item_view.rs` owns input/controller/hit-test, and `item_view_renderer.rs` owns render plan, metadata projection, and fallback media. They consume the shared media/font/line/cell/row source instead of owning duplicate zoom formulae, while fallback image sources stay theme-level so zoom updates only touch target geometry.
  ...
  - Current: selected backgrounds and drag/drop target feedback are now separate overlays. Selection state is kept out of `ItemViewEntry` row data; `src/app/model_update.rs` updates pane-local `ItemViewRowToken` sidecars and projects selected rows into pane-local cached `ItemViewHighlightEntry` models with item x/y/width already attached, while Slint draws only those highlight rectangles plus one `drag-target-slice-index` rectangle that reuses the matching `ItemViewPaintEntry` geometry instead of re-evaluating selected/drop state inside every visible item. The former `selection_revision` field/property path is deleted rather than kept as a compatibility trigger.
  - Current: file/folder fallback media rendering has moved out of Slint and into `src/app/item_view_renderer.rs`, then down into pane-level `PaneView` cache. Successful thumbnail images have moved out of `ItemViewEntry` into Rust-only `ItemViewMediaSource` rows that are projected into pane-local sparse `ItemViewMediaEntry` overlay rows; `SplitPaneView` draws pane-level folder/file fallback through separate Rust-projected sparse `ItemViewFallbackMediaEntry` folder/file sidecars with fixed image sources, then overlays only loaded thumbnails. The sparse media overlay now reuses its pane-local `VecModel` through a Rust-only `slice_index/media_token` sidecar and carries only image/x/y on the Slint-facing row, so thumbnail refreshes and cached relayouts do not replace the whole media model, compare `Image` rows, look up `root.bounds[media.slice_index]`, or keep `slice_index` in the paint loop. Fallback file/folder media is cached per pane/theme as one fixed 72px source image, inherited by split-pane snapshots, and reused across icon zoom levels so first zoom does not swap the Slint fallback image source or run per-row `is_dir` source branching.
  - Current: folder/file fallback media sidecars now update as sparse row models instead of pretending to be continuous virtual-range models, avoiding avoidable remove/insert churn when a mixed directory scroll shifts past items of the other kind. Their Slint-facing rows now carry only `x/y`, with `slice_index` kept out of the fallback draw loop because the two sidecars already encode folder/file identity.
  - Current: metadata overlay rendering now uses a pane-local sparse `ItemViewMetadataEntry` model. Rust projects only non-empty group/location rows through Rust-only `ItemViewMetadataOverlaySource`, attaches item x/y from the current bounds sidecar, and then drops `slice_index` before publishing the Slint-facing row. `SplitPaneView` draws one metadata `Text` loop from `root.metadata`, and ordinary empty metadata rows no longer instantiate hidden `Text` primitives when `show-location` is active. Non-empty metadata refreshes reuse the same pane-local `VecModel` with sparse row updates instead of replacing the whole overlay model.
  - Current: virtual slice row reuse now uses pane-local `ItemViewRowToken` sidecars owned by `src/app/model_update.rs`, including a lightweight `media_token` for thumbnails/fallback media. Loaded thumbnail images live in a pane-local sparse `ItemViewMediaEntry` overlay, so reused rows no longer need `VecModel::row_data()` clones just to compare visible `ItemViewEntry` values. The Rust-projected `ItemViewItemBounds`, Slint-facing `ItemViewPaintEntry`, and folder/file `ItemViewFallbackMediaEntry` sidecars reuse their pane-local `VecModel`s during range slides and cached relayouts, the sparse media overlay reuses its pane-local model through Rust-only `ItemViewMediaToken`, and metadata updates use Rust-only `ItemViewMetadataOverlaySource` before publishing compact Slint-facing rows. `PaneViewData` no longer exposes `ItemViewEntry` or `bounds` to Slint, and visible primitive loops no longer cross-index bounds for highlight/media/metadata/drop overlays; they read only their own paint/overlay rows.
  ...
  - Current: Slint-facing layout metrics are projected by `CompactItemViewLayout::layout_metrics()`, virtual range anchors by `range_anchor()`, virtual slice start/width by `virtual_slice_geometry()`, range-hint alignment by `expand_virtual_range_to_hint()`, and virtual-view cache compatibility by `matches_layout_signature()`. `split_view.rs` no longer reads column offsets directly or derives range anchors itself when publishing pane view metrics, `virtual_view.rs` no longer applies rows-per-column hint alignment itself, and `pane.rs` no longer compares compact column/item vectors directly for cache reuse. This is still compact-only, but it moves more layout details behind the layouter boundary needed for Dolphin-style polymorphic layout modes.
  - Current: item-view press, activation, item context-menu hit-test/select intent, and blank-area gesture lifecycle now route through `src/app/item_view.rs`; item press hit-test plus drag-source setup are handled by `press_entry_at_pane_point()`, activation hit-test returns an `ActivatePath` action from `activate_entry_at_pane_point()`, context-menu hit-test returns a `RequestContextMenu` action from `context_menu_entry_at_pane_point()`, blank press/move/release/cancel use slot-level controller helpers, and `main.rs` executes returned `ItemViewControllerAction` values instead of directly mutating `ItemViewInputState` internals or matching gesture details. This is still callback-driven, but it moves another step toward Dolphin's controller signal/action split.
  ...
  - Remaining: **Polymorphic Layouter** — Dolphin 的 `kitemlistviewlayouter` 通过 `setItemLayout()` 支持 Compact / Details / Icons 三种模式，内部按逻辑坐标计算后用 `itemRect()` 转置到物理坐标。Fika `CompactItemViewLayout`（`geometry.rs`）仍仅实现 compact 列优先，虽已收拢 Slint metrics projection、range anchor、virtual slice geometry、range-hint alignment 和 cache signature 查询，但还没有 layouter trait、逻辑→物理转置层或 Details/Icons 模式。
  - Remaining: **Self-rendered Tile Frame** — Dolphin 的 `kstandarditemlistwidget` 用 `QPainter` 在 widget 上一次性绘制整个可见区域（图标+文字+高亮+拖放反馈）并复用 widget 实例。Fika 当前仍使用 Slint 原生 `Text`/`Image` primitive 循环，未达到 `SharedPixelBuffer`/`Image` 自渲染 tile 帧。tile 复用也停留在 `VecModel` 滑动更新（`update_sliding_vec_model`），缺少 Dolphin 式的 tile 实例复用池。
  - Remaining: **Model Trait Abstraction** — Dolphin 的 `kfileitemmodel` 是抽象基类，layouter/controller/view 通过接口消费。Fika 用具体 `FileEntry`/`PaneEntrySnapshot` 类型，layouter 和 controller 与数据结构耦合，更换数据源需要适配。
  - Remaining: **Controller Signal Bus** — Dolphin 的 `kitemlistcontroller` 是独立输入层，通过信号与 view 通信。Fika 已把 item press hit-test/drag-source setup、activation hit-test、item context-menu hit-test/select intent 和 blank gesture lifecycle 收敛进 `item_view.rs` controller helpers，并通过 `ItemViewControllerAction` 返回执行意图，但 controller 仍不是完整事件总线：selection 执行、context-menu UI route/service-menu async refresh 和其他 UI/async routing 仍由 `main.rs` 回调编排。
  - Remaining: **Two-phase View Refresh** — Dolphin 显式保留旧 visible state → 新数据就绪 → 原子替换。Fika 在 uncached 导航时保留了旧视图占位，但过滤/搜索/监控刷新直接原地更新 virtual slice，无统一的 pending→commit 状态机。
  - Next: 从自渲染 tile frame spike（`SharedPixelBuffer`/`Image`）开始推进 renderer 侧，逐步向 Dolphin 五层拆分靠拢。
