...
- [~] 实现目录缓存（Directory Cache）。
  ...
  - 已完成：后台 listing worker 已按路径/mode 合并同一批 pending 请求，合并结果共享 entries 载荷；watcher/operation refresh 路径批量调度同一轮 listing request；`Entry` 文本字段使用共享 `Arc<str>`，减少分屏同目录时 pane-local `ItemId` 重建造成的文件名深拷贝；`DirectoryModel` 现在只为 pane 分配轻量 `ModelEntry` 身份，底层 `EntryData` 在 split pane 和 retargeted listing 之间共享；pane load/reload/back/forward/close 会清理该 pane 的 visible slot pool、compact column width cache、viewport origin、rubber-band/context menu/rename state 和 properties modal，避免旧大目录的 UI 布局缓存继续留存；`src/core/cache.rs` 已新增 `DirectoryCache`，缓存键做路径规范化，值保存共享 `Arc<Vec<Entry>>`、fresh/stale 状态、加载序号、目录 LRU、总 entry 预算、单目录 entry 上限和命中/未命中/淘汰/跳过统计；`ListingWorkerState` 成为 per-app cache owner，成功的 `ListingRefreshed` 写入 fresh cache，普通 `Load` 只在 fresh cache 命中时即时投递 cached `ListingRefreshed + ListingCompleted`，并直接完成该请求，不再排队后台全量 `read_dir`；stale cache 不再直接用于 pane；`Reload` 不直接命中 cache 并会把该路径标记 stale；watcher add/delete/refresh 标记 stale，current-directory-removed 删除对应缓存；超过单目录 entry 上限的大目录不再被历史 cache 持有完整 payload。这先解决分屏同目录时重复 `read_dir`、worker result queue 重复大 `Vec<Entry>`、entries clone 中重复字符串分配、切目录后旧列宽 metrics 滞留、回到已缓存目录首帧空白和历史 cache 长期抓住大目录内存的问题。
  - 剩余验收：freshness 检查仍需继续对齐 Dolphin `KDirectoryListerCache` 的增量策略；大目录缓存还需要摘要或索引结构，当前策略是避免历史 cache 持有完整 payload；pane 关闭时是否写回/提升现有 `DirectoryModel` 快照仍需和 worker cache 统一；缓存统计还未接入性能调优日志或 UI。
...
- [x] 接入 pane-local navigation history。
  - 验收：Back/Forward 通过 `PaneId` 路由，切换 focused pane 不会改变历史事件目标；鼠标侧键按 Dolphin `BackButton` / `ForwardButton` 语义绑定到鼠标所在 pane，而不是全局焦点 pane。
- [~] 建立 chooser shell。
...
- [x] 实现滚动条平滑插值滚动。
  ...
  - 已完成：新增 `docs/SMOOTH_SCROLL_REFERENCE.md`，记录 Dolphin `KItemListSmoothScroller`、`KItemListContainer`、`KItemListView::setScrollOffset()` 和 `QScroller` 对应路径；新增 `src/core/scroll.rs`，提供 pane-local `SmoothScroll`、180ms `InOutQuad` 初始滚动、`OutQuad` retarget，并按 Dolphin `scrollContentsBy()` 的 `distance/currentOffset/oldEndOffset/endOffset/startOffset` 顺序执行中断动画的一帧前推；滚轮事件只更新目标并启动 16ms tick，tick 写回 pane `ViewState.scroll_x/scroll_y`，因此现有 compact visible range 虚拟化继续按当前 animated offset 计算；普通鼠标 wheel 的 line delta 按 compact-view 平均列宽和当前 pane viewport 换算，不再只按字体 line-height 横向移动；scrollbar drag 立即写 offset，释放后按采样速度启动惯性；viewport/content bounds、zoom、pane 关闭、目录切换都会清理 smooth scroll 状态；viewport 尺寸和 scroll clamp 用像素级容差比较，避免 fractional layout 抖动反复重置动画；snapshot 阶段先用 pane split 分配宽度投影 layout viewport，横向 scrollbar 在窄 pane 中按 viewport 宽高夹紧 track/handle，无法有效显示时隐藏，滚动条根控件不再使用旧 viewport 像素宽度参与布局，而是 `w_full + min_w_0 + max_w_full` 填满当前 pane 父容器，拖拽计算使用实际控件 bounds；core 导航/back/forward 重置 scroll 到 `0,0`；`ViewState` 保持 `f32`，GPUI 内容平移渲染时 round 到物理像素。
...
- [~] 建立 Dolphin 右键菜单源码执行流参考清单。
  ...
  - 剩余验收：继续补齐 Open in New Window、设备和拖拽相关 context menu 执行流。
- [~] 实现基础右键菜单（空白区域）。
...
- [~] 实现 pane item 拖拽源（drag source）。
  ...
  - 剩余验收：补 `text/uri-list` MIME data、内部/外部 copy/move/link action 和 Ctrl 切换。
- [~] 实现 pane item 拖拽目标（drop target）。
  - 参考：Dolphin `DolphinView::dropEvent()` 判断 drop action，对目录执行 move/copy/link 操作。
  - 已完成：pane file grid 已接收内部 item drag；拖到目录 item 时目标目录写入 pane-local transient `ItemDropTarget`，目录 tile 使用绿色 drop 高亮，和蓝色 selected 高亮区分；拖到非目录 item 或 pane 空白区域时 target 回到当前 pane 目录并可执行 drop；drop 执行复用 core file transfer primitives，支持 Copy/Move/Link mode 和受影响目录 refresh；GPUI 平台层 `ExternalPaths` 外部文件 drop 已接入 pane/目录 drop target，默认 Copy；breadcrumb path segment 已支持内部 item drag 和外部 `ExternalPaths` drop，drop 前会把目标 pane 导航到对应目录；drop target 设置会刷新 pane-local stale generation，若超过 3s 没有任何 drag-move 刷新则清理 pane/Places 高亮，避免 drag cancel/leave 后高亮残留；drop、chooser 拒绝、operation busy、pane lifecycle clear 都会清理 drag/drop transient state；新增 drop target state 测试。
...
- [x] 实现 Places 侧栏 drop target。
  ...
  - 已完成：Places sidebar 维护独立 `PlaceDropTarget`，和 pane drop target 互斥；拖到 place 行中部时执行内部 item drag Copy/Move/Link 或外部 `ExternalPaths` Copy 到该 place 目录；拖到行上下边缘或 section boundary 时显示 2px 蓝色插入线，并把单个目录插入为 user bookmark（写入 `user-places.xbel`），非目录或多路径 drop 给出状态提示；用户 bookmark 插入位置会夹在可持久化 user-place 区间，避免插入内置 Home/Trash 前后重启丢失顺序；已补 Places drop target 状态、bookmark 插入和拒绝路径测试。
- [~] 实现 pane 到侧栏、侧栏到 pane 的互相拖拽。
  - 验收：从 pane 拖到侧栏 → 行为如上（插入 bookmark 或 copy 到目标目录）；从侧栏拖到 pane → 导航到该目录。
  - 已完成：pane item drag 和外部 `ExternalPaths` 可拖到 Places 行/插入位置，分别执行 copy/move/link 到 place 目录或插入目录 bookmark；Places 内部 `PlaceDrag` 可拖到 pane 并导航目标 pane。
  - 剩余验收：补 Places drag source 的外部 `text/uri-list` MIME 发布。
- [~] 实现拖拽过程中的视觉反馈。
  - 参考：Dolphin 拖拽悬停在目录上时该目录条目显示高亮背景；拖拽悬停在侧栏条目之间时显示插入指示线。
  - 已完成：pane 目录 drop target 使用绿色背景，和 selected 蓝色背景、普通 hover 浅灰背景区分；pane 空白 drop target 使用浅绿色 viewport tint；breadcrumb path segment drag-over 使用同一绿色高亮；Places drop-on-row 使用绿色背景，drop-between 使用 2px 蓝色插入线；内部 item drag 和外部 `ExternalPaths` drag 移动到其它 pane/目录/Places target 会替换上一 target，drop 和 pane 生命周期清理会移除高亮；GPUI 没有内部 drag leave 回调，因此当前用 3s stale timeout 清理无后续 drag-move 的残留高亮。
...
- [~] MIME 类型自我识别。
  ...
  - 已完成补充：新增 `src/core/launcher.rs`，解析 `.desktop` `Desktop Entry`、`Desktop Action`、`MimeType=`、Exec field code 和 `mimeapps.list` 的 Default/Added/Removed Associations；`src/main.rs` 只消费 core 输出的 `MimeApplication` 列表生成 Open With 子菜单，不在 GPUI widget 内解析 desktop 文件；Open With launch plan 已转换为 systemd user transient unit 并通过 session bus `StartTransientUnit()` 启动。
  - 剩余验收：继续对齐 Dolphin `KFileItemModelRolesUpdater`，把昂贵 MIME/thumbnail/preview 角色拆到可取消的后台 role updater；支持 parent MIME 查询；Service Menu 使用同一 MIME 数据源生成 action。
...
- [~] Open With / Service Menu 完整实现。
  ...
  - 已完成：Open With 子菜单动态列出可打开单文件 MIME 类型的应用，默认应用排在顶部，`mimeapps.list` Added Associations 排在 desktop 自声明关联之前，Removed Associations 会从菜单中移除；`.desktop` `Actions=` 已在 core parser 中保留为结构化 `DesktopAction`；点击具体应用会生成 core launch plan 并通过 systemd user transient unit 启动，pane-local 状态反馈真实 launch 结果，不直接 `std::process::Command` 启动。
...
- [~] 为 core 和 GPUI shell 补齐任务级测试。
  - 已完成：core `ItemId` 稳定身份、rename/refresh 后 selection 跟随、cancellable directory listing、per-pane coalesced listing worker、compact select-all/exclusion、column-first compact layout、visible item virtualization、large-directory visible range bound、pane-local reusable visible item slot pool、recycled slot cap、horizontal scrollbar layout、pane-local scroll clamp、pane-local smooth scroll easing/retarget/kinetic drag velocity/navigation reset、Dolphin 中断滚动公式测试、分屏/窄 viewport 下 wheel line delta 列宽换算和 scrollbar 初始抓取点保持测试、pane-local zoom level / split inheritance / shortcut classification / zoom-derived compact layout options、pane-local filter matching / shortcut classification / input routing / filtered model projection / cache invalidation / navigation clear behavior、clipboard URI-list 编解码 / GPUI `ClipboardItem` metadata 往返 / URI-list 导入 / plain text 导入 / 文本 paste create undo、compact item `visual_rect` 按 required text width 收窄、系统主题图标解析/MIME 候选/主题继承/尺寸缓存测试、`.desktop` / `mimeapps.list` Open With 应用排序、Exec field code 和 systemd transient unit/property 生成测试、右键菜单 action 生成覆盖 Paste enabled state 和 Open With 动态子菜单、Open With pane 状态回写、目录 Open in New Pane、单目录 Paste、Properties 和多选批量菜单、pane-local 状态栏摘要/space info 格式化/zoom track 映射/进度百分比/internal paste progress 和取消路由、目录 loading state 的 request-key 生命周期、pane item drag selection payload / same-directory drop reject / drop target lifecycle / stale timeout、Places drop target 状态/插入 bookmark/拒绝路径、Places drag-to-pane 导航、Trash metadata 读取、Trash 删除时间排序、Trash reload/metadata refresh 保持 `ItemId`、Trash restore/delete permanently 操作结果和 affected dirs。
  - 剩余：完整 Trash Details 列和冲突对话框测试、缩略图管线测试、设备发现/挂载测试、拖拽测试、右键菜单 action 测试、Wayland 多 MIME clipboard backend 测试。
- [ ] 持续性能优化。
