...
- [~] **当前首要目标：pane 复用。** 所有 pane 必须共享 `ShellPaneState`、
  pane view/projection、scroll metrics、slot pool、layout adapter 和后续
  input/action routing 边界；禁止继续让某个 pane 以散落字段形成第二套实现。
