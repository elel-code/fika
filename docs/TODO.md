...
- [~] Devices sidebar.
  ...
  - Current: UDisks2 system-bus `ObjectManager` discovery is used as a best-effort enhancement for user-visible external media, including unmounted drives. It accepts media-backed drives that are marked removable, ejectable, or attached over the USB bus, while still respecting UDisks2 hidden/system block hints. Mounted mountinfo entries stay first and win duplicate paths, but UDisks2 still fills in operation metadata such as `/dev/...` `device_path` and eject support for those duplicate rows. UDisks2 failures fall back silently to mountinfo/directory discovery.
  - Current: UDisks2 display names follow the user-visible volume/mount identity first: explicit filesystem labels win, mounted media without a label use the mount-point name, and unmounted unlabeled media falls back to the drive vendor/model before the raw device path.
