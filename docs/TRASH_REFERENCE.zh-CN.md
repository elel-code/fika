...
- Places：
  - `src/main.rs` 暴露一个导航到回收站文件目录的回收站 place。
  - `FikaApp` 拥有回收站空/非空状态，类似 Dolphin 的 Places model 缓存
    `Trash::emptinessChanged`。它初始化一次状态，在回收站影响操作后刷新，
    从回收站 pane lister 事件更新，并在没有回收站 pane 打开时从 core
    `TrashEmptinessMonitor` 单例 watcher 获取外部变化。Places 投影消费
    该状态，不轮询文件系统。
  - `src/ui/places.rs` 以状态点和标记颜色渲染回收站状态。
