...
- [~] 输出 frame timing、visible range、draw-command counters、临时 text-atlas counters
  ...
  counters、visible slot reuse counters 和 thumbnail candidate counters；glyph-level
  以及 thumbnail atlas counters 会在对应 resource retention 层接入后开始。
...
- thumbnail candidates 已从可见 retained pane items 投影；已缓存 thumbnail 的 decode/upload
  和 atlas retention 是下一步。
...
actions 仍待完成。thumbnail 工作已有 visible-item candidate projection 和 frame telemetry，
但 decode/upload 与 atlas retention 仍待完成。更完整 Places actions/DnD、更完整 Trash
conflict handling、undo、更完整 properties、完整 inline rename、完整 Create New 子菜单/模板
和 Open With default-app selection 仍待完成。
