...
- [~] Icons、Compact 和 Details 的 layout/hit-test/paint 已共享同一 shell layout
  abstraction。主 pane 与 split pane 现在共享 `ShellPaneView`、`ShellPaneProjection`、
  generic item paint、scrollbar metrics，以及参考既有 Dolphin retained item-view
  工作的 path-keyed visible slot pool/reuse list。
