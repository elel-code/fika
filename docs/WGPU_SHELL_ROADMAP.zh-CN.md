...
后的 retained 宽度；Places splitter 和 split-pane divider hover 或 drag 时都会显示 `ColResize` cursor。Trash
为空时不再绘制蓝色状态圆点。当前 split pane 仍是最小可见骨架：focus、split-pane
scrollbar、真实 DnD 和 file operations 仍作用于主 pane，但 divider resize 已接入。multi-pane
工作现在已把主 pane 与右侧 pane 投影为同一个 `ShellPaneView`，并通过共享
`pane_layout(...)` 生成 Icons/Compact/Details layout；pane geometry/item hit-test
也已进入共享路径，split-pane paint 开始复用 `ShellPaneGeometry`。下一步必须继续把 pane
paint/event routing 接入同一个可复用 pane component，不能继续追加 split-only 特例。第一层 DnD
准备已通过 `ShellDropTarget` lookup 覆盖 primary/split pane item、pane blank、place row 和 Places
blank；真实 Wayland DnD hover/drop/export wiring 仍待接入。更完整 Places
