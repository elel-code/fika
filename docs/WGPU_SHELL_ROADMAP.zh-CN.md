...
- `src/bin/fika-sctk.rs` 已作为新的 backend spike 存在。它通过
  ...
  keyboard navigation、目录激活和 reload path。SCTK renderer 会上传并绘制
...
- [~] 初版 projection 的 scroll、hover、keyboard navigation、runtime mode switching、
  reload、hidden-file visibility、selection 和清空快捷键已走 `SctkPane` retained
  geometry。projection zoom、location editing、filtering 和全选仍待接入 SCTK shell。
