...
- 第一版 shell-owned Places 侧栏现在作为圆角浅色 panel 绘制，顶部与 app-level
  ...
  Places actions（sidebar add/edit 和 Trash actions）以及真实 Wayland DnD hover/drop/export
  仍留到 Phase 4。
...
- [~] 基本 pointer hover、鼠标 selection、keyboard navigation、全选/清空快捷键、右键
  ...
  DnD target lookup 已能解析 pane/place targets；真实 Wayland DnD hover/drop/export
  仍待完成。
...
- [~] Pane item/blank 右键 context target selection 以及第一版 shell-owned context menu
  ...
  处理。Device/place edit/hide/add action dispatch 仍待完成。第一版 `ShellDropTarget`
  lookup 已能通过共享 `ShellPaneView`/pane geometry 区分 primary/split pane item、pane
  blank、place row 和 Places blank；真实 Wayland DnD hover/drop/export wiring 仍待完成。
...
当前 split pane 仍是最小可见骨架：focus、split-pane scrollbar、真实 DnD 和 file
operations 仍作用于主 pane，但 divider resize 已接入。multi-pane 工作现在已把主 pane
与右侧 pane 投影为同一个 `ShellPaneView`，并通过共享 `pane_layout(...)` 生成
Icons/Compact/Details layout；pane geometry/item hit-test 也已进入共享路径，primary/split
pane item paint 已开始共用 `ShellPaneProjection` + generic pane item painter，主 pane
scrollbar metrics 也从同一 projection/metrics 读取。下一步必须继续把 pane focus、scrollbar
和 file-operation routing 接入同一个可复用 pane component，不能继续追加 split-only 特例。第一层
DnD 准备已通过 `ShellDropTarget` lookup 覆盖 primary/split pane item、pane blank、place
row 和 Places blank；retained DnD hover state/telemetry 已有，但普通 pointer move 不启用额外
drop hit-test，等待真实 Wayland DnD event 接入。真实 Wayland DnD hover/drop/export
wiring 仍待接入。更完整 Places
