...
- 初版 thumbnail loading 现在复用现有 core thumbnail 架构，不再在 frame path
  ...
  失败 probe 会在 resolver 中缓存，steady redraw 不会反复重排不可能成功的 thumbnail。
  这仍是初版 visible-item 路径，还不是完整 Dolphin/KIO PreviewJob parity 或 read-ahead
  scheduler 集成。
- `[fika-wgpu]` 日志包含 view mode、window/UI scale、path、entry count、visible item count、thumbnail candidate count、
  ...
  icon resolve/raster time、thumbnail loaded/quad/deferred counters、text label count、text cache hit/miss count、text cache bytes、text atlas bytes、
...
- [~] 输出 frame timing、visible range、draw-command counters、临时 text-atlas counters
  ...
  thumbnail loaded/quad/deferred counters；glyph-level 以及长期 thumbnail atlas eviction
  counters 会在对应 resource retention 层接入后开始。
...
- thumbnail candidates 已从可见 retained pane items 投影。第一版 wgpu thumbnail worker
  ...
  thumbnailer registry commands，再把就绪 thumbnail 光栅化进 per-frame atlas 路径。完整
  Dolphin/KIO-style preview job cancellation、read-ahead scheduling、model role writeback
  和长期 thumbnail atlas retention 仍是下一步。
...
handling 和 frame telemetry；read-ahead scheduling、model role writeback 和长期 atlas
retention 仍待完成。Properties、Create、Rename、Open With 和 Trash conflict overlay
