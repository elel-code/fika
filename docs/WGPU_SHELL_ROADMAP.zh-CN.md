...
- `src/bin/fika-sctk.rs` 已作为新的 backend spike 存在。它通过
  ...
  `Ctrl/Meta+H` 和 `F5`/`Ctrl/Meta+R` 会转换成 `SceneCommand` 并分派到 active
...
- [~] 初版 projection 的 scroll、scrollbar drag、hover、keyboard navigation、
  ...
  和清空快捷键已走 `SctkPane`/`SctkScene` retained geometry。projection zoom、
  location editing、filtering、rubber-band 和全选仍待接入 SCTK shell。
...
- [~] Pane item/blank 右键 context target selection 以及第一版 shell-owned context menu
  ...
  处理。Mounted devices 已由 GIO snapshot 投影进 Places；实时 device monitoring 以及
