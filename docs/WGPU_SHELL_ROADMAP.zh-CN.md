...
- 初版 thumbnail loading 现在复用现有 core thumbnail 架构，不再在 frame path
  ...
  有界 read-ahead 集合，让滚动到附近图片时能复用已开始的后台工作。失败 probe 会在
  resolver 中缓存，steady redraw 不会反复重排不可能成功的 thumbnail。这仍是初版
  shell-local 路径，还不是完整 Dolphin/KIO PreviewJob parity 或 model-role writeback。
- `[fika-wgpu]` 日志包含 view mode、window/UI scale、path、entry count、visible item count、thumbnail candidate count、
  ...
  icon resolve/raster time、thumbnail loaded/quad/deferred/read-ahead counters、text label count、text cache hit/miss count、text cache bytes、text atlas bytes、
...
- [~] 输出 frame timing、visible range、draw-command counters、临时 text-atlas counters
  ...
  thumbnail loaded/quad/deferred/read-ahead counters；glyph-level 以及长期 thumbnail atlas
  eviction counters 会在对应 resource retention 层接入后开始。
...
- thumbnail candidates 已从可见 retained pane items 投影。第一版 wgpu thumbnail worker
  ...
  Visible request 会优先于 deferred work，且有界 Dolphin-order read-ahead queue 现在会覆盖附近 offscreen thumbnails。完整
  Dolphin/KIO-style preview job cancellation、model role writeback 和长期 thumbnail atlas
  retention 仍是下一步。
...
已接入，model role writeback 和长期 atlas retention 仍待完成。Properties、Create、Rename、Open With 和 Trash conflict overlay
