...
- `src/core/launcher/ark.rs`
  - 通过 `ark` 命令构建 `Compress`、`Extract Here`、`Extract To` 启动计划。
  - Ark 执行复用 Open With 和 service-menu 动作使用的 `DesktopLaunchPlan` 与 systemd-user launcher。
- `src/main.rs`
  - 添加 Fika 自有右键动作 ID：`fika.builtin.ark.compress`、`fika.builtin.ark.extract-here`、`fika.builtin.ark.extract-to`。
  - 对本地文件/目录条目选择显示 `Compress...`。
  - 仅对单个本地归档显示 `Extract Here` 和 `Extract To...`，优先使用 MIME 信息，扩展名作为后备。
  - 不对 Trash 或纯网络 URI 目标暴露 Ark 本地动作。
  - 当已发现的 service menu 提供相同可见标签时，按标签去重，避免重复归档动作。
