...
`src/ui/pane.rs` and `src/ui/file_grid.rs` no longer create, size or mount the
item-view scrollbar. `src/ui/file_grid.rs` only records the item viewport's
window geometry. `src/main.rs` mounts `src/ui/item_view/scroll_bar.rs` as a
