...
- Static item label、Details cell/header 和 Places row label 现在共用
  ...
  各自复制。Places 还通过同一个 cache primitive retained GPUI glyph-raster paint
  数据；这是 Fika GPUI fork 中 `ShapedLine::compute_glyph_raster_data` 和
  `ShapedLine::paint_with_raster_data` hook 的第一个消费端。
- File-grid static text 和 Details text 的 retained glyph-raster miss 现在按可见层帧预算
  ...
  Static opposite-mode warm/read-ahead 层排在真实可见层之后，并使用小预算，不能抢当前可见
  内容的 glyph miss 预算。新增 `[fika item-glyph-budget]` 和
  `[fika details-glyph-budget]` 作为冷 miss 峰值画像。
