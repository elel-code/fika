...
- 缩略图图像和 MIME/主题图标都由保留的内容级图像层绘制，该层复用 GPUI 的 `RetainAllImageCache -> RenderImage -> Window::paint_image` 路径；GPUI `img()` 是显式 baseline/fallback，不是默认 image 架构
- 重命名编辑器可以保持为专门的 GPUI 子路径，直到其平台契约可被替换；拖拽启动已经通过 Fika GPUI fork 由 retained hitbox 拥有
...
- 当 GPUI 拥有硬平台契约时优先使用 GPUI 内置元素，例如文本编辑，或经测量优于自定义层的图像/缓存路径。
...
- 除非替换物在冷加载、缩放、内存和 SVG/主题行为方面全面优于它，否则不要替换 GPUI 自有的图像解码/RenderImage 后端；默认渲染器仍应由 retained image layer 驱动。
- Places 渲染器：默认已经是 full retained/custom row visual 加 retained-hitbox event/DnD delivery；GPUI/chrome/text policy 只作为显式 baseline 保留。
...
- 自定义绘制的条目视觉消费快照并绘制 quads/文本/缩略图和 MIME/主题图像；GPUI `img()` 主题图标仅作为显式 baseline/handoff renderer 消费保留条目快照。
...
- 缩略图角色成功/失败保持 model 驱动，图像绘制层仅在已尝试相同缩略图的保留图像后才绘制后备。MIME/主题图标默认使用同一个 retained custom image layer；`FIKA_GPUI_THEME_ICONS=1` 强制旧 GPUI `img()` baseline，`FIKA_HYBRID_THEME_ICONS=1` 保留显式 readiness handoff 对照路径。
...
非重命名条目和 Details 交互由 retained hitbox 拥有：

- 稳定 hitbox 从 retained Compact/Icons visual rect 和 Details row rect 插入
- hover、cursor、click/menu/drop targeting 和 typed drag start 都通过 retained interaction layer 路由
- 剩余 item slot `Div` 是显式 GPUI image baseline 和重命名编辑的 layout/overlay carrier，不是 DnD/event shell

Viewport 级 hit testing 对于普通点击、右键菜单、中键点击、rubber-band 框选和放置目标路由保持权威。

Pane 内部条目拖拽悬停不由 GPUI 每元素 `on_drag_move` 拥有。运行时证据显示条目自拖拽可以成功启动，但后续元素拖拽移动回调不会被传递。因此，保留交互层在存在 `ActiveItemDrag` 时安装窗口鼠标跟踪器。该跟踪器将当前窗口位置路由通过保留 pane hit-test，并更新由 Places 到 pane 和外部路径放置使用的相同 `ItemDropTarget` 状态。当平台/后端在同一窗口活动条目拖拽期间不传递底层移动回调时，GPUI 拖拽预览重绘使用当前窗口鼠标位置运行相同的保留 hit-test 更新。拖拽启动现在通过 Fika GPUI fork 注册在 retained hitbox 上。

#### 同窗口条目拖拽悬停根本原因

2026-06-17 运行时追踪隔离了 pane 内部悬停缺失：

- `item-start` 被发出，因此 retained hitbox drag-start 路径创建了 `ItemDragPayload` 并且 Fika 填充了 `ActiveItemDrag`。
...
因此具体原因不是过时的条目几何、目录拒绝或放置目标绘制。它是事件源：在同窗口 app-internal 条目拖拽期间，底层的 pane/条目拖拽移动回调可能在拖拽启动后不被传递。retained-hitbox 路径现在拥有拖拽启动；当后端不提供更强的 active-drag move callback 时，拖拽预览重绘仍是 pane 自拖拽悬停的已接受 fallback tick。

重命名条目保留现有编辑器子树。在阶段 8 之前，缩略图和主题图标条目使用 pane 本地图像缓存下的 slot 稳定保留 `img()` 元素。阶段 8 将缩略图移动到自定义绘制层；后续 retained icon cache 工作又将 MIME/主题图标默认移动到同一个 full custom image layer。

当前 Compact/Icons item slot 位于 `src/ui/file_grid/item_shell.rs`，不再包含每条目静态文本视觉子元素、默认图像元素或 DnD/event handler。基础视觉、缩略图和 MIME/主题图标默认由内容级自定义绘制层拥有；slot 仅为显式 GPUI image baseline 和 rename overlay 保留。
...
- 在该阶段每个条目 slot 仍保留为透明交互/拖拽边界；当前代码已将交互和拖拽启动移动到 retained hitbox
...
- 在该阶段每个非重命名条目 slot 仍携带透明交互/拖拽边界；当前代码只保留 slot carrier，交互和拖拽启动由 retained hitbox 拥有
- 重命名条目保持当前子子树和编辑器行为
...
用自定义绘制元素替换内容级缩略图 `img()` 层；MIME/主题图标现在默认也使用该 retained image layer：

- 继续使用 GPUI 的 `ImageAssetLoader` 和 pane 本地 `RetainAllImageCache` 进行缩略图路径加载、图像解码和渲染图像生命周期
- 对于 MIME/主题图标，保留相同的 GPUI 图像缓存解码路径，但按 semantic icon key 保留并通过 `Window::paint_image` 绘制；仅在 `FIKA_GPUI_THEME_ICONS=1` 时使用旧 GPUI `img()` baseline
...
- 默认主题图标使用 retained custom image layer；custom image 运行不得将已加载的同 `iconName` 图像替换为标记、空白矩形或无关后备
- 已加载图像边界匹配 GPUI `ObjectFit::Contain`
- 图像缓存状态保持 pane 本地，并随 pane/层释放

### 阶段 9：绘制交互 Hitbox

分两步将条目交互移出每条目 `Div` shell：先迁移 retained hover/cursor hitbox，再通过 Fika GPUI fork 迁移 retained drag-start hitbox。
...
- 悬停和光标通过保留 slot 表路由
- 在 P9b 之前，每条目 shell 仅作为 GPUI 拖拽源边界保留
...
- P9a 性能证据不是单独改变拖拽行为的许可；retained hitbox drag start 仍由 DnD smoke 和 `gpui_drag_shell=0` gate 守卫

#### 阶段 9b：拖拽源 Hitbox

通过经过审计的 Fika GPUI retained-hitbox patch 移除剩余的非重命名每条目拖拽 shell：

- 拖拽源从保留 hitbox 启动
- Compact/Icons 非重命名条目不再分配每条目 GPUI DnD/event shell；剩余 slot carrier 只服务显式 image baseline 和 rename overlay
...
- P11b 将行背景、图标和文本单元格移入内容级自定义视觉层。拖拽启动已经安全移动到 retained row hitbox，并由行为/性能证据守卫。
- P11c 保持保留详情行数据显式：路径、目录标志、名称/图标、选择计数、放置目标状态和 drag-start 字段从保留行快照投影并受测试覆盖。回收站独有列也投影到视觉层单元格中。
...
- P11c 证明回收站视觉列在绘制器迁移中存活，且保留详情行数据仍携带 retained hitbox drag-start registration 所需的字段。
- P11d 保持详情绘制计时可通过 `[fika details-visual]` 归因，详情文本缓存活动可通过 `[fika details-shape-cache]` 归因，同时 retained-hitbox 交互和 DnD 继续由 analyzer gate 覆盖。
...
- Compact/Icons retained item hitbox：通过 Fika GPUI fork 拥有 typed drag start。它们不携带 GPUI `img()` 或静态文本视觉子元素；视觉、图像、悬停/光标、点击/菜单/放置 hit testing 和拖拽覆盖状态是保留/绘制器驱动的。
- Details retained row hitbox：通过 Fika GPUI fork 拥有 typed drag start。行视觉、放置分发和行悬停/点击/菜单/导航是保留/绘制器或 viewport 驱动的。
- 重命名叠加层：用于 caret hit testing、选择、警告/错误助手文本和光标文本行为的文本编辑边界。

Fika GPUI fork 添加了该边界需要的 retained-hitbox typed drag/drop hook。后续变更必须保持 `gpui_drag_shell=0`，并避免为了 typed DnD 重新引入可见 GPUI row/item element。
