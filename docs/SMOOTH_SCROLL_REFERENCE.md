...
- Dolphin `KItemListContainer` owned scrollbars -> no active Fika UI mapping
  exists after the deletion pass. `src/ui/scrollbar.rs` and
  `src/ui/item_view_container*` have both been removed.
- Dolphin scrollbar maximum invalidation, `updateGeometries()`, and
  track/thumb geometry still define the required rewrite target. The next Fika
  implementation must rebuild those responsibilities as a new container layer.
...
- Dolphin `QScroller` kinetic gesture path is represented in core by
  `ScrollDragTracker` plus kinetic `SmoothScroll`, but it belongs to viewport
  gesture scrolling. Dolphin's scrollbar mouse release only clears the pressed
  scrollbar state and disables smooth-scrolling continuation; Fika keeps that
  split and does not start kinetic scrolling on scrollbar release.
...
- The previous GPUI app-side smooth-scroll bridge and pane scrollbar drag/cache
  ...
  cached scrollbar track, item-view scrollbar value layer, or pane wheel smooth
  tick in the current UI code.
- Ordinary pane wheel scrolling is intentionally not wired to the removed path.
  Ctrl/secondary+wheel remains routed to pane-local zoom.
...
- The removed horizontal scrollbar widget used a GPUI canvas, pane-local drag
  state and cached track snapshots. Those files are gone; the next scrollbar
  must be a new Dolphin-style container component.
