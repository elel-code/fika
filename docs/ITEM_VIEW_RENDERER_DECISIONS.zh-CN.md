...
截至 2026-06-20，普通 MIME/theme icon 不再有 GPUI image-element 或 hybrid
readiness-handoff 运行时开关。当前路径是 Dolphin-style self paint：file roles 解析
`iconName`/theme path，item image layer 绘制按 `ThemeIconImageKey`（icon name、请求尺寸、
DPR/scale、mode sentinel）键控的 retained pixmap。可见 SVG cache miss 会在
`[fika item-image-cache-refresh]` 中、image/details visual prepaint 前处理，对齐
Dolphin `paint() -> triggerCacheRefreshing() -> updatePixmapCache()` 边界。下面的
GPUI/hybrid 日志只保留为历史考古，不再是当前架构。

对 startup MIME blank、zoom size jump 和 `/etc` zoom smoothness 做调查时，必须先对照本地
Dolphin 源码，再接受新的 custom image-layer 变更：

- Dolphin 源码：`KStandardItemListWidget::paint()` 先触发 cache refresh；
  `KStandardItemListWidget::updatePixmapCache()` 和 `pixmapForIcon()` 同步生成当前尺寸
  pixmap，并按 icon name/height/DPR/mode 缓存；paint body 消费 `m_pixmap`。
- 当前 Fika self-painted 路径：默认运行不设置 image renderer override。缩略图和
  MIME/theme icon 都通过 custom image layer 绘制，普通 pane 日志必须保持
  `gpui_image_element=0`。
- 决策规则：如果 self-painted 路径重新出现可见 first-load placeholder 或 zoom-time
  decode/size churn，保留 retained model/projection/controller 拆分，修 pixmap-key cache
  或 visible-first role updater，不得重新引入 GPUI image children。

2026-06-20 cold-frame 检查：

- 旧 self-painted SVG load 路径：
  `/tmp/fika-dolphin-icon-cache-zoom-scroll.log` 有
  `item-image max_prepaint=7437us`，并在可见 image-prepaint 中出现
  `theme_decoded=5`。
- 当前 cache-refresh 路径：
  `/tmp/fika-dolphin-cold-cache-refresh-zoom-scroll.log` 有
  `item-image max_prepaint=156us`、`theme_loaded=0`、`theme_decoded=0`、
  `theme_retained=550`、`theme_placeholder=0`，且
  `renderer_policy max_gpui_image_element=0`。
- Cold work 已移到 `item-image-cache-refresh`：
  `requested=550 retained=545 loaded=5 decoded=5 max_total=8904us`。这属于预期的
  Dolphin-style cache-refresh/build 边界，不是 image prepaint 延迟。
- Scroll-end 内存按 `scroll_x=2303.5 max_scroll_x=2303.5` 后采样：
  `/tmp/fika-dolphin-cold-cache-refresh-scroll-end-memory-rerun.log` 显示
...
readiness handoff 基础当时位于 `FIKA_HYBRID_THEME_ICONS=1` 后面。app 拥有 size/scale
...
这是让 MIME/theme icon 离开 GPUI `img()` 且不重新引入 placeholder/decode churn 的中间步骤。
...
`theme_prewarm_pending=44`、`max_paint=378us`。这支持了后续默认策略代码切片；最终默认值在
retained semantic cache、source-image reuse、prewarm 和预算控制落地后继续推进到 full custom。
...
## 2026-06-19 Pane 可见集合级 Image Handoff（历史中间态）
...
决策：该阶段保留可见集合级 handoff，因为它减少可见切换且没有重新暴露 visible
placeholder。这个结论后来被 full custom 默认路径取代：semantic cache、source-image
reuse、app-level prewarm 和 cache budget 落地后，pane MIME/theme icon 默认改为 full
custom，`FIKA_GPUI_THEME_ICONS=1` 保留为 GPUI baseline。
...
图标复用从 render-frame 热路径移出去。后续 image 工作转向消除剩余 GPUI `img()`
fallback 边界、降低 cold first-resolve 成本和收束 cache 生命周期，而不是继续处理
cached same-kind lookup。

## 2026-06-19 Places Full Row Visual Default

Places full row visual 路径现在是默认值，不再是 opt-in 提升候选。

当前 full 路径是：

- `DEFAULT_PLACES_ROW_VISUAL_POLICY = CustomFull`；测试需要显式表达默认值时也可设置
  `FIKA_PLACES_ROW_VISUAL_POLICY=full`。
- 文本和 vector icon 在 retained row visual layer 中绘制。
- `FIKA_PLACES_ROW_VISUAL_HANDOFF=1`：ready-only handoff。warmup 帧继续显示 GPUI
  ...
  full custom paint。该路径现在作为回归 suite，而不是默认提升前置条件。
...
决策：保持 Places rows 默认 `CustomFull`。Chrome、text 和 GPUI row policy 只作为
analyzer 覆盖的 fallback/A-B 路径。core evidence 现在直接用
`--expect-custom-row-full-policy` gate 默认 full policy。
...
突破：旧的 8-14ms chrome icon 尖峰已经消失。该 owner 清理随后成为 `CustomFull`
能进入 core evidence 默认 gate 的前置证据。
...
当时实现：`FikaApp` 拥有 pane theme `RenderImage` cache。在构建 `PaneSnapshot` 时，
...
`RenderImage`，写入 app cache，并把这些语义 key 标记 ready。

这个 2026-06-19 snapshot-prewarm 边界已在 2026-06-20 被 file-grid surface
cache-refresh 边界取代。Cache 仍归 app 所有，但当前 surface 会在构造 image/details
visual layer 前刷新可见 SVG theme-icon key；visual prepaint body 只通过
`Window::paint_image` 消费 retained image。
