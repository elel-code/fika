...
| Places rows 和 sidebar scrollbar | 默认 custom chrome layer 加 GPUI text/icons、retained-DnD mixed event delivery、一个 sidebar typed DnD payload shell 和 GPUI row drag-start shell；`gpui` fallback、`FIKA_CUSTOM_PLACES_ROWS=1` text-only 基准路径和 `FIKA_PLACES_ROW_VISUAL_POLICY=full` text+vector-icon 基准路径仍可用 | `places` model/projection、`places/interaction.rs`、retained event layer 和 `drag_drop` state | 保持 Dolphin 对齐的 chrome 拆分和 retained-DnD mixed event policy 为默认。文本/图标、typed DnD payload delivery 或 drag start 在 Fika 自己拥有的 retained/static cache 与 hitbox 达到或超过 GPUI 前不得移出 GPUI。这不应被理解为等待新的 GPUI API：Dolphin 也是通过稳定 item 状态和应用层 static text/pixmap cache 解决，Fika 必须证明等价的 retained handoff 后才能晋升默认 | 默认日志必须通过 `--expect-custom-row-chrome-policy` 和 `--require-interaction-policy`，并显示 `event_policy=retained-dnd`、`retained_hitboxes=rows+sections`、`gpui_event_shells=1`、`gpui_row_section_event_shells=0`、`gpui_typed_dnd_payload_shells=1`、`gpui_sidebar_leave_shells=0`、`text_gpui=rows`、`visual_kind=chrome`、无 row shape-cache 日志，且聚合 `[fika places-row-visual]` rows 匹配策略行数。GPUI fallback 必须通过 `--expect-current-gpui-policy`；text-only 继续由 `--expect-custom-row-visual-policy` 约束；full text+icon 必须通过 `--expect-custom-row-full-policy` 且 `max_icon_gpui=0`，并同时通过显式 `--row-visual-paint-us` 与 `--row-visual-warm-paint-us` 门后才可晋升 |
