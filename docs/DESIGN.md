...
- Slint 侧把虚拟 tile 放进以 `virtual_start_column + virtual_start_row` 为锚点的局部 layer；tile local x/y/width/text_width 由 Rust-projected `ItemViewBoundsEntry` sidecar 提供，避免超大目录产生很大的每项坐标，也允许未按当前 `rows_per_column` 对齐的缓存窗口在 zoom 后继续复用。
