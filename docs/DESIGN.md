...
- 设置 `FIKA_DEBUG_PORTAL=1` 时，backend 还会为每个 OpenFile / SaveFile / SaveFiles 请求打印一行请求摘要，包含 request handle、起始目录、选择/保存模式、filter/choice 数量、初始 filter、parent-window 转发状态，并继续显式标记 `native_transient=false`。chooser future 结束时还会记录收尾原因：选择成功、用户取消、空输出、portal request Close、Close stream 结束或异常失败，方便验证取消清理和子进程生命周期。
