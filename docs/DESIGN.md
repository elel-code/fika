...
- `ui/split_pane.slint` 负责主栏 viewport、pane-level input/DnD、横向滚动条和当前可见 tile overlay 渲染；选择、命中、右键、激活、DnD payload 语义由 Rust item-view controller 决定，可见 tile 的 size/media/text rect、每项 bounds、selection background、drop target 和 thumbnail raster 由 Rust render plan/raster layer 下发，基础 file/folder fallback icon 由 PaneView 按尺寸/主题缓存后通过 paint loop 贴图，避免写入每个 `ItemViewEntry` row 或走重 raster 刷新。
