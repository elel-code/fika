...
- `ui/split_pane.slint` 负责主栏 viewport、pane-level input/DnD、横向滚动条和当前可见 tile overlay 渲染；选择、命中、右键、激活、DnD payload 语义由 Rust item-view controller 决定，可见 tile 的 size/media/text rect、每项 bounds、selection background 和 folder/file fallback glyph 由 Rust render plan/raster layer 下发，避免写入每个 `ItemViewEntry` row 或 Slint per-item fallback loop。
