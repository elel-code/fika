...
- 菜单外观和通用交互件集中在 `ui/widgets.slint`：`MenuItem` / `MenuSeparator` / `MenuTitle` / `PopupSurface` / `MenuHoverBridge` / `MenuDismissLayer`。具体菜单内容和弹出层承载组件在 `ui/menus.slint`；`RootContextMenuLayer` 统一挂载 file / Places / Devices / blank-area 菜单，`TransferMenuLayer` 统一挂载拖放操作菜单，`ChooserChoicePopupLayer` 统一挂载 chooser choice 弹出菜单并封装锚点定位公式，`ui/app.slint` 保留菜单动作、父子菜单生命周期和对 Rust 菜单几何 callback 的绑定。所有右键入口通过 `show-context-menu()` 统一写入 kind、触发坐标、关闭子菜单并停止延迟关闭 timer。Open With / Create New 的父项、hover bridge 和子菜单内容都走同一个 child-submenu hover/timer 入口，chooser-choice popup 保留从按钮上方弹出的语义但边界 clamp 也由 Rust helper 计算。
