...
- winit fallback 的 source/mime 常量、禁用开关解析、启动诊断摘要和 DnD trace 格式化集中在 `src/app/dnd.rs`；`main.rs` 只保留实际 winit 事件接入、debug 开关判断和 UI wakeup。
...
- 搜索 strip 的布局集中在 `ui/search_panel.slint` 的 `SearchPanel`。`ui/app.slint` 只保留查询/过滤状态、后端 callback 转发和主栏高度计算。
