# Fika Design: GPUI Baseline and winit/wgpu Direction
...
`docs/WGPU_SHELL_ROADMAP.md`: a Linux-focused, Fika-specific shell built on
official upstream `winit` master and official crates.io `wgpu`. The old SCTK
spike is now experimental/reference code, not the target window/event backend.
