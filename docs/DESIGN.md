...
- 设置 `FIKA_DEBUG_DND=1` 启动 Fika 时，会把 Places 外部 drop 诊断输出到 stderr，包括 backend、phase、MIME type、逻辑坐标、slot、target、gap/item 状态和截断后的 payload 摘要。这个开关同时覆盖 Slint `DropArea` 和 winit `DroppedFile` fallback，方便对比真实桌面环境到底走了哪条路径。
