...
- 设置 `FIKA_DEBUG_DND=1` 启动 Fika 时，会先把 DnD 启动配置输出到 stderr，包括 Slint DropArea 接受的 MIME 和 winit `DroppedFile` fallback 是否启用；之后每次 Places 外部 drop 也会输出 backend、phase、MIME type、逻辑坐标、slot、target、gap/item 状态和截断后的 payload 摘要。主栏 DropArea 也会输出 can-drop / dropped、rejected 状态和目标文件夹路径。Slint `can-drop` 拒绝时，phase 会附带 `unsupported-mime`、`empty-payload` 或 `no-local-file-path`，实际 drop 解析失败时状态栏也会显示对应原因。这个开关同时覆盖 Slint `DropArea` 和 winit `DroppedFile` fallback，方便对比真实桌面环境到底走了哪条路径。
