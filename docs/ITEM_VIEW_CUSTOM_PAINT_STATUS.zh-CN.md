...
在此之前，保持 Places 在 GPUI 元素上，由保留 places 投影和拖放状态馈送。`FIKA_CUSTOM_PLACES_ROWS=1` 路径仅是一个可选基准表面；它不替换 GPUI 行事件传递、GPUI 图标、行右键菜单 shell、行 DnD 或拖拽启动 shell。溢出证据现在通过 `FIKA_AUTOSMOKE_PLACES=overflow` 可用，它添加非持久仅快照行并验证 `[fika places-scrollbar] visible=1`。
