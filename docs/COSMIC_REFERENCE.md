...
- The desired layer model is stable: the window-wide shell/header row owns global tools and search. Below it, the sidebar content panel and right main pane share one equal-height content row; the right main pane starts with `PathBar`, then the search filter strip, horizontal column-first file view, and status bar.
