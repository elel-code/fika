...
- Current shell direction: the top bar and main pane share one calm base surface, while the sidebar content below the top bar is treated as a raised rounded panel with only a narrow resize divider between the two layers.
...
- Current chrome pass: sidebar content geometry uses explicit panel margin/radius tokens (10px / 16px) rather than ad-hoc offsets, keeping the below-header rounded panel consistent as future COSMIC-style sidebar polish continues.
