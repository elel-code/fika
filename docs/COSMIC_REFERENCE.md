...
- Navigation, address entry, search, split, and theme controls belong at the top of the right main pane inside the below-header content row. They do not belong in the window-wide shell/header row.
...
- The desired layer model is stable: the window-wide shell/header row is separate; below it, the main-pane toolbar is the first row inside the right main pane, and the search strip, grid, and status bar continue below it. The sidebar content panel starts in that same below-header row and is equal-height with the right main pane, not a window-height overlay.
...
- Current shell direction: the window-wide shell/header row is kept separate from navigation/address controls. Below it, the main-pane toolbar and file area share one calm base surface while the sidebar is treated as a raised rounded panel in the same content row; its right border is the visible divider, and the resize hit area is transparent without adding layout width.
...
- Current chrome pass: Back/Forward/Up, the path entry, search, split, and theme controls now live as the first row inside the right main pane in the below-header content row, rather than in the global shell/header.
