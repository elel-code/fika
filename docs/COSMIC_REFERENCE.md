...
- Current shell direction: the window-wide shell/header row owns search and global tools, while navigation/address controls stay in the right main pane's `PathBar`. Below the shell/header, the path bar and file area share one calm base surface while the sidebar is treated as a raised rounded panel in the same content row; its right border is the visible divider, and the resize hit area is transparent without adding layout width.
...
- Current chrome pass: `AppWindow` owns shared base/sidebar/separator color tokens; `TopBar`, `PathBar`, `SearchPanel`, and `StatusBar` stay transparent and only draw separator lines, while path/search fields use quiet white/dark input surfaces so the light theme remains readable.
