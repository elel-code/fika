...
| Details header, row backgrounds, icons, and text cells | custom content-level painter | Details paint slots, image cache, text shape cache, glyph-raster cache, glyph-raster miss budget, background file-icon resolve queue | Keep custom paint. Render frames use cached/preliminary icon snapshots only. The header background, separators, and labels are painted by the Details visual layer rather than GPUI child elements. Details text follows the Places model: retained `ShapedLine` plus retained GPUI glyph-raster paint data, while GPUI remains the backend text substrate. Cold glyph-raster misses are budgeted across the header and visible rows; over-budget text falls back to normal text paint and continues cache fill on subsequent frames. | Logs must include `[fika details-visual]`, `[fika details-shape-cache]`, `[fika details-glyph-cache]`, and `[fika details-glyph-budget]` with no steady build regression or synchronous icon-theme lookup spike. Renderer policy must keep `gpui_details_header=0`. |
