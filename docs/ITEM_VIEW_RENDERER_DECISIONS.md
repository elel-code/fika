...
| Places rows and sidebar scrollbar | Default custom chrome layer plus GPUI text/icons, retained-DnD mixed event delivery, one sidebar typed DnD payload shell, and GPUI row drag-start shells; `gpui` fallback, `FIKA_CUSTOM_PLACES_ROWS=1` text-only benchmark, `FIKA_PLACES_ROW_VISUAL_POLICY=full` text+vector-icon benchmark, and opt-in `FIKA_PLACES_ROW_VISUAL_HANDOFF=1` ready-only benchmark remain available | `places` model/projection, `places/interaction.rs`, retained event layer, and `drag_drop` state | Keep the Dolphin-aligned chrome split and retained-DnD mixed event policy as default. Do not move text/icons, typed DnD payload delivery, or drag start out of GPUI until Fika-owned retained/static caches and hitboxes match or beat GPUI. This is not blocked on a new GPUI API: Dolphin also solves this through stable item state and static text/pixmap caching, so Fika must prove an equivalent retained handoff before default promotion. | Default logs must pass `--expect-custom-row-chrome-policy` and `--require-interaction-policy` with `event_policy=retained-dnd`, `retained_hitboxes=rows+sections`, `gpui_event_shells=1`, `gpui_row_section_event_shells=0`, `gpui_typed_dnd_payload_shells=1`, `gpui_sidebar_leave_shells=0`, `text_gpui=rows`, `visual_kind=chrome`, no row shape-cache logs, and aggregated `[fika places-row-visual]` rows matching policy rows. GPUI fallback must pass `--expect-current-gpui-policy`; text-only remains behind `--expect-custom-row-visual-policy`; raw full text+icon must pass `--expect-custom-row-full-policy` with `max_icon_gpui=0`; ready-only full handoff must pass `--expect-custom-row-handoff-policy` plus explicit `--row-visual-prepaint-us`, `--row-visual-paint-us`, `--row-visual-warm-prepaint-us`, and `--row-visual-warm-paint-us` gates before promotion. |
