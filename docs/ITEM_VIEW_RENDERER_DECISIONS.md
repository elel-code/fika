...
| Places rows and sidebar scrollbar | Default custom chrome layer plus GPUI text/icons/event shells; `gpui` fallback and `FIKA_CUSTOM_PLACES_ROWS=1` full-text benchmark remain available | `places` model/projection and `drag_drop` state | Keep the Dolphin-aligned chrome split as default. Do not move text/icons or event delivery out of GPUI until retained/static caches and hitboxes match or beat GPUI. | Default logs must pass `--expect-custom-row-chrome-policy` with `text_gpui=rows`, `visual_kind=chrome`, no row shape-cache logs, and aggregated `[fika places-row-visual]` rows matching policy rows. GPUI fallback must pass `--expect-current-gpui-policy`; full text remains behind `--expect-custom-row-visual-policy`. |
