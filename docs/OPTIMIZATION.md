       ...
       └─ root.view_changed() → PaneRouting.view-changed(slot)
            └─ Rust: PaneViewSyncScheduler 同步重建当前 visible slice
                 └─ sync_virtual_entries_for_slot()  [src/main.rs]
                      ...
                      ├─ schedule_thumbnail_roles_for_entries() [file_item_roles_updater.rs]
                      │    └─ Dolphin KFileItemModelRolesUpdater-style indexesToResolve order
                      ├─ renderer batch derives paint/raster geometry；set pane-local raster/paint/media/metadata → Slint
...
Rectangle viewport shell (clip: true)
  ...
  │    ├─ single raster Image primitive
  │    │    └─ Rust 绘制当前 virtual slice 的选中背景 + folder/file fallback glyph + 已加载 thumbnail
...
主文件视图使用 Dolphin compact 模式语义：物理滚动轴固定为 X，条目按 `index % rows_per_column` 先填满一列，再按 `index / rows_per_column` 进入下一列。compact item 尺寸沿用 Dolphin 的公式：`itemWidth = padding * 4 + iconSize + fontHeight * 5`，`itemHeight = padding * 2 + max(iconSize, textLines * lineSpacing)`，列间 margin 为 `8px`。普通目录使用 1 行标题，Trash 和递归搜索使用 pane-local 的 3 行 group/title/location 布局；每个 pane 独立计算自身行数、可见范围和 viewport。Dolphin 对照点是 `KStandardItemListView::setItemLayout()` 在 CompactLayout 下设置 `Qt::Horizontal`，`KItemListViewLayouter` 通过转置逻辑把纵向流映射成物理横向滚动，`KStandardItemListWidget::updateCompactLayoutTextCache()` 预先计算 compact 文本位置和最大宽度，并把 compact text frame 关联到整个 item 高度；文字本身使用居中的 line position。Fika 同样在 Rust render plan 中投影非零 `title_y/title_line_height/text_width`；普通标题的 Rust-projected `Text` frame 覆盖整 tile 高度并由 Slint vertical alignment 居中，避免大 zoom 下单行 frame 被裁剪导致 name 消失；带 group/location 的标题行继续使用 Rust 投影的 line frame，额外 group/location 行通过 pane-local sparse metadata overlay 绘制。`PaneEntrySnapshot` 在目录条目进入 pane 时缓存 `name_width_units`，后续 virtual layout 和 cached zoom relayout 直接按当前字体尺寸缩放这些宽度单位，不再在 zoom/切目录热路径反复 clone 文件名或逐字符估宽。基础 `ItemViewEntry` 热 row 只留在 Rust `PaneView` 内部，携带业务身份、目录标记、缩略图状态和 media token，供 controller、hit-test、DnD 和 thumbnail token sidecar 使用；它不再作为 `PaneViewData` 字段传给 Slint。Slint 基础 name loop 消费 pane-local `ItemViewPaintEntry`，selection background、folder/file fallback glyph、已加载 thumbnail image 和当前 drop target 都由 Rust 生成当前 virtual slice 的 `item_view_raster_layer` 底图。media/metadata 的 `slice_index` 只保留在 Rust-only `ItemViewMediaSource` / `ItemViewMetadataOverlaySource` 和 media token sidecar 中，已加载 thumbnail image 只作为 Rust-only raster 输入，不再发布 Slint-facing media row。drop target 通过 pane-local transient `drop_target_slice_index` 进入 raster input，因此完整 `ItemViewItemBounds` 模型不再进入 `PaneViewData`。text/title 几何挂在 pane-level `PaneViewData` 上；selection/fallback/drop-target geometry 由 tile frame batch 按当前 row token + bounds 临时生成，thumbnail media 作为 Rust-only raster input 注入，不再进入 Slint-facing model。metadata 由 renderer 的 text projection 边界投影成 `ItemViewMetadataEntry` 稀疏 rows。当前 title/metadata backend 仍是 Slint 原生 `Text`，文字 raster 只作为后续可选实验保留，必须以实测快于原生 `Text` 为前提。`SplitPaneView` 只消费 raster 底图、pane-level 几何和 paint/metadata overlay sidecar，不再在可见 title loop 内按 `group/location` 或 `is_dir` 做 per-item 判断。

### 现有优化

补充进展：`ItemViewPaintEntry` 已继续收窄为 title 文本和 Rust-projected 绘制几何，不再携带 `is_dir` 或绘制 loop 不读取的 `slice_index`。selection 和 folder/file fallback 不再维护独立 sidecar model，而是由 `ItemViewTileFrameBatch` 直接从 row token + bounds 生成当前 virtual slice 的 `item_view_raster_layer`；已加载 thumbnail 仍通过 Rust-only `ItemViewRasterMediaEntry` 覆盖 fallback glyph。Slint 不再有 highlight loop、folder-media loop、file-media loop 或 thumbnail raster loop。`ItemViewMetadataEntry` 不再携带 Slint-facing `slice_index`，身份只保留在 Rust-only source/token 中。

| 措施 | 位置 | 效果 |
|------|------|------|
| 虚拟化：Slint 只接收可见范围条目 | `virtual_view.rs` | 大目录不实例化全部 tile |
| 缓存命中免重建：`should_rebuild_virtual_model` | `virtual_view.rs:180` | 同范围内滚动零模型更新 |
| Dolphin-style thumbnail/preview roles updater | `file_item_roles_updater.rs` / `thumbnail_pipeline.rs` | 缩略图/预览 role 调度从 `main.rs` 拆到 Dolphin `KFileItemModelRolesUpdater` 对应层；输入是 pane-local row token、visible range、maximum visible items 和 icon size，排序采用 Dolphin `indexesToResolve()`：可见文件、可见目录、向后预读、向前反向预读、末页、首页，再补全到上限 |
...
| Rust item-view render plan | `item_view_renderer.rs` / `split_view.rs` / `split_pane.slint` | 主视图行列/滚动 metrics、tile x/y/width/text width、media/text rect、尺寸/字体 token 和 selection/fallback/thumbnail/drop-target raster 输入不再由 Slint 每项公式或 hot row data 承载；可见底图由 Rust raster layer 提供，Slint 只叠加 text/metadata |
| Pane-local tile raster cache | `pane.rs` / `model_update.rs` / `item_view_renderer.rs` | row token、bounds、media token、selection 和 drop target 变化收敛为 pane-local raster revision；cache 只比较 raster input + revision，重复 view sync 不再重画同一张底图 |
| Thumbnail raster input | `thumbnail_pipeline.rs` / `item_view_renderer.rs` / `model_update.rs` | 基础 `ItemViewEntry` 不再携带 `Image`；已加载 thumbnail 先进入 renderer-owned Rust-only `ItemViewMediaSource`，再投影为 pane-local `ItemViewRasterMediaEntry` rows，并通过 renderer-owned media token sidecar 复用 pane-local raster input，zoom/cached relayout 时由 `model_update.rs` 裁剪和平移；Slint 不再查 `root.bounds[media.slice_index]`，也不再持有 media row 或 media loop |
| Sparse metadata overlay | `item_view.rs` / `item_view_renderer.rs` / `model_update.rs` / `models.slint` | 基础 `ItemViewEntry` 不再携带 group/location 字符串和 metadata 几何；show-location 先发布 Rust-only 非空 `ItemViewMetadataOverlaySource` rows，再由 renderer 投影为当前 text backend 使用的 Slint-facing `ItemViewMetadataEntry`，row 自带 item x/y 且不携带 `slice_index`，Slint metadata loop 不再查 bounds sidecar |
...
| Coalesced icon zoom thumbnail/preview roles | `app.slint` / `main.rs` / `file_item_roles_updater.rs` | `icon_zoom_level` 变化立即刷新可见 pane layout 和当前 raster；300ms 后按当前可见切片 latest-only 调用 roles updater 调度预览，贴近 Dolphin 的 `triggerIconSizeUpdate()` / `updateIconSize()` 分层 |
| Cached icon zoom relayout + layout reuse | `geometry.rs` / `virtual_view.rs` / `model_update.rs` / `pane.rs` / `main.rs` | zoom 后只要求当前 slice 覆盖新真实可见区，未按新 `rows_per_column` 对齐的窗口也通过 `virtual_start_column + virtual_start_row` 直接复用，避免首次 zoom 裁剪 `VecModel` rows。同步 cached relayout 覆盖已缓存/预热过的大目录；未缓存 layout 只有不超过 `ICON_ZOOM_SYNC_UNCACHED_RELAYOUT_ENTRY_LIMIT` 的小目录才在 UI 线程构造，普通/大目录不再同步刷新当前可见 slice 的 icon/text style 或 tile raster，而是直接走后台 full prepare，避免在 Ctrl+wheel/input 事件中构造完整 layout 或重画当前 slice 底图。cache miss 不在 UI 线程同步计算所有 zoom 档，而是复用 pane-local `Arc<ItemViewLayoutEngine>` 做可见区规划；后台 snapshot result 和 pane cache commit 继续传递同一个 `Arc`，`VirtualViewCache` 还保留最近几档 layout history，并在 virtual result / zoom fast path 后按距离顺序对全部 zoom 档做后台 layout prewarm。prewarm 逐档提交，最邻近 zoom 档算完就进入 history，不等待全部档位结束。zoom 回到已构造或预热过的 signature 时直接复用历史 `Arc`，避免重新扫描 entries/visible-index 并重建 offset/width 数组。cache-hit 签名检查先按 `Arc::ptr_eq` 命中同一个 layout，避免滚动/viewport cache-hit 时再扫整张 offset/width 数组。compact layout 内部的 item/column width 和 offset 数组也用 `Arc` slice 承载。文本宽度由 item width 派生，不再为每个 entry 维护第二张 text-width 数组。compact 文本字号保持 Dolphin-style 稳定，只让 icon/media size 参与 zoom |
| Cached filename width units | `pane.rs` / `geometry.rs` / `virtual_view.rs` / `main.rs` | `PaneEntrySnapshot` 缓存文件名估宽单位；virtual snapshot 和 cached zoom relayout 直接消费 `f32` 宽度单位，避免大目录 zoom/切换目录时重复 clone 文件名并逐字符估宽。后台 cache-miss layout 构造现在直接从 entries / visible-index iterator 流式喂给 compact layouter，不再先分配整目录 `visible_name_width_units` 中间 `Vec` |
...
- `PaneViewData` 承载 viewport、entry count、layout metrics、空状态、drop/content interactive、当前 tile raster base layer，以及 pane-local `paint` / `media` / `metadata` models；`ItemViewItemBounds` 留在 Rust 侧，selection/fallback/drop-target geometry 由 renderer batch 按当前 row token + bounds 派生并进入 raster layer。
...
**收益**：虚拟切片、viewport clamp、目录 view-state 恢复、selection 更新不再重建 pane chrome row；raster/paint/media/metadata 都是 pane-local view row 数据，bounds 留在 Rust 侧，selection/fallback/drop-target geometry 只作为 renderer batch/raster 输入存在，避免全局 slot 0/1 sidecar 重新引入“主 pane”概念。

**验证**：源码守卫测试确认 `PaneViewData` 不携带 `entries` 或 `bounds`，但携带 `paint`、`item_view_raster_layer`、`media`、`metadata`，`PaneSlotSurface` 直接绑定 `root.view.*`，并且 `ui/app.slint` / `split_view.rs` 不再包含 `pane_slot_0_*` / `pane_slot_1_*` sidecar 或 `sync_pane_entries_ui()`。
...
`ItemViewRowToken` 覆盖 name/path/is_dir/selection/thumbnail_state/media token 等轻量身份字段，避免为了比较复用而从 Slint `VecModel` 读取 row；它现在归属 `src/app/model_update.rs`，与 virtual row reuse / selection/media/paint sidecar 更新放在同一层，而不是放在 input/controller/hit-test 的 `item_view.rs`。split pane snapshot 会复制到独立 `VecModel` 和 sidecar，避免两个 pane 共享同一个可见 row model。选中态只保存在 token sidecar 中；`PaneView` 生成当前 virtual slice 的 raster 底图时，直接通过 `ItemViewTileFrameBatch` 从 row token + bounds 取得 selection/fallback/drop target 几何。selection 或 virtual row 变化时不再发布或维护任何独立高亮/fallback 模型。基础 `Text` loop 和 `ItemViewEntry` row data 不再携带 per-item selected 状态、已加载 thumbnail image、group/location metadata 或 per-row 几何；基础 title 绘制消费由 row token + Rust bounds 投影出的 pane-local `ItemViewPaintEntry` sidecar。folder/file fallback media 不再从 paint row 分支投影成独立 model，而是作为 tile frame plan 的 raster 输入画入同一张 raster 底图；Slint 不再使用两个固定 source `Image` loop，也不再对每个可见 row 绑定 `paint.is_dir ? folder : file`。已加载 thumbnail image 从缓存投影成 Rust-only `ItemViewMediaSource`，再投影为 pane-local `ItemViewRasterMediaEntry` raster input 并画入同一张底图；当前 drop target index 也作为 PaneView transient state 进入 raster input。show-location metadata 从 `FileEntry` snapshot 经 Rust render plan 投影成 Rust-only `ItemViewMetadataOverlaySource`，再发布为独立的 pane-local sparse `ItemViewMetadataEntry` model。可见项的 x/y/width/text-width 继续由 pane-local `ItemViewItemBounds` sidecar 在 Rust 侧维护并投影到 Slint-facing paint/metadata rows，同时供 tile raster batch 使用；Slint 基础 loop 不再重复计算 `virtual-start-row + index` 和 modulo row，也不再接收完整 bounds model。所有 remaining overlay 都不再查 `root.bounds[slice_index]`。Rust bounds sidecar 和 Slint paint sidecar 都复用自己的 pane-local `VecModel`，普通滚动和 cached zoom relayout 只做 row 级滑动/更新，不再每次替换整个 model；thumbnail sidecar 留在 Rust 内作为 raster input，selection/fallback/drop-target 变化则只 bump pane-local raster revision。media sidecar 使用 Rust-only `slice_index + media_token` 判断 thumbnail raster input 是否变更，复用 pane-local `Vec<ItemViewRasterMediaEntry>`，避免为了模型复用去比较或读取 `Image`；Slint-facing media row 已删除。metadata sidecar 是稀疏 overlay，一个 item 可能对应 0/1/2 行，因此不套用 item-range 滑动算法；非空 metadata 更新复用已有 pane-local `VecModel` 做 row 级比较/更新，空 metadata 仍回到空模型语义，且 Slint-facing metadata row 不再携带 `slice_index`。

**收益**：连续滚动时重叠 virtual rows 不再因为 slice-local 坐标、pane-level 几何或 fallback glyph 变化被重发；selection/fallback/真实 thumbnail 变化只刷新 Rust sidecar 和当前 raster 底图；Rust bounds sidecar、Slint paint 和非空 metadata model 保持稳定，且 Slint 少持有一份完整可见 bounds model，也不再在 fallback image loop 或 thumbnail raster loop 对每个 row 求分支/图像绑定，避免 Rust-projected 几何/图像/文本 overlay 下沉后又通过整模型替换刺激 Slint 可见 primitive 绑定。

**验证**：`app::model_update` 测试覆盖前滑、后滑、无重叠、sidecar 修复、media token 比较、selection sidecar 更新、bounds/paint sidecar 滑动复用、media sidecar token 驱动复用、metadata sidecar 稀疏 row 级复用/清空、cached relayout 复用 bounds/paint model 并裁剪 Rust-only raster media input；源码守卫测试防止重叠 row reuse 重新读取 `VecModel::row_data()`，防止 `ItemViewEntry.selected` / `ItemViewEntry.media` 回归，并确认 Slint-facing `PaneViewData` 不再携带 `bounds`。
...
**Dolphin 对照**：Dolphin 的 `KItemListView::setStyleOption()` 会立即更新可见 widget 的 style、size hint cache 和 layouter；`KItemListViewLayouter::updateVisibleIndexes()` 在 row offsets 上用二分找 first/last visible index；`KFileItemListView::triggerIconSizeUpdate()` 只对 icon-size 引发的昂贵 model role / preview 更新使用 `LongInterval` timer 合并。Compact layout 的 item hints 使用 `styleOption.fontMetrics`，icon zoom 改 `iconSize`，文本字体本身不随每档 icon zoom 改变。Fika 采用同样的分层：滚动/resize/zoom layout 和当前可见 fallback/icon/text 绘制不延迟；只有缩略图/预览 role 调度用 300ms latest-only timer 合并；layout offset 数组在 pane-local cache 中常驻，scroll/cache miss 只更新可见 slice，内容或过滤变化才让 layout 变脏。

**涉及代码**：
- `ui/app.slint` — `changed icon_zoom_level => icon_zoom_layout_changed()`
- `src/main.rs` — `PaneLayoutSyncScheduler`
- `src/app/virtual_view.rs` — `cached_snapshot_layout()`
- `src/app/pane.rs` — `VirtualViewCache`

**实际实现**（✅ 已完成）：`icon_zoom_level` 不再调用普通 `pane_layout_changed()`：
- UI 线程不再为目录/滚动重建同步计算所有 zoom 档的可见 range；`VirtualViewSnapshotInput.range_hint` 在当前 UI 路径传 `None`，避免请求创建阶段扫描整目录并构造多份 compact layout。zoom layout prewarm 只在 virtual result / zoom fast path 之后通过 `tokio::spawn_blocking` 后台执行，并按距离从近到远覆盖全部 zoom 档；每个 pane 对当前 generation 同时只允许一个 prewarm in flight，结果返回后再按 pane-local generation 校验，不触碰 Slint model。prewarm 现在逐档提交：相邻 `+1/-1` zoom 档算完立刻写入 `layout_history`，最后一档才清掉 in-flight 标记。
- `VirtualViewCache.layout` 持有 `Arc<ItemViewLayoutEngine>`，并保留最近几档 previous layout history；构造后台 snapshot input、pane surface metrics、hit-test 和框选时只 clone `Arc`，不复制 column widths/offsets 等 layout 数组。`CompactItemViewLayout` 内部的 `item_widths`、`column_widths` 和 `column_offsets` 也使用 `Arc<[f32]>`，layout clone 不会复制整目录数组；item text width 从 item width 和固定 reserved 宽度派生，不再维护同长度的 `item_text_widths`。`prepare_virtual_view_snapshot_update()` 在 layout signature 和 entry count 匹配时通过 `cached_snapshot_layout()` 复用当前或历史 compact layout，只按新的 viewport 计算 visible range；thumbnail size 只决定当前可见 raster/preview 是否需要 rebuild，不再阻止 layout reuse。zoom layout prewarm 复用同一纯 layout 构造函数，结果只写入 `layout_history`，不发布 pane view。`VirtualViewSnapshotUpdate.layout` 和 pane cache 写回也继续共享同一个 `Arc`。`VirtualViewCache::matches_layout_arc()` 在 thumbnail size 匹配后先用 `Arc::ptr_eq` 判断是否为同一个 layout，只有非同一 Arc 时才回退完整 signature 比较，避免滚动/viewport cache-hit 时扫描整目录 `item_widths` / `column_offsets` 数组。
- 内容变化走 `PaneState::set_entries_with_location_state()` / `apply_filter_for_slot()` 的 `clear_virtual_view()`，清掉旧 layout；滚动补片和 chrome-only layout 变化仍可只失效 range 或通过 signature mismatch 触发重建。这对应 Dolphin 的 dirty layouter/visible-index 分层，避免 entry_count 相同但文件名不同的旧 layout 被复用。
- `icon_zoom_layout_changed()` 立即调用 `sync_visible_pane_icon_zoom_layouts()`；`try_relayout_cached_pane_icon_zoom_layout()` 优先复用当前或 history 中的 layout，并且只要求当前 virtual slice 覆盖新 zoom 的真实可见 range。若目标 layout 未缓存/未预热，只有不超过 `ICON_ZOOM_SYNC_UNCACHED_RELAYOUT_ENTRY_LIMIT` 的小目录会在 UI 线程同步构造，普通/大目录直接进入后台 prepare，避免在 zoom 输入事件里构造过大的整目录 `visible_name_width_units`、完整 compact layout。cache-hit relayout 成功时立即 `sync_pane_view_ui()`，当前可见 fallback/icon/text/raster 跟随 Dolphin visible widget style 更新；`cached_zoom_relayout_range()` 不再要求旧 slice 起点按新 `rows_per_column` 对齐，也不再裁剪到对齐列边界；Slint 侧用派生的 `virtual_start_column + virtual_start_row` 绘制任意起点的窗口，保留 selection/highlight sidecar，不重新 snapshot 当前目录。
- fast path 覆盖不了新真实可见 range，或 show-location metadata 仍在热路径里时，不再在 Ctrl+wheel/input 事件里同步 snapshot rebuild，也不再为了“临时 style”同步调用 `sync_pane_view_ui()` 生成一张新 raster。`prepare_pane_icon_zoom_layout_for_slot()` 发起后台 virtual prepare，并在结果返回后原子提交准确的全目录 layout 和当前可见 raster。这比旧的 visible-style refresh 更贴近 Dolphin 的代价分层：输入路径更新目标状态和 layouter/cache，昂贵 icon/preview/raster 工作不压在同一个回调里。
...
- Folder/file fallback glyph 已画入当前 virtual slice 的 tile raster base layer；目录/滚动 snapshot、theme refresh 和 zoom fast path 都同步重新生成当前可见 raster，符合 Dolphin visible item/widget 立即重绘。新的 folder/file fallback `Image` source 不再下发给 Slint，贴近 Dolphin 的可见区 pixmap/cache 思路并降低 `/etc` 这类 fallback-heavy 目录的首次 zoom 卡顿。
- `PaneLayoutSyncScheduler` 使用 `TimerMode::SingleShot` 以 300ms 合并连续 zoom 后的缩略图/预览 role 调度；timer flush 时从当前可见 virtual slice 调用 `schedule_thumbnail_roles_for_visible_panes()`。该路径已从 `main.rs` 拆到 `src/app/file_item_roles_updater.rs`，对应 Dolphin `KFileItemModelRolesUpdater`：输入是 pane-local `ItemViewRowToken` sidecar、visible range、maximum visible items 和 icon size；排序采用 Dolphin `indexesToResolve()` 语义，先可见文件、再可见目录，再向后预读、向前反向预读、末页、首页，并补全到 `RESOLVE_ALL_ITEMS_LIMIT`。`src/app/thumbnail_pipeline.rs` 只保留候选判断、cache/failure/pending key 和 capped batch 生成；roles updater 不从 Slint `VecModel<ItemViewEntry>` 读取 row，只有真正入队的缩略图任务才转成 owned `PathBuf`。
...
**收益**：连续 zoom 时避免每个 zoom step 都启动缩略图解码/预览更新；当前 slice 覆盖新行数且 layout 已缓存时复用当前 visible model，只更新 pane-level layout/text/paint/raster metrics，不裁剪 `VecModel`；未缓存普通/大目录不在 UI 线程构造完整 compact layout；普通目录在初次 virtual result 后后台预热全部 zoom layout，并且相邻档位先提交，后续 zoom 或回退 zoom 可更早命中 history，减少首次交互输入事件里的列宽/offset 构造；`/etc` 这类非图片目录的首次 zoom 不再因为缩略图候选探测或未缓存 layout 构造而冷启动重活；稳定字体减少新 zoom 档位的 Text layout 冷启动；预计算文件名宽度单位和流式 layouter 输入减少大目录 zoom/目录切换时的字符串 clone、逐字符估宽和中间宽度数组分配；同时保留 resize/fullscreen/末尾 viewport 修复的即时路径。

**验证**：源码守卫测试确认 `PaneViewSyncScheduler` 的滚动路径仍无 timer，`icon_zoom_layout_changed` 同步刷新 layout，优先尝试 cached relayout，未缓存普通/大目录 cache miss 进入后台 prepare，cached relayout 立即 `sync_pane_view_ui()` 刷新当前可见层，只有缩略图/预览 role 调度走 300ms coalesced timer，并且 zoom input path 不再调用旧的多档 range 同步计算或 cache-miss visible-style 刷新。`file_item_roles_updater` 测试覆盖 Dolphin `indexesToResolve()` 顺序、virtual slice 内的全局顺序保持、zoom level 到 preview size 映射；`virtual_view` 测试覆盖 `range_hint` 的后台扩展能力、thumbnail-size 变化时的 layout reuse、layout history reuse，以及 layout prewarm 复用/构造；`main` 测试覆盖 cached zoom relayout 只要求真实可见 range、已对齐和未对齐热窗口都不裁剪，并守卫 zoom layout prewarm 逐档调用 `prepare_virtual_view_layout_prewarm_input()`、逐档提交、最后一档才 finish；`pane` 测试覆盖 fallback media 多槽复用、split snapshot 继承热 cache、recent previous layout history、current layout 不重复写入 history、layout prewarm generation-scoped finish 和内容变化清空旧 virtual layout；`model_update` 测试覆盖 cached relayout 复用 `VecModel`、保留 selection/highlight 并裁剪 Rust-only raster media input；`thumbnail_pipeline` 测试覆盖旧 size thumbnail 不阻塞新 size 调度、pending/candidate/cache/failure batch，以及非候选 row/token 在 zoom 后直接跳过；`fs::thumbnails` 测试覆盖未知扩展不进入 thumbnailer lookup。`cargo test app::file_item_roles_updater` / `app::virtual_view` / `app::geometry` / `app::pane` 覆盖 roles updater、预计算宽度单位接入后的 virtual layout、Dolphin compact geometry 和 pane snapshot/raster cache 路径。
...
**涉及代码**：
- `src/app/file_item_roles_updater.rs` — `schedule_thumbnail_roles_for_entries` / `schedule_thumbnail_roles_for_visible_panes`
- `src/main.rs` — `ThumbnailFlushScheduler` 及 async event flush
...
**验证**：`thumbnail_results_are_batched_before_virtual_refresh` 守卫 flush 路径；`file_item_roles_updater` 测试覆盖 Dolphin role 调度顺序；thumbnail pipeline 测试覆盖 pending key、成功/失败 cache 和 capped batch。
...
**实际实现**（✅ 已完成）：采用快照模式而非锁迁移——`PaneEntrySnapshot`（不含 `Image` 的轻量结构体，`Arc<[PaneEntrySnapshot]>` 零拷贝共享）+ `VirtualViewSnapshotInput`（完全 owned 的纯函数输入）。UI 线程按 pane slot 构建 snapshot 输入后通过 `tokio::spawn_blocking` 在后台执行 `prepare_virtual_view_snapshot_update`（纯函数），结果通过 `AsyncEvent::VirtualViewPrepared` 回传。搜索/过滤产生的 pane-local `visible_entry_indices` 和 `visible_location_groups` 也以 `Arc<[usize]>` / `Arc<[String]>` 保存并传入 snapshot input，滚动或 cache-miss 请求只 clone 引用，不复制整个过滤结果缓存。`virtual_generation` 独立于 `load_generation` 做 staleness 检测，async virtual refresh 的 in-flight/pending 请求现在由 pane-local `VirtualViewRefreshState` 持有，`apply_virtual_view_result` 先在 `borrow_mut` 块内写 state 再 drop 后写 Slint model，无 RefCell 跨线程风险。所有可见 pane 都走同一条 slot 驱动虚拟视图管线，旧的 preview/副 pane 专用路径已删除。全局过滤/搜索/监控刷新仍未统一成完整 pending→commit 状态机。

---

### P3 — 可见 tile primitive 简化

**问题**：主视图每个可见 tile 的 selection、fallback icon、loaded thumbnail 和 drop target 已进入 Rust raster base layer，但标题和 show-location metadata 仍由 Slint `Text` primitive 循环绘制。当有 80-120 个可见 tile 时，剩余文本 primitive 仍有属性绑定和文本布局成本。
...
5. 虚拟切片仍输出 Rust-side `virtual_entries`，但它只留在 `PaneView` 中服务 controller、hit-test、DnD 和 token sidecar；主视图热字段已通过 `PaneViewData` 接收 Rust item-view layouter metrics（`rows_per_column`、cell size、padding、content width、virtual slice width、scroll max）、media/text/title 几何、当前 slice 的 raster base layer、viewport 和空状态。Slint-facing `PaneViewData` 只下发 raster、paint、media、metadata pane-local 绘制数据，避免 nested model in row，也不再下发完整 bounds model；`PaneSlotData` 只保留 pane chrome/status/search/chooser 冷数据。普通 item 使用 Dolphin-style compact 横向布局，图标在左、名字在右，基础 name loop 直接消费 Rust-projected `ItemViewPaintEntry` paint rows，selection/fallback glyph 已在 raster 底图里；带 group/location 的递归搜索结果通过 sparse metadata overlay 叠加 group/location 文本。item `x/y/width/text_width` 由 Rust-side `ItemViewItemBounds` sidecar 维护并投影到 Slint-facing rows，不写入 `ItemViewEntry` hot row；Slint 不再在主视图内计算 content width、scroll extent、zoom 派生公式、title metadata 分支或每个 loop 的 row modulo。
...
8. 文件/目录 fallback media 和已加载 thumbnail image 已从 Slint `Image` overlay 迁到 `item_view_renderer.rs` 并继续下沉到 tile raster base layer：folder/file fallback glyph 先画进当前 virtual slice 的 `PaneViewData.item_view_raster_layer`，已加载 thumbnail image 再通过 Rust-only `ItemViewRasterMediaEntry` 覆盖 fallback glyph。主视图不再下发 `item_view_folder_media` / `item_view_file_media`，也不再有 folder/file fallback 或 thumbnail raster `Image` loop。theme refresh 和 zoom fast path 都通过 `sync_pane_view_ui()` 重新生成当前可见 raster，保持 Dolphin visible widgets/style option 立即重绘语义；只有 thumbnail/preview role 调度走 300ms coalesced icon-size timer，为后续 renderer state 继续收敛到 Rust 侧铺路。
9. `ItemViewEntry.media_token` 作为真实 thumbnail 更新令牌进入可见 row；`src/app/model_update.rs` 同时维护 pane-local `ItemViewRowToken` sidecar、`ItemViewItemBounds` bounds sidecar 和 `ItemViewPaintEntry` paint sidecar，并消费 renderer-owned `ItemViewMediaSource` / `ItemViewMediaToken` / `ItemViewRasterMediaEntry` raster input types。虚拟切片滑动的重叠 row 先比较 sidecar token，token 相同就不读取 `VecModel::row_data()`；bounds/paint sidecar 复用各自 pane-local `VecModel` 做滑动更新，selection/fallback/drop-target geometry 由 renderer batch 从 row token + bounds 按需生成，raster media input 复用同一个 pane-local `Vec` 并只用 Rust-only `slice_index + media_token` 判断 thumbnail row 更新，cached relayout 会同步裁剪 bounds、paint 和 Rust-only raster inputs。Slint-facing media row 已删除；raster layer 在 pane view sync 时按当前 pane 的可见 sidecar 重新生成。split pane 快照也会复制到独立 `VecModel`、cloned row-token sidecar、cloned paint sidecar、cloned raster-media sidecar 和 cloned media token sidecar，避免两个 pane 共享同一个可见 row 模型。
10. show-location metadata overlay 已从 per-item 透明 `Rectangle` wrapper 和 `visible:` 过滤，收敛成 pane-local `ItemViewMetadataEntry` 稀疏模型；普通空 metadata row 不再进入 Slint overlay loop，非空 metadata overlay 先通过 Rust-only `ItemViewMetadataOverlaySource` 保留 slice identity，再由 `item_view_renderer.rs` 完成 bounds projection 并发布为不带 `slice_index` 的 Slint-facing row。当前 text backend 仍是 Slint 原生 `Text`，但 title/metadata 的输入边界已经归入 renderer，后续可替换 backend 不需要改 `model_update.rs` 的模型复用逻辑。空 metadata 仍清回空模型，split snapshot 也会复制独立 metadata model，保持 pane 独立。
11. Dolphin compact visual metrics 已收敛到 `src/app/item_view_metrics.rs`：`geometry.rs` 负责 viewport/visible range/layout，并新增 `ItemViewLayouter` trait 暴露 Slint metrics projection、virtual plan、range anchor、item rect、virtual slice geometry、range-hint 对齐、layout cache signature、hit-test、矩形选择候选范围、layout mode、scroll axis 和 logical→physical item projection；`CompactItemViewLayout` 是第一版 compact 实现，`ItemViewLayoutEngine::Compact` 已成为 runtime view/cache state 的 enum dispatch 边界。`item_view_model.rs` 现在提供 `ItemViewModelEntry` 与统一 query/kind/modified/size/chooser filter predicates，virtual snapshot、selection/search 和 layout width 输入共用同一模型入口，不再分别复制 `PaneEntrySnapshot` 字段过滤逻辑。`item_view.rs` 负责 input/controller/hit-test，`item_view_renderer.rs` 负责 render plan、metadata projection、fallback media、`ItemViewFrameEntry`、`ItemViewTileFrameSource`、`ItemViewTileFramePlan` 和 `ItemViewTileFrameBatch`。统一 Rust-side tile frame batch 现在派生 paint rows、selection/fallback/drop-target raster geometry 和 media-token lookup；`model_update.rs` 只消费 batch API 来复用/发布 pane-local models，不再拆 `plans()` 分别重组 paint/highlight/fallback 状态。frame source 可由 `ItemViewEntry` 或 cached `ItemViewRowToken` 通过同一个 renderer/model input trait 提供；bounds-backed frame plan 只承载可绘制 text/fallback/highlight primitives，token-only thumbnail 更新通过 batch API 取 media token，batch 把二者作为一个 renderer/cache 输入边界保存。live entries / cached row tokens + bounds + selection 到 tile frame batch 的构建规则，以及 source-index 到 media-token 的查找规则也已下沉到 renderer，避免把无 bounds 的 media refresh 误塞进绘制列表，且不再由 `model_update.rs` 分别重组 entry/bounds/token 状态。`ItemViewTileFrameBatch` 现在还能从同一批 frame plan 生成 `SharedPixelBuffer`/`Image` raster layer，绘制选中背景、folder/file fallback glyph、loaded thumbnail 和 drop target；`PaneView` 对当前 tile raster 保存 pane-local cache，row token、bounds、media token、selection 和 drop target 更新只 bump `raster_revision`，命中检查只比较 raster input + revision，重复 view sync 不再重画未变化底图。它们不再为每个 Slint primitive model 分别维护 zoom/media/font/line/cell/row 公式；fallback 源图、zoom 几何、model filter 和 text projection 已经解耦，为后续可选 text backend、真正的 Details/Icons polymorphic layouter、完整 model trait 或 tile-frame 自绘提供更稳定的输入边界。
12. `ItemViewInputState` 已开始输出 `ItemViewControllerAction`：item press 通过 `press_entry_at_pane_point()` 在 `item_view.rs` 内完成坐标 hit-test、记录 pane-local drag source 并返回选择路径 action，activation 通过 `activate_entry_at_pane_point()` 返回 `ActivatePath` action，item context menu 通过 `context_menu_entry_at_pane_point()` 返回 `RequestContextMenu` action 并携带必要的选择意图和 controller-facing `ItemViewHitEntry`，blank press/move/release/cancel 也通过 slot-level controller helpers 更新 pane-local gesture state，blank release 再返回清空选择或矩形选择 action；`main.rs` 只执行 action，不再直接写 input state、复制 item press/activation/context-menu hit-test、匹配 blank gesture 内部细节，或在 controller action 边界继续接收完整 `FileEntry`。这仍不是 Dolphin 式完整 controller signal bus，但已经把 press/release/activation/context-menu 的手势状态和 UI 执行动作分开，并开始收窄 controller/model payload。
13. DnD 仍保留 Slint 原生 `data-transfer` 路径，drag payload 和 drop target 解析都继续向 Rust hit-test 收敛。
14. Dolphin `KItemListSmoothScroller` 的分层已经映射到 Slint 主视图：logical `viewport-x` 继续立即驱动 scrollbar、Rust visible slice、hit-test、DnD 和 selection 坐标；新增 `paint-viewport-x` 只作为绘制 primitive 的 offset，并在普通滚轮滚动时用 120ms ease-out 平滑追随。`PaneViewData` 下发当前 virtual slice 的 content 起点/宽度，`SplitPaneView` 只有在旧 paint window 与目标 viewport window 都被当前 slice 覆盖时才动画；否则立即同步 paint offset，避免虚拟模型已切到新 range 时动画途中露出空白。scrollbar 点击/拖动、pane-local relayout、`scroll-max-x` 变化、pointer press、slice 起点/宽度变化和外部 viewport 写入都会调用 `stop-smooth-scroll()`，保证手动拖动、目录恢复和 fullscreen/resize clamp 不落后于动画。
...
**实际实现**（✅ 已完成）：`ItemViewEntry` 不再带 `selected` 字段。Rust 侧把当前 pane 的 selection 只写入 pane-local `ItemViewRowToken` sidecar，由 `PaneView` 通过 `ItemViewTileFrameBatch` 直接画进当前 virtual slice 的 `item_view_raster_layer`。Slint 不再有 `root.highlights` loop，Rust 侧也不再维护单独的高亮模型。

选择变化时只更新 token sidecar、bump pane-local raster revision，并重新生成当前 raster 底图，不再对 `VecModel<ItemViewEntry>` 做 row-data 写回，也不再由 `split_view` 每次 UI 同步临时构造 Slint 高亮模型。旧的 `selection_revision` 全局属性和 `PaneViewData.selection_revision` 热字段已删除，选择变化不再为了触发 tile 绑定而污染 pane view row；后台虚拟视图结果应用时会用当前 pane selection 初始化 token sidecar，旧异步结果不会把高亮写进基础 row。渲染路径上的 `PaneRouting.is-selected` / `FilePane.is_selected` 回调已删除；item 右键命中后是否需要先选中由 Rust 坐标 helper 按 pane selection 状态直接判断。

**收益**：每次选择变化（点击、框选、Ctrl+A）省 80-120 次 FFI 调用，并避免 selection 变化重写可见 `ItemViewEntry` rows。

**难度**：已完成。源码守卫测试覆盖 raster highlight 路径，并防止恢复 tile 级 `is_selected` 回调、`selection-revision`、`selected: item.selected` 或 `ItemViewEntry.selected`。
...
该直接索引路径不只用于 `filtered_entries_range`，也用于可见路径收集、单项读取、可见 range 迭代、虚拟视图后台 snapshot，以及缩略图可见范围判断。`PaneState::set_entries` / `clear_entries` 会清空旧的 `visible_entry_indices` 和 `visible_location_groups`，目录加载路径使用 `set_entries_with_location_state(entries, has_locations)` 复用已有 location 统计，避免为了失效缓存额外扫描大目录。`visible_entry_indices` / `visible_location_groups` 现在作为 `Arc` slice 保存在 pane 上并传给 `VirtualViewSnapshotInput`，搜索后滚动不会在每次 snapshot request 构造时复制整张可见索引表或 group 表。
...
**实际实现**（✅ 已完成）：不维护易过期的局部 `(start_visible_index, previous_location, annotations)` 滚动缓存，而是在过滤/search 重建可见索引时一次性生成 pane-local `visible_location_groups`。`VirtualViewSnapshotInput` 将该缓存作为 `Arc<[String]>` 随条目快照传入后台 `prepare_virtual_view_snapshot_update`，虚拟切片标注直接按 `start_visible_index + offset` 读取预计算 group；只有缺少缓存时才回退到按 slice 边界推断。
...
**历史问题**：旧 `schedule_visible_thumbnails` 为每个路径创建独立的异步任务——N 个缩略图 = N 个 `bridge.handle.spawn` + N 个 `tokio::spawn_blocking`。Phase 4 的 `ThumbnailFlushScheduler` 已批量化 UI 线程的结果写入，但 spawn 风暴仍消耗 tokio 调度开销。

**涉及代码**：
- `src/app/file_item_roles_updater.rs` — `spawn_thumbnail_preview_job`
- `src/main.rs` — `ThumbnailFlushScheduler`
...
**实际实现**（✅ 已完成）：thumbnail/preview role 调度已迁入 `file_item_roles_updater.rs`，`spawn_thumbnail_preview_job` 只创建一个 async task，并在其中用一个 `tokio::spawn_blocking` 顺序处理整批 `paths`。加载成功后仍逐个发送 `AsyncEvent::ThumbnailLoaded`，沿用现有 `ThumbnailFlushScheduler` 的 UI 线程批量写入；如果 blocking task 失败，会按原始 path 逐个生成失败 `ThumbnailLoad`，确保 pending 状态能被清理。
...
`viewport-content-width`、`virtual-slice-start-x`、`virtual-slice-width` 和 `scroll-max-x` 现在由 Rust `pane_slot_item_view_metrics()` 通过同一套 `MainItemViewLayout` / `compact_item_view_layout()` 计算，并随 `PaneViewData` 下发给 `SplitPaneView`。其中 Slint-facing metrics 由 `CompactItemViewLayout::layout_metrics()` 投影，virtual range anchor 由 `range_anchor()` 提供，virtual slice 起点/宽度由 `virtual_slice_geometry()` 统一提供；`split_view.rs` 不再直接读取 column offsets 或用 rows-per-column 推导 anchor。Slint 只消费这些 metrics 做 scrollbar、viewport clamp、smooth paint viewport 覆盖判断和切片偏移，不再自己根据 `entry-count` / `rows-per-column` / `zoom-level` 重算主视图 layouter。
...
- **Phase 5**: `PaneEntrySnapshot`（不含 `Image` 的轻量快照, `Arc` 零拷贝共享，并缓存 `name_width_units`）+ `VirtualViewSnapshotInput`（完全 owned 的纯函数输入）— 虚拟视图的条目过滤/切片/clone/location 标注全部在 `tokio::spawn_blocking` 中完成，UI 线程只做 generation staleness 检查 + Slint 模型写入 + 缩略图缓存装饰。Dolphin compact size-hint 所需的文件名估宽在 entry 进入 pane 时完成，virtual layout/cached zoom relayout 只按当前 font size 缩放宽度单位，后台 cache-miss layout 直接从 visible source 流式构造，不再额外分配整目录宽度中间表；cached layout、recent layout history、zoom 后台 prewarm 逐档结果、snapshot result 和 pane cache commit 都通过 `Arc<ItemViewLayoutEngine>` 共享 compact layout，thumbnail size 不再阻止 layout reuse，cache-hit 签名检查优先使用 `Arc::ptr_eq`，compact 内部宽度/offset 数组同样用 `Arc<[f32]>`，搜索/过滤可见索引和 location group 缓存也通过 `Arc` slice 共享到 snapshot input。thumbnail/preview role 调度已拆到 `file_item_roles_updater.rs`，按 Dolphin `KFileItemModelRolesUpdater::indexesToResolve()` 顺序消费 pane-local row token，`thumbnail_pipeline.rs` 只保留候选、cache/failure/pending 和 capped batch。filter/search filter 刷新不再先清空 active virtual view，而是只失效 active cache range、保留旧 visible rows/layout，使用 forced-uncached prepare 输入构造新 filtered layout，等 async virtual result 返回后再提交，避免 pending 期间出现空白或旧 visible range 误命中。`virtual_generation` 独立于 `load_generation`，目录切换时自动推进；virtual refresh in-flight/pending bookkeeping 已收敛进 `VirtualViewRefreshState`，layout prewarm 是 pane-local current-generation single-flight、相邻 zoom 先提交且不参与 Slint model commit，为后续完整 two-phase pending→commit view refresh 留出 pane-local 状态边界。`apply_virtual_view_result` 先在 `borrow_mut` 内写 state 再 drop 后写 Slint，避免 RefCell 跨线程风险。所有可见 pane 走同一条 slot-aware 虚拟视图管线。
...
| 阶段 | 改进 | 预计工作量 | 状态 |
|------|------|-----------|------|
| **Phase V0** | `is_selected` FFI 移到 token sidecar / raster highlight | 1h | ✅ 已完成 |
