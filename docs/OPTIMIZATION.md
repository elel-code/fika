...
11. Dolphin compact visual metrics 已收敛到 `src/app/item_view_metrics.rs`：`geometry.rs` 负责 viewport/visible range/layout，并新增 `ItemViewLayouter` trait 暴露 Slint metrics projection、virtual plan、range anchor、item rect、virtual slice geometry、range-hint 对齐、layout cache signature、hit-test 和矩形选择候选范围；`CompactItemViewLayout` 是第一版 compact 实现。`item_view.rs` 负责 input/controller/hit-test，`item_view_renderer.rs` 负责 render plan、metadata projection 和 fallback media。它们不再分别维护 zoom/media/font/line/cell/row 公式；fallback 源图和 zoom 几何已经解耦，为后续 renderer cache、polymorphic layouter 或 tile-frame 自绘提供更稳定的输入边界。
