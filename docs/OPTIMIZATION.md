...
主文件视图使用 Dolphin compact 模式语义：物理滚动轴固定为 X，条目按 `index % rows_per_column` 先填满一列，再按 `index / rows_per_column` 进入下一列。compact item 尺寸沿用 Dolphin 的公式：`itemWidth = padding * 4 + iconSize + fontHeight * 5`，`itemHeight = padding * 2 + max(iconSize, textLines * lineSpacing)`，列间 margin 为 `8px`。普通目录使用 1 行标题，Trash 和递归搜索使用 pane-local 的 3 行 group/title/location 布局；每个 pane 独立计算自身行数、可见范围和 viewport。Dolphin 对照点是 `KStandardItemListView::setItemLayout()` 在 CompactLayout 下设置 `Qt::Horizontal`，`KItemListViewLayouter` 通过转置逻辑把纵向流映射成物理横向滚动，`KStandardItemListWidget::updateCompactLayoutTextCache()` 预先计算 compact 文本位置和最大宽度，并把 compact text frame 关联到整个 item 高度；文字本身使用居中的 line position。Fika 同样在 Rust render plan 中投影非零 `title_y/title_line_height/text_width`；普通标题的 Rust-projected `Text` frame 覆盖整 tile 高度并由 Slint vertical alignment 居中，避免大 zoom 下单行 frame 被裁剪导致 name 消失；带 group/location 的标题行继续使用 Rust 投影的 line frame，额外 group/location 行通过 pane-local sparse metadata overlay 绘制。`PaneEntrySnapshot` 在目录条目进入 pane 时缓存 `name_width_units`，后续 virtual layout 直接按当前字体尺寸缩放这些宽度单位，不再在 zoom/切目录热路径反复 clone 文件名或逐字符估宽。基础 `ItemViewEntry` 热 row 只留在 Rust `PaneView` 内部，携带业务身份、目录标记、缩略图状态和 media token，供 controller、hit-test、DnD 和 thumbnail token sidecar 使用；它不再作为 `PaneViewData` 字段传给 Slint。Slint 基础 name/icon loop 消费 pane-local `ItemViewPaintEntry`，基础 file/folder 图标通过 `media_kind` 选择 PaneView 缓存的当前尺寸 image；selection background、已加载 thumbnail image 和当前 drop target 由 Rust 生成当前 virtual slice 的 `item_view_raster_layer` 底图。media/metadata 的 `slice_index` 只保留在 Rust-only `ItemViewMediaSource` / `ItemViewMetadataOverlaySource` 和 media token sidecar 中，已加载 thumbnail image 只作为 Rust-only raster 输入，不再发布 Slint-facing media row。drop target 通过 pane-local transient `drop_target_slice_index` 进入 raster input，因此完整 `ItemViewItemBounds` 模型不再进入 `PaneViewData`。text/title 几何挂在 pane-level `PaneViewData` 上；selection/drop-target geometry 由 tile frame batch 按当前 row token + bounds 临时生成，thumbnail media 作为 Rust-only raster input 注入，不再进入 Slint-facing model。metadata 由 renderer 的 text projection 边界投影成 `ItemViewMetadataEntry` 稀疏 rows。当前 title/metadata backend 仍是 Slint 原生 `Text`，文字 raster 只作为后续可选实验保留，必须以实测快于原生 `Text` 为前提。`SplitPaneView` 只消费 raster 底图、缓存基础图标、pane-level 几何和 paint/metadata overlay sidecar，不再在可见 title loop 内按 `group/location` 或 `is_dir` 做 per-item 判断。
...
| Cached filename width units | `pane.rs` / `geometry.rs` / `virtual_view.rs` / `main.rs` | `PaneEntrySnapshot` 缓存文件名估宽单位；virtual snapshot 直接消费 `f32` 宽度单位，避免大目录 zoom/切换目录时重复 clone 文件名并逐字符估宽。cache-miss layout 构造直接从 entries / visible-index iterator 流式喂给 compact layouter，不再先分配整目录 `visible_name_width_units` 中间 `Vec` |
...
**验证**：`app::model_update` 测试覆盖前滑、后滑、无重叠、sidecar 修复、media token 比较、selection sidecar 更新、bounds/paint sidecar 滑动复用、media sidecar token 驱动复用、metadata sidecar 稀疏 row 级复用/清空；源码守卫测试防止重叠 row reuse 重新读取 `VecModel::row_data()`，防止 `ItemViewEntry.selected` / `ItemViewEntry.media` 回归，并确认 Slint-facing `PaneViewData` 不再携带 `bounds`。
...
- `PaneEntrySnapshot` 在目录加载/缓存写入阶段预计算文件名宽度单位；snapshot cache miss 直接把 entries / visible-index iterator 喂给 compact layouter，不再额外 materialize 整目录 `visible_name_width_units` 中间 `Vec`。
- Folder/file fallback glyph 已画入当前 virtual slice 的 tile raster base layer；zoom 同步重新生成当前可见 raster，符合 Dolphin visible item/widget 立即重绘。
...
8. 文件/目录基础图标已从慢的 tile raster 更新路径拆出：`PaneView` 按当前 media 宽高和 dark 签名缓存 file/folder 两张 fallback icon image，Slint 的 paint loop 只用 `ItemViewPaintEntry.media_kind` 贴同一组缓存图，zoom 时跟随 `media-x/y/width/height` 与文本同帧更新。已加载 thumbnail image 仍通过 Rust-only `ItemViewRasterMediaEntry` 进入 `PaneViewData.item_view_raster_layer`，并继续受 Dolphin-style 300ms coalesced icon-size timer 控制。主视图不再下发 `item_view_folder_media` / `item_view_file_media`，也不再为 fallback 图标重建整层 raster；只有 thumbnail/preview role 调度走延迟路径。
9. `ItemViewEntry.media_token` 作为真实 thumbnail 更新令牌进入可见 row；`src/app/model_update.rs` 同时维护 pane-local `ItemViewRowToken` sidecar、`ItemViewItemBounds` bounds sidecar 和 `ItemViewPaintEntry` paint sidecar，并消费 renderer-owned `ItemViewMediaSource` / `ItemViewMediaToken` / `ItemViewRasterMediaEntry` raster input types。虚拟切片滑动的重叠 row 先比较 sidecar token，token 相同就不读取 `VecModel::row_data()`；bounds/paint sidecar 复用各自 pane-local `VecModel` 做滑动更新，selection/fallback/drop-target geometry 由 renderer batch 从 row token + bounds 按需生成，raster media input 复用同一个 pane-local `Vec` 并只用 Rust-only `slice_index + media_token` 判断 thumbnail row 更新。Slint-facing media row 已删除；raster layer 在 pane view sync 时按当前 pane 的可见 sidecar 重新生成。split pane 快照也会复制到独立 `VecModel`、cloned row-token sidecar、cloned paint sidecar、cloned raster-media sidecar 和 cloned media token sidecar，避免两个 pane 共享同一个可见 row 模型。
...
11. Dolphin compact visual metrics 已收敛到 `src/app/item_view_metrics.rs`：`geometry.rs` 负责 viewport/visible range/layout，并新增 `ItemViewLayouter` trait 暴露 Slint metrics projection、virtual plan、range anchor、item rect、virtual slice geometry、range-hint 对齐、layout cache signature、hit-test、矩形选择候选范围、layout mode、scroll axis 和 logical→physical item projection；`CompactItemViewLayout` 是第一版 compact 实现，`ItemViewLayoutEngine::Compact` 已成为 runtime view/cache state 的 enum dispatch 边界。`item_view_model.rs` 现在提供完整 `ItemViewModelEntry` 字段接口、统一 query/kind/modified/size/chooser filter predicates、`ItemViewModelEntrySummary` 汇总 helper，以及 Slint `FileEntry` / `ItemViewEntry` / metadata source 默认投影；`RawFileEntry`、`PaneEntrySnapshot`、`FileEntry` 和 `&T` 共用同一模型入口，virtual snapshot、selection/search、navigation/status、thumbnail range checks 和 layout width 输入不再分别复制 `PaneEntrySnapshot` 字段逻辑。`PaneEntryModel` 现在包装 pane/cache/virtual entry storage，`PaneState`、`PreparedDirectoryEntries`、`VirtualViewSnapshotInput/Update` 不再暴露裸 `Arc<[PaneEntrySnapshot]>` / `Vec<PaneEntrySnapshot>` 容器，selection helper iterators 也暴露 `&dyn ItemViewModelEntry` 而不是 `&PaneEntrySnapshot`。`item_view.rs` 负责 input/controller/hit-test，`item_view_renderer.rs` 负责 render plan、metadata projection、fallback media kind、`ItemViewFrameEntry`、`ItemViewTileFrameSource`、`ItemViewTileFramePlan` 和 `ItemViewTileFrameBatch`。统一 Rust-side tile frame batch 现在派生 paint rows、selection/drop-target raster geometry、fallback media kind 和 media-token lookup；`model_update.rs` 只消费 batch API 来复用/发布 pane-local models，不再拆 `plans()` 分别重组 paint/highlight/fallback 状态。frame source 可由 `ItemViewEntry` 或 cached `ItemViewRowToken` 通过同一个 renderer/model input trait 提供；bounds-backed frame plan 只承载可绘制 text/fallback/highlight primitives，token-only thumbnail 更新通过 batch API 取 media token，batch 把二者作为一个 renderer/cache 输入边界保存。live entries / cached row tokens + bounds + selection 到 tile frame batch 的构建规则，以及 source-index 到 media-token 的查找规则也已下沉到 renderer，避免把无 bounds 的 media refresh 误塞进绘制列表，且不再由 `model_update.rs` 分别重组 entry/bounds/token 状态。`ItemViewTileFrameBatch` 现在还能从同一批 frame plan 生成 `SharedPixelBuffer`/`Image` raster layer，绘制选中背景、loaded thumbnail 和 drop target；`PaneView` 对当前 tile raster 保存 pane-local cache，并另外缓存当前尺寸/主题的 file/folder fallback icon image。row token、bounds、media token、selection 和 drop target 更新只 bump `raster_revision`，命中检查只比较 raster input + revision，重复 view sync 不再重画未变化底图。它们不再为每个 Slint primitive model 分别维护 zoom/media/font/line/cell/row 公式；fallback 源图、zoom 几何、model filter 和 text projection 已经解耦，为后续可选 text backend、真正的 Details/Icons polymorphic layouter、store-level model polymorphism 或 tile-frame 自绘提供更稳定的输入边界。
