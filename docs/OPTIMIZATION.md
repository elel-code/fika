...
| Thumbnail raster input | `thumbnail_pipeline.rs` / `model_update.rs` / `item_view_renderer.rs` | 基础 `ItemViewEntry` 不再携带 `Image`；已加载 thumbnail 先进入 Rust-only `ItemViewMediaSource`，再投影为 pane-local `ItemViewRasterMediaEntry` rows，并通过 Rust-only media token sidecar 复用 pane-local raster input，zoom/cached relayout 时可独立裁剪和平移；Slint 不再查 `root.bounds[media.slice_index]`，也不再持有 media row 或 media loop |
...
9. `ItemViewEntry.media_token` 作为真实 thumbnail 更新令牌进入可见 row；`src/app/model_update.rs` 同时维护 pane-local `ItemViewRowToken` sidecar、`ItemViewItemBounds` bounds sidecar、`ItemViewPaintEntry` paint sidecar、Rust-only media source/token sidecar 和 `ItemViewRasterMediaEntry` raster input。虚拟切片滑动的重叠 row 先比较 sidecar token，token 相同就不读取 `VecModel::row_data()`；bounds/paint sidecar 复用各自 pane-local `VecModel` 做滑动更新，selection/fallback/drop-target geometry 由 renderer batch 从 row token + bounds 按需生成，raster media input 复用同一个 pane-local `Vec` 并只用 Rust-only `slice_index + media_token` 判断 thumbnail row 更新，cached relayout 会同步裁剪 bounds、paint 和 Rust-only raster inputs。Slint-facing media row 已删除；raster layer 在 pane view sync 时按当前 pane 的可见 sidecar 重新生成。split pane 快照也会复制到独立 `VecModel`、cloned row-token sidecar、cloned paint sidecar、cloned raster-media sidecar 和 cloned media token sidecar，避免两个 pane 共享同一个可见 row 模型。
