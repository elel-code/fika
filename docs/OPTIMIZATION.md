...
| Cached icon zoom relayout + layout reuse | `geometry.rs` / `virtual_view.rs` / `model_update.rs` / `pane.rs` / `main.rs` | zoom 后只要求当前 slice 覆盖新真实可见区，未按新 `rows_per_column` 对齐的窗口也通过 `virtual_start_column + virtual_start_row` 直接复用，避免首次 zoom 裁剪 `VecModel` rows。同步 cached relayout 覆盖已缓存/预热过的大目录；未缓存 layout 只有不超过 `ICON_ZOOM_SYNC_UNCACHED_RELAYOUT_ENTRY_LIMIT` 的小目录才在 UI 线程构造，普通/大目录先同步刷新当前可见 slice 的 icon/text style，再走后台 full prepare，避免在 Ctrl+wheel/input 事件中收集过大的整目录宽度并构造完整 layout。cache miss 不在 UI 线程同步计算所有 zoom 档，而是复用 pane-local `Arc<ItemViewLayoutEngine>` 做可见区规划；后台 snapshot result 和 pane cache commit 继续传递同一个 `Arc`，`VirtualViewCache` 还保留最近几档 layout history，并在 virtual result / zoom fast path 后按距离顺序对全部 zoom 档做后台 layout prewarm。zoom 回到已构造或预热过的 signature 时直接复用历史 `Arc`，避免重新扫描 entries/visible-index 并重建 offset/width 数组。cache-hit 签名检查先按 `Arc::ptr_eq` 命中同一个 layout，避免滚动/viewport cache-hit 时再扫整张 offset/width 数组。compact layout 内部的 item/column width 和 offset 数组也用 `Arc` slice 承载。文本宽度由 item width 派生，不再为每个 entry 维护第二张 text-width 数组。compact 文本字号保持 Dolphin-style 稳定，只让 icon/media size 参与 zoom |
...
- `icon_zoom_layout_changed()` 立即调用 `sync_visible_pane_icon_zoom_layouts()`；`try_relayout_cached_pane_icon_zoom_layout()` 优先复用当前或 history 中的 layout，并且只要求当前 virtual slice 覆盖新 zoom 的真实可见 range。若目标 layout 未缓存/未预热，只有不超过 `ICON_ZOOM_SYNC_UNCACHED_RELAYOUT_ENTRY_LIMIT` 的小目录会在 UI 线程同步构造，普通/大目录退到 visible-style refresh + 后台 prepare，避免在 zoom 输入事件里构造过大的整目录 `visible_name_width_units` 和完整 compact layout。`cached_zoom_relayout_range()` 不再要求旧 slice 起点按新 `rows_per_column` 对齐，也不再裁剪到对齐列边界；Slint 侧用派生的 `virtual_start_column + virtual_start_row` 绘制任意起点的窗口，保留 selection/highlight sidecar，不重新 snapshot 当前目录。
- fast path 覆盖不了新真实可见 range，或 show-location metadata 仍在热路径里时，不再在 Ctrl+wheel/input 事件里同步 snapshot rebuild。普通无 metadata 目录会先调用 `sync_pane_icon_zoom_style_for_slot()`，把当前可见 slice 的 media/text style 按新 zoom 立即发布给 Slint；随后 `prepare_pane_icon_zoom_layout_for_slot()` 发起后台 virtual prepare，并在结果返回后提交准确的全目录 layout。这对应 Dolphin `KItemListView::setStyleOption()` 先更新 visible widgets、`KFileItemListView::triggerIconSizeUpdate()` 再延迟昂贵 icon/preview role 更新的分层。
...
- **Phase 5**: `PaneEntrySnapshot`（不含 `Image` 的轻量快照, `Arc` 零拷贝共享，并缓存 `name_width_units`）+ `VirtualViewSnapshotInput`（完全 owned 的纯函数输入）— 虚拟视图的条目过滤/切片/clone/location 标注全部在 `tokio::spawn_blocking` 中完成，UI 线程只做 generation staleness 检查 + Slint 模型写入 + 缩略图缓存装饰。Dolphin compact size-hint 所需的文件名估宽在 entry 进入 pane 时完成，virtual layout/cached zoom relayout 只按当前 font size 缩放宽度单位，后台 cache-miss layout 直接从 visible source 流式构造，不再额外分配整目录宽度中间表；cached layout、recent layout history、zoom 后台 prewarm 结果、snapshot result 和 pane cache commit 都通过 `Arc<ItemViewLayoutEngine>` 共享 compact layout，thumbnail size 不再阻止 layout reuse，cache-hit 签名检查优先使用 `Arc::ptr_eq`，compact 内部宽度/offset 数组同样用 `Arc<[f32]>`，搜索/过滤可见索引和 location group 缓存也通过 `Arc` slice 共享到 snapshot input。`virtual_generation` 独立于 `load_generation`，目录切换时自动推进；virtual refresh in-flight/pending bookkeeping 已收敛进 `VirtualViewRefreshState`，layout prewarm 是 pane-local current-generation single-flight 且不参与 Slint model commit，为后续完整 two-phase pending→commit view refresh 留出 pane-local 状态边界。`apply_virtual_view_result` 先在 `borrow_mut` 内写 state 再 drop 后写 Slint，避免 RefCell 跨线程风险。所有可见 pane 走同一条 slot-aware 虚拟视图管线。
