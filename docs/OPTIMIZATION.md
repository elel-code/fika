...
| Cached icon zoom relayout + layout reuse | `geometry.rs` / `virtual_view.rs` / `model_update.rs` / `pane.rs` / `main.rs` | zoom 后只要求当前 slice 覆盖新真实可见区，未按新 `rows_per_column` 对齐的窗口也通过 `virtual_start_column + virtual_start_row` 直接复用，避免首次 zoom 裁剪 `VecModel` rows。同步 cached relayout 只允许很小目录；超过 `ICON_ZOOM_SYNC_RELAYOUT_ENTRY_LIMIT` 的目录直接走后台 prepare，避免在 Ctrl+wheel/input 事件中收集整目录宽度并构造完整 layout。cache miss 不在 UI 线程同步计算所有 zoom 档，而是复用 pane-local `Arc<ItemViewLayoutEngine>` 做可见区规划；后台 snapshot result 和 pane cache commit 继续传递同一个 `Arc`，compact layout 内部的 item/column width 和 offset 数组也用 `Arc` slice 承载。文本宽度由 item width 派生，不再为每个 entry 维护第二张 text-width 数组。compact 文本字号保持 Dolphin-style 稳定，只让 icon/media size 参与 zoom |
| Cached filename width units | `pane.rs` / `geometry.rs` / `virtual_view.rs` / `main.rs` | `PaneEntrySnapshot` 缓存文件名估宽单位；virtual snapshot 和 cached zoom relayout 直接消费 `f32` 宽度单位，避免大目录 zoom/切换目录时重复 clone 文件名并逐字符估宽 |
...
- `VirtualViewCache.layout` 持有 `Arc<ItemViewLayoutEngine>`；构造后台 snapshot input、pane surface metrics、hit-test 和框选时只 clone `Arc`，不复制 column widths/offsets 等 layout 数组。`CompactItemViewLayout` 内部的 `item_widths`、`column_widths` 和 `column_offsets` 也使用 `Arc<[f32]>`，layout clone 不会复制整目录数组；item text width 从 item width 和固定 reserved 宽度派生，不再维护同长度的 `item_text_widths`。`prepare_virtual_view_snapshot_update()` 在 cache 签名、entry count 和 thumbnail size 匹配时通过 `cached_snapshot_layout()` 复用已有 compact layout，只按新的 viewport 计算 visible range；`VirtualViewSnapshotUpdate.layout` 和 pane cache 写回也继续共享同一个 `Arc`，避免后台结果返回 UI 线程时再次复制 layout 数组。
...
- `PaneEntrySnapshot` 在目录加载/缓存写入阶段预计算文件名宽度单位；`prepare_virtual_view_snapshot_update()` 和 `try_relayout_cached_pane_icon_zoom_layout()` 都直接消费这些 `f32` 宽度单位生成 compact layout。这样 Dolphin-style 每列取最长文件名的语义不变，但目录第一次 zoom、cache miss 后台 layout 和目录切换热路径不再重复分配 `Vec<String>` 或逐字符估宽。
...
- **Phase 5**: `PaneEntrySnapshot`（不含 `Image` 的轻量快照, `Arc` 零拷贝共享，并缓存 `name_width_units`）+ `VirtualViewSnapshotInput`（完全 owned 的纯函数输入）— 虚拟视图的条目过滤/切片/clone/location 标注全部在 `tokio::spawn_blocking` 中完成，UI 线程只做 generation staleness 检查 + Slint 模型写入 + 缩略图缓存装饰。Dolphin compact size-hint 所需的文件名估宽在 entry 进入 pane 时完成，virtual layout/cached zoom relayout 只按当前 font size 缩放宽度单位；cached layout、snapshot result 和 pane cache commit 都通过 `Arc<ItemViewLayoutEngine>` 共享 compact layout，compact 内部宽度/offset 数组同样用 `Arc<[f32]>`，搜索/过滤可见索引和 location group 缓存也通过 `Arc` slice 共享到 snapshot input。`virtual_generation` 独立于 `load_generation`，目录切换时自动推进；virtual refresh in-flight/pending bookkeeping 已收敛进 `VirtualViewRefreshState`，为后续完整 two-phase pending→commit view refresh 留出 pane-local 状态边界。`apply_virtual_view_result` 先在 `borrow_mut` 内写 state 再 drop 后写 Slint，避免 RefCell 跨线程风险。所有可见 pane 走同一条 slot-aware 虚拟视图管线。
