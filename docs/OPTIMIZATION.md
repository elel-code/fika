...
- **borrow-note**: pane status、pane slot model、Undo 注册/启动/完成状态等先在 `AppState` 借用内生成快照/summary，再释放借用后写 Slint row/model、Undo 按钮状态或清理旧 overwrite backup，避免 async operation status 更新期间被 Slint 回调重入触发 `RefCell` 借用 panic
