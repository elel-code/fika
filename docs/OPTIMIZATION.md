...
| Cached icon zoom relayout + layout reuse | `geometry.rs` / `virtual_view.rs` / `model_update.rs` / `pane.rs` / `main.rs` | zoom 后只要求当前 slice 覆盖新真实可见区，未按新 `rows_per_column` 对齐的窗口也通过 `virtual_start_column + virtual_start_row` 直接复用，避免首次 zoom 裁剪 `VecModel` rows。cache miss 不在 UI 线程同步计算所有 zoom 档，而是复用 pane-local `Arc<ItemViewLayoutEngine>` 做可见区规划；后台 snapshot result 和 pane cache commit 继续传递同一个 `Arc`，不会复制 compact offset 数组。compact 文本字号保持 Dolphin-style 稳定，只让 icon/media size 参与 zoom |
...
- `VirtualViewCache.layout` 持有 `Arc<ItemViewLayoutEngine>`；构造后台 snapshot input、pane surface metrics、hit-test 和框选时只 clone `Arc`，不复制 column widths/offsets 等 layout 数组。`prepare_virtual_view_snapshot_update()` 在 cache 签名、entry count 和 thumbnail size 匹配时通过 `cached_snapshot_layout()` 复用已有 compact layout，只按新的 viewport 计算 visible range；`VirtualViewSnapshotUpdate.layout` 和 pane cache 写回也继续共享同一个 `Arc`，避免后台结果返回 UI 线程时再次复制 layout 数组。
