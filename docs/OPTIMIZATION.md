...
| Cached icon zoom relayout + layout reuse | `geometry.rs` / `virtual_view.rs` / `model_update.rs` / `pane.rs` / `main.rs` | zoom 后只要求当前 slice 覆盖新真实可见区，未按新 `rows_per_column` 对齐的窗口也通过 `virtual_start_column + virtual_start_row` 直接复用，避免首次 zoom 裁剪 `VecModel` rows。同步 cached relayout 只允许很小目录；超过 `ICON_ZOOM_SYNC_RELAYOUT_ENTRY_LIMIT` 的目录先同步刷新当前可见 slice 的 icon/text style，再走后台 full prepare，避免在 Ctrl+wheel/input 事件中收集整目录宽度并构造完整 layout。cache miss 不在 UI 线程同步计算所有 zoom 档，而是复用 pane-local `Arc<ItemViewLayoutEngine>` 做可见区规划；后台 snapshot result 和 pane cache commit 继续传递同一个 `Arc`，cache-hit 签名检查先按 `Arc::ptr_eq` 命中同一个 layout，避免滚动/viewport cache-hit 时再扫整张 offset/width 数组。compact layout 内部的 item/column width 和 offset 数组也用 `Arc` slice 承载。文本宽度由 item width 派生，不再为每个 entry 维护第二张 text-width 数组。compact 文本字号保持 Dolphin-style 稳定，只让 icon/media size 参与 zoom |
...
- `icon_zoom_layout_changed()` 立即调用 `sync_visible_pane_icon_zoom_layouts()`；`try_relayout_cached_pane_icon_zoom_layout()` 只在可见 entry 数不超过 `ICON_ZOOM_SYNC_RELAYOUT_ENTRY_LIMIT` 时尝试同步重排，并且只要求当前 virtual slice 覆盖新 zoom 的真实可见 range。大目录或过滤结果超过阈值时直接转入后台 prepare，避免在 zoom 输入事件里构造整目录 `visible_name_width_units` 和完整 compact layout。`cached_zoom_relayout_range()` 不再要求旧 slice 起点按新 `rows_per_column` 对齐，也不再裁剪到对齐列边界；Slint 侧用派生的 `virtual_start_column + virtual_start_row` 绘制任意起点的窗口，保留 selection/highlight sidecar，不重新 snapshot 当前目录。
