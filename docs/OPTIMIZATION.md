...
| Sparse metadata overlay | `item_view.rs` / `model_update.rs` / `models.slint` | 基础 `ItemViewEntry` 不再携带 group/location 字符串和 metadata 几何；show-location 先发布 Rust-only 非空 `ItemViewMetadataOverlaySource` rows，再投影为 Slint-facing `ItemViewMetadataEntry`，row 自带 item x/y 且不携带 `slice_index`，Slint metadata loop 不再查 bounds sidecar |
