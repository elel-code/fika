...
`viewport-content-width`、`virtual-slice-start-x`、`virtual-slice-width` 和 `scroll-max-x` 现在由 Rust `pane_slot_item_view_metrics()` 通过同一套 `MainItemViewLayout` / `compact_item_view_layout()` 计算，并随 `PaneViewData` 下发给 `SplitPaneView`。其中 virtual slice 起点/宽度由 `CompactItemViewLayout::virtual_slice_geometry()` 统一提供，`split_view.rs` 不再直接读取 column offsets。Slint 只消费这些 metrics 做 scrollbar、viewport clamp、smooth paint viewport 覆盖判断和切片偏移，不再自己根据 `entry-count` / `rows-per-column` / `zoom-level` 重算主视图 layouter。
