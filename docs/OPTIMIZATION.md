...
4. 普通 compact item-view 的 tile height 与 row height 同源；普通标题按 Dolphin compact text cache 的分层方式使用整 tile 高度作为 text frame，带 group/location 的 metadata title 行继续复用 Rust 投影的 `title_y/title_line_height` 并通过 sparse overlay 绘制额外文本
