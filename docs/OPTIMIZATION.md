...
| Dolphin-style pane search selectors | `search_panel.slint` / `app.slint` | 搜索过滤不再展开成 pane 内过滤框；`Filter` 主菜单只列 selector，active chip 直接打开对应 Type/Modified/Size selector，popup anchored 到按钮/chip 且不改变主视图高度 |
...
11. Dolphin compact visual metrics 已收敛到 `src/app/item_view_metrics.rs`：`geometry.rs` 负责 viewport/visible range/layout，Slint metrics projection、range anchor、item rect、virtual slice geometry、range-hint 对齐和 layout cache signature 查询也由 `CompactItemViewLayout` 提供；`item_view.rs` 负责 input/controller/hit-test，`item_view_renderer.rs` 负责 render plan、metadata projection 和 fallback media。它们不再分别维护 zoom/media/font/line/cell/row 公式；fallback 源图和 zoom 几何已经解耦，为后续 renderer cache 或 tile-frame 自绘提供更稳定的输入边界。
12. `ItemViewInputState` 已开始输出 `ItemViewControllerAction`：item press 通过 `press_entry_at_pane_point()` 在 `item_view.rs` 内完成坐标 hit-test、记录 pane-local drag source 并返回选择路径 action，activation 通过 `activate_entry_at_pane_point()` 返回 `ActivatePath` action，blank press/move/release/cancel 也通过 slot-level controller helpers 更新 pane-local gesture state，blank release 再返回清空选择或矩形选择 action；`main.rs` 只执行 action，不再直接写 input state、复制 item press/activation hit-test 或匹配 blank gesture 内部细节。这仍不是 Dolphin 式完整 controller signal bus，但已经把 press/release/activation 的手势状态和 UI 执行动作分开。
