...
12. `ItemViewInputState` 已开始输出 `ItemViewControllerAction`：item press 通过 `press_entry_at_pane_point()` 在 `item_view.rs` 内完成坐标 hit-test、记录 pane-local drag source 并返回选择路径 action，blank press/move/release/cancel 也通过 slot-level controller helpers 更新 pane-local gesture state，blank release 再返回清空选择或矩形选择 action；`main.rs` 只执行 action，不再直接写 input state、复制 item press hit-test 或匹配 blank gesture 内部细节。这仍不是 Dolphin 式完整 controller signal bus，但已经把 press/release 的手势状态和 UI 执行动作分开。
