...
| Cached icon zoom relayout + zoom range hint | `geometry.rs` / `virtual_view.rs` / `model_update.rs` / `main.rs` | 目录/滚动重建时为各 icon zoom 档的真实可见区预留 pane-local 热窗口；zoom 后只要求当前 slice 覆盖新真实可见区，并优先保留已按新 `rows_per_column` 对齐的热窗口，避免首次 zoom 裁剪 `VecModel` rows。compact 文本字号保持 Dolphin-style 稳定，只让 icon/media size 参与 zoom |
