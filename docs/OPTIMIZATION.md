...
Rectangle viewport shell (clip: true)
  ...
  │    │    └─ Rust 绘制当前 virtual slice 的选中背景 + folder/file fallback glyph
  ...
  │    │    └─ 只叠加已加载 thumbnail；slice_index 只留在 Rust-only source/token
...
主文件视图使用 Dolphin compact 模式语义：物理滚动轴固定为 X，条目按 `index % rows_per_column` 先填满一列，再按 `index / rows_per_column` 进入下一列。compact item 尺寸沿用 Dolphin 的公式：`itemWidth = padding * 4 + iconSize + fontHeight * 5`，`itemHeight = padding * 2 + max(iconSize, textLines * lineSpacing)`，列间 margin 为 `8px`。普通目录使用 1 行标题，Trash 和递归搜索使用 pane-local 的 3 行 group/title/location 布局；每个 pane 独立计算自身行数、可见范围和 viewport。Dolphin 对照点是 `KStandardItemListView::setItemLayout()` 在 CompactLayout 下设置 `Qt::Horizontal`，`KItemListViewLayouter` 通过转置逻辑把纵向流映射成物理横向滚动，`KStandardItemListWidget::updateCompactLayoutTextCache()` 预先计算 compact 文本位置和最大宽度，并把 compact text frame 关联到整个 item 高度；文字本身使用居中的 line position。Fika 同样在 Rust render plan 中投影非零 `title_y/title_line_height/text_width`；普通标题的 Rust-projected `Text` frame 覆盖整 tile 高度并由 Slint vertical alignment 居中，避免大 zoom 下单行 frame 被裁剪导致 name 消失；带 group/location 的标题行继续使用 Rust 投影的 line frame，额外 group/location 行通过 pane-local sparse metadata overlay 绘制。基础 `ItemViewEntry` 热 row 只留在 Rust `PaneView` 内部，携带业务身份、目录标记、缩略图状态和 media token，供 controller、hit-test、DnD 和 thumbnail token sidecar 使用；它不再作为 `PaneViewData` 字段传给 Slint。Slint 基础 name loop 消费 pane-local `ItemViewPaintEntry`，成功 thumbnail media 由 pane-local `ItemViewMediaEntry` 稀疏 overlay 承载，selection highlight 和 folder/file fallback glyph 由 Rust 生成当前 virtual slice 的 `item_view_raster_layer` 底图。media/metadata 的 `slice_index` 只保留在 Rust-only `ItemViewMediaSource` / `ItemViewMetadataOverlaySource` 和 media token sidecar 中，Slint-facing overlay rows 只携带绘制字段。drop target 复用 `paint[drag-target-slice-index]` 的几何，因此完整 `ItemViewItemBounds` 模型不再进入 `PaneViewData`。media/text/title 几何挂在 pane-level `PaneViewData` 上，Rust-only highlight/fallback sidecar 只作为 raster 输入，不再进入 Slint-facing model。metadata 由 Rust render plan 直接投影成 `ItemViewMetadataEntry` 稀疏 rows。`SplitPaneView` 只消费 raster 底图、pane-level 几何和 paint/media/metadata overlay sidecar，不再在可见 title loop 内按 `group/location` 或 `is_dir` 做 per-item 判断。

### 现有优化

补充进展：`ItemViewPaintEntry` 已继续收窄为 title 文本和 Rust-projected 绘制几何，不再携带 `is_dir` 或绘制 loop 不读取的 `slice_index`。`ItemViewHighlightEntry` 和 `ItemViewFallbackMediaEntry` 已从 Slint-facing struct 下沉为 Rust-only sidecar，用于生成当前 virtual slice 的 `item_view_raster_layer`；Slint 不再有 highlight loop、folder-media loop 或 file-media loop。`ItemViewMediaEntry` 和 `ItemViewMetadataEntry` 同样不再携带 Slint-facing `slice_index`，身份只保留在 Rust-only source/token 中。
...
| Rust item-view render plan | `item_view_renderer.rs` / `split_view.rs` / `split_pane.slint` | 主视图行列/滚动 metrics、tile x/y/width/text width、media/text rect、尺寸/字体 token 和 selection/fallback glyph 不再由 Slint 每项公式或 hot row data 承载；可见底图由 Rust raster layer 提供，Slint 只叠加 thumbnail/text/metadata |
| Sparse thumbnail media overlay | `thumbnail_pipeline.rs` / `model_update.rs` / `models.slint` | 基础 `ItemViewEntry` 不再携带 `Image`；已加载 thumbnail 先进入 Rust-only `ItemViewMediaSource`，再投影为 pane-local `ItemViewMediaEntry` rows，并通过 Rust-only media token sidecar 复用 pane-local model，zoom/cached relayout 时可独立裁剪和平移；Slint-facing overlay row 只带 image/x/y，Slint 不再查 `root.bounds[media.slice_index]` 也不再持有 `media.slice_index` |
...
`ItemViewRowToken` 覆盖 name/path/is_dir/selection/thumbnail_state/media token 等轻量身份字段，避免为了比较复用而从 Slint `VecModel` 读取 row；它现在归属 `src/app/model_update.rs`，与 virtual row reuse / selection sidecar/media/paint sidecar 更新放在同一层，而不是放在 input/controller/hit-test 的 `item_view.rs`。split pane snapshot 会复制到独立 `VecModel` 和 sidecar，避免两个 pane 共享同一个可见 row model。选中态只保存在 token sidecar 中，并投影成 Rust-only pane-local `ItemViewHighlightEntry` geometry，再由 `PaneView` 生成当前 virtual slice 的 raster 底图；selection 或 virtual row 变化时不再发布 Slint highlight model。基础 `Text` loop 和 `ItemViewEntry` row data 不再携带 per-item selected 状态、成功 thumbnail image、group/location metadata 或 per-row 几何；基础 title 绘制消费由 row token + Rust bounds 投影出的 pane-local `ItemViewPaintEntry` sidecar。folder/file fallback media 也从 paint row 分支拆成 Rust-only sparse `ItemViewFallbackMediaEntry` sidecar，并画入同一张 raster 底图；Slint 不再使用两个固定 source `Image` loop，也不再对每个可见 row 绑定 `paint.is_dir ? folder : file`。成功 thumbnail image 从缓存投影成 Rust-only `ItemViewMediaSource`，再发布为独立的 pane-local sparse `ItemViewMediaEntry` model；show-location metadata 从 `FileEntry` snapshot 经 Rust render plan 投影成 Rust-only `ItemViewMetadataOverlaySource`，再发布为独立的 pane-local sparse `ItemViewMetadataEntry` model。可见项的 x/y/width/text-width 继续由 pane-local `ItemViewItemBounds` sidecar 在 Rust 侧维护并投影到 Slint-facing paint/thumbnail-media/metadata rows，同时供 Rust-only highlight/fallback raster 输入使用；Slint 基础 loop 不再重复计算 `virtual-start-row + index` 和 modulo row，也不再接收完整 bounds model。drop target 复用 `paint[drag-target-slice-index]`，所有 remaining overlay 都不再查 `root.bounds[slice_index]`。Rust bounds sidecar 和 Slint paint sidecar 都复用自己的 pane-local `VecModel`，普通滚动和 cached zoom relayout 只做 row 级滑动/更新，不再每次替换整个 model；highlight/fallback sidecar 留在 Rust 内作为 raster invalidation 输入。media sidecar 使用 Rust-only `slice_index + media_token` 判断 sparse thumbnail row 是否变更，复用已有 pane-local `VecModel<ItemViewMediaEntry>`，避免为了模型复用去比较或读取 `Image`；Slint-facing media row 只保留 image/x/y。metadata sidecar 是稀疏 overlay，一个 item 可能对应 0/1/2 行，因此不套用 item-range 滑动算法；非空 metadata 更新复用已有 pane-local `VecModel` 做 row 级比较/更新，空 metadata 仍回到空模型语义，且 Slint-facing metadata row 不再携带 `slice_index`。

**收益**：连续滚动时重叠 virtual rows 不再因为 slice-local 坐标、pane-level 几何或 fallback glyph 变化被重发；selection/fallback 变化只刷新 Rust sidecar 和当前 raster 底图，真实 thumbnail 变化只刷新 sparse media overlay；Rust bounds sidecar、Slint paint/thumbnail-media 和非空 metadata model 保持稳定，且 Slint 少持有一份完整可见 bounds model，也不再在 fallback media loop 对每个 row 求 `is_dir` 分支，避免 Rust-projected 几何/图像/文本 overlay 下沉后又通过整模型替换刺激 Slint 可见 primitive 绑定。
...
8. 文件/目录 fallback media 已从 Slint `FolderGlyph` 组件迁到 `item_view_renderer.rs` 并继续下沉到 tile raster base layer：folder/file fallback glyph 直接画进当前 virtual slice 的 `PaneViewData.item_view_raster_layer`，主视图不再下发 `item_view_folder_media` / `item_view_file_media`，也不再有 folder/file fallback `Image` loop。成功 thumbnail image 先进入 Rust-only `ItemViewMediaSource`，再通过 pane-local sparse `ItemViewMediaEntry` overlay 覆盖 raster 底图中的 fallback glyph。theme refresh 和 zoom fast path 通过 `sync_pane_view_ui()` 重新生成当前可见 raster，zoom 时只更新 media rect/row/cell 目标几何和当前 slice 底图，为后续 renderer state 继续收敛到 Rust 侧铺路。
9. `ItemViewEntry.media_token` 作为真实 thumbnail 更新令牌进入可见 row；`src/app/model_update.rs` 同时维护 pane-local `ItemViewRowToken` sidecar、`ItemViewItemBounds` bounds sidecar、`ItemViewPaintEntry` paint sidecar、Rust-only highlight/fallback sidecar、Rust-only media source/token sidecar 和 `ItemViewMediaEntry` sparse overlay。虚拟切片滑动的重叠 row 先比较 sidecar token，token 相同就不读取 `VecModel::row_data()`；bounds/paint sidecar 复用各自 pane-local `VecModel` 做滑动更新，media overlay 复用同一个 pane-local `VecModel` 并只用 Rust-only `slice_index + media_token` 判断 thumbnail row 更新，cached relayout 会同步裁剪 bounds、paint、Rust-only raster inputs 和 sparse media overlay。Slint-facing media row 只保留 image/x/y；raster layer 在 pane view sync 时按当前 pane 的可见 sidecar 重新生成。split pane 快照也会复制到独立 `VecModel`、cloned row-token sidecar、cloned paint sidecar、cloned media sidecar 和 cloned media token sidecar，避免两个 pane 共享同一个可见 row 模型。
