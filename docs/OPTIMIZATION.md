...
`ItemViewRowToken` 覆盖 name/path/is_dir/selection/thumbnail_state/media token 等轻量身份字段，避免为了比较复用而从 Slint `VecModel` 读取 row；它现在归属 `src/app/model_update.rs`，与 virtual row reuse / selection sidecar/media/paint sidecar 更新放在同一层，而不是放在 input/controller/hit-test 的 `item_view.rs`。split pane snapshot 会复制到独立 `VecModel` 和 sidecar，避免两个 pane 共享同一个可见 row model。选中态只保存在 token sidecar 中，并投影成 Rust-only pane-local `ItemViewHighlightEntry` geometry，再由 `PaneView` 生成当前 virtual slice 的 raster 底图；selection 或 virtual row 变化时不再发布 Slint highlight model。基础 `Text` loop 和 `ItemViewEntry` row data 不再携带 per-item selected 状态、已加载 thumbnail image、group/location metadata 或 per-row 几何；基础 title 绘制消费由 row token + Rust bounds 投影出的 pane-local `ItemViewPaintEntry` sidecar。folder/file fallback media 也从 paint row 分支拆成 Rust-only sparse `ItemViewFallbackMediaEntry` sidecar，并画入同一张 raster 底图；Slint 不再使用两个固定 source `Image` loop，也不再对每个可见 row 绑定 `paint.is_dir ? folder : file`。已加载 thumbnail image 从缓存投影成 Rust-only `ItemViewMediaSource`，再投影为 pane-local `ItemViewRasterMediaEntry` raster input 并画入同一张底图；当前 drop target index 也作为 PaneView transient state 进入 raster input。show-location metadata 从 `FileEntry` snapshot 经 Rust render plan 投影成 Rust-only `ItemViewMetadataOverlaySource`，再发布为独立的 pane-local sparse `ItemViewMetadataEntry` model。可见项的 x/y/width/text-width 继续由 pane-local `ItemViewItemBounds` sidecar 在 Rust 侧维护并投影到 Slint-facing paint/metadata rows，同时供 Rust-only highlight/fallback/thumbnail/drop-target raster 输入使用；Slint 基础 loop 不再重复计算 `virtual-start-row + index` 和 modulo row，也不再接收完整 bounds model。所有 remaining overlay 都不再查 `root.bounds[slice_index]`。Rust bounds sidecar 和 Slint paint sidecar 都复用自己的 pane-local `VecModel`，普通滚动和 cached zoom relayout 只做 row 级滑动/更新，不再每次替换整个 model；highlight/fallback/thumbnail/drop-target sidecar 留在 Rust 内作为 raster invalidation 输入。media sidecar 使用 Rust-only `slice_index + media_token` 判断 thumbnail raster input 是否变更，复用 pane-local `Vec<ItemViewRasterMediaEntry>`，避免为了模型复用去比较或读取 `Image`；Slint-facing media row 已删除。metadata sidecar 是稀疏 overlay，一个 item 可能对应 0/1/2 行，因此不套用 item-range 滑动算法；非空 metadata 更新复用已有 pane-local `VecModel` 做 row 级比较/更新，空 metadata 仍回到空模型语义，且 Slint-facing metadata row 不再携带 `slice_index`。
...
9. `ItemViewEntry.media_token` 作为真实 thumbnail 更新令牌进入可见 row；`src/app/model_update.rs` 同时维护 pane-local `ItemViewRowToken` sidecar、`ItemViewItemBounds` bounds sidecar、`ItemViewPaintEntry` paint sidecar、Rust-only highlight/fallback sidecar、Rust-only media source/token sidecar 和 `ItemViewRasterMediaEntry` raster input。虚拟切片滑动的重叠 row 先比较 sidecar token，token 相同就不读取 `VecModel::row_data()`；bounds/paint sidecar 复用各自 pane-local `VecModel` 做滑动更新，raster media input 复用同一个 pane-local `Vec` 并只用 Rust-only `slice_index + media_token` 判断 thumbnail row 更新，cached relayout 会同步裁剪 bounds、paint 和 Rust-only raster inputs。Slint-facing media row 已删除；raster layer 在 pane view sync 时按当前 pane 的可见 sidecar 重新生成。split pane 快照也会复制到独立 `VecModel`、cloned row-token sidecar、cloned paint sidecar、cloned raster-media sidecar 和 cloned media token sidecar，避免两个 pane 共享同一个可见 row 模型。
...
**实际实现**（✅ 已完成）：`ItemViewEntry` 不再带 `selected` 字段。Rust 侧把当前 pane 的 selection 只写入 pane-local `ItemViewRowToken` sidecar，再投影成 Rust-only `ItemViewHighlightEntry` 几何，并由 `PaneView` 生成当前 virtual slice 的 `item_view_raster_layer`。Slint 不再有 `root.highlights` loop。

选择变化时只更新 token sidecar、Rust-only highlight geometry 和当前 raster 底图，不再对 `VecModel<ItemViewEntry>` 做 row-data 写回，也不再由 `split_view` 每次 UI 同步临时构造 Slint highlight model。旧的 `selection_revision` 全局属性和 `PaneViewData.selection_revision` 热字段已删除，选择变化不再为了触发 tile 绑定而污染 pane view row；后台虚拟视图结果应用时会用当前 pane selection 初始化 token sidecar，旧异步结果不会把高亮写进基础 row。渲染路径上的 `PaneRouting.is-selected` / `FilePane.is_selected` 回调已删除；item 右键命中后是否需要先选中由 Rust 坐标 helper 按 pane selection 状态直接判断。
