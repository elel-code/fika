...
- Folder/file fallback glyph 已画入当前 virtual slice 的 tile raster base layer；目录/滚动 snapshot 和 theme refresh 仍可同步重新生成当前可见 raster，但 zoom fast path 只更新 pane-level media 目标几何和当前 slice paint/text metrics，复用旧 raster 直到 coalesced timer flush。新的 folder/file fallback `Image` source 不再下发给 Slint，贴近 Dolphin 的可见区 pixmap/cache 思路并降低 `/etc` 这类 fallback-heavy 目录的首次 zoom 卡顿。
- `PaneLayoutSyncScheduler` 使用 `TimerMode::SingleShot` 以 300ms 合并连续 zoom 后的 raster/icon layer 刷新和缩略图调度；timer flush 时先调用 `refresh_visible_pane_tile_frame_rasters()` 生成 latest zoom 的当前可见 raster，再从当前可见 virtual slice 调用 `schedule_visible_thumbnails_for_visible_panes()`。该路径从 pane-local `ItemViewRowToken` sidecar 派生轻量 `ThumbnailScheduleEntry`，不再从 Slint `VecModel<ItemViewEntry>` 读取 row；缩略图优先级按“真实可见区、前置 overscan、后置 overscan”三段 slice iterator 流式调度，不再为每次滚动/virtual sync 构造 `Vec<&T>`；临时 path 使用 `SharedString`，只有真正入队的缩略图任务才转成 owned `PathBuf`。
...
- 普通 `pane_layout_changed()` 仍调用 `sync_now()`，会停止 pending zoom raster/thumbnail timer 并立即刷新；窗口大小、sidebar、split ratio、pane 宽度变化不等待 timer。

**收益**：连续 zoom 时避免每个 zoom step 都启动缩略图解码/预览更新，也避免每个 cache-hit step 都同步重画 tile raster；当前 slice 覆盖新行数且 layout 已缓存时只做 pane-level layout/text/paint metrics 更新，不再裁剪 `VecModel`；未缓存普通/大目录不再在 UI 线程构造完整 compact layout，也不再同步重画当前 slice raster；普通目录在初次 virtual result 后后台预热全部 zoom layout，并且相邻档位先提交，后续 zoom 或回退 zoom 可更早命中 history，减少首次交互输入事件里的列宽/offset 构造；`/etc` 这类非图片目录的首次 zoom 不再因为缩略图候选探测、未缓存 layout 构造或临时 raster 重画而冷启动重活；稳定字体减少新 zoom 档位的 Text layout 冷启动；预计算文件名宽度单位和流式 layouter 输入减少大目录 zoom/目录切换时的字符串 clone、逐字符估宽和中间宽度数组分配；同时保留 resize/fullscreen/末尾 viewport 修复的即时路径。

**验证**：源码守卫测试确认 `PaneViewSyncScheduler` 的滚动路径仍无 timer，`icon_zoom_layout_changed` 同步刷新 layout，优先尝试 cached relayout，未缓存普通/大目录 cache miss 进入后台 prepare，cached relayout 通过 `sync_pane_view_ui_defer_raster()` 复用旧 raster，raster/icon layer 和缩略图调度才走 300ms coalesced timer，并且 zoom input path 不再调用旧的多档 range 同步计算或 cache-miss visible-style/raster 刷新。`virtual_view` 测试覆盖 `range_hint` 的后台扩展能力、thumbnail-size 变化时的 layout reuse、layout history reuse，以及 layout prewarm 复用/构造；`main` 测试覆盖 cached zoom relayout 只要求真实可见 range、已对齐和未对齐热窗口都不裁剪，并守卫 zoom layout prewarm 逐档调用 `prepare_virtual_view_layout_prewarm_input()`、逐档提交、最后一档才 finish；`pane` 测试覆盖 fallback media 多槽复用、split snapshot 继承热 cache、recent previous layout history、current layout 不重复写入 history、layout prewarm generation-scoped finish 和内容变化清空旧 virtual layout；`model_update` 测试覆盖 cached relayout 复用 `VecModel`、保留 selection/highlight 并裁剪 Rust-only raster media input；`thumbnail_pipeline` 测试覆盖旧 size thumbnail 不阻塞新 size 调度、row-token 调度不读取 Slint row model，以及非候选 row/token 在 zoom 后直接跳过；`fs::thumbnails` 测试覆盖未知扩展不进入 thumbnailer lookup。`cargo test app::virtual_view` / `app::geometry` / `app::pane` 覆盖预计算宽度单位接入后的 virtual layout、Dolphin compact geometry 和 pane snapshot/raster cache 路径。
