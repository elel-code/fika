...
- `VirtualViewCache.layout` 持有 `Arc<ItemViewLayoutEngine>`；构造后台 snapshot input、pane surface metrics、hit-test 和框选时只 clone `Arc`，不复制 column widths/offsets 等 layout 数组。`CompactItemViewLayout` 内部的 `item_widths`、`column_widths` 和 `column_offsets` 也使用 `Arc<[f32]>`，layout clone 不会复制整目录数组；item text width 从 item width 和固定 reserved 宽度派生，不再维护同长度的 `item_text_widths`。`prepare_virtual_view_snapshot_update()` 在 cache 签名、entry count 和 thumbnail size 匹配时通过 `cached_snapshot_layout()` 复用已有 compact layout，只按新的 viewport 计算 visible range；`VirtualViewSnapshotUpdate.layout` 和 pane cache 写回也继续共享同一个 `Arc`。`VirtualViewCache::matches_layout_arc()` 在 thumbnail size 匹配后先用 `Arc::ptr_eq` 判断是否为同一个 layout，只有非同一 Arc 时才回退完整 signature 比较，避免滚动/viewport cache-hit 时扫描整目录 `item_widths` / `column_offsets` 数组。
