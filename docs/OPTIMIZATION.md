...
| Pane-local tile raster cache | `pane.rs` / `item_view_renderer.rs` | 当前 virtual slice 的 raster base layer 按 raster input、row token、bounds、media token 精确签名复用；重复 view sync、未变化的 hover/drop 状态或 surface 同步不再重画同一张底图 |
