...
| Dolphin-style synchronous icon zoom layout | `geometry.rs` / `virtual_view.rs` / `pane.rs` / `main.rs` | `icon_zoom_layout_changed()` 立即同步刷新当前 pane layout、visible slice 和 raster，匹配 Dolphin `KItemListView::setStyleOption()` / `doLayout()`；只把 thumbnail/preview roles updater 延迟 300ms。`VirtualViewCache` 只保留当前 `Arc<ItemViewLayoutEngine>` |
...
- `icon_zoom_layout_changed()` 立即调用 `sync_visible_pane_icon_zoom_layouts()`，每个 pane 通过 `sync_virtual_entries_for_slot_with_count(... immediate=true, publish_layout_on_cache=true)` 同步执行 `prepare_virtual_view_snapshot_update()`，提交当前 layout、visible slice 和 tile raster。这对应 Dolphin 的 `KItemListWidget::setStyleOption()` / `KItemListView::doLayout()` 立即更新可见 widget。
