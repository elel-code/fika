...
- UI 线程不再为目录/滚动重建同步计算所有 zoom 档的可见 range；`VirtualViewSnapshotInput.range_hint` 在当前 UI 路径传 `None`，避免请求创建阶段扫描整目录并构造多份 compact layout。相邻 zoom 档 layout prewarm 只在 virtual result / zoom fast path 之后通过 `tokio::spawn_blocking` 后台执行；每个 pane 同时只允许一个 prewarm in flight，结果返回后再按 pane-local generation 校验，不触碰 Slint model。
...
**收益**：连续 zoom 时保留即时视觉反馈，避免每个 zoom step 都启动缩略图解码/预览更新；当前 slice 覆盖新行数时只做 pane-level layout 更新和 active fallback cache 切换，不再裁剪 `VecModel`；cache miss 不再同步计算所有 zoom 档、clone layout 数组或装饰虚拟切片；普通目录在初次 virtual result 后后台预热相邻 zoom layout，后续相邻 zoom 或回退 zoom 可直接命中 history，减少首次交互输入事件里的列宽/offset 构造；`/etc` 这类非图片目录的首次 zoom 不再因为缩略图候选探测而冷启动 thumbnailer registry；稳定字体减少新 zoom 档位的 Text layout 冷启动；预计算文件名宽度单位和流式 layouter 输入减少大目录 zoom/目录切换时的字符串 clone、逐字符估宽和中间宽度数组分配；同时保留 resize/fullscreen/末尾 viewport 修复的即时路径。

**验证**：源码守卫测试确认 `PaneViewSyncScheduler` 的滚动路径仍无 timer，`icon_zoom_layout_changed` 同步刷新 layout，优先尝试 cached relayout，cache miss 进入后台 prepare，缩略图调度才走独立 coalesced timer，并且 zoom input path 不再调用旧的多档 range 同步计算。`virtual_view` 测试覆盖 `range_hint` 的后台扩展能力、thumbnail-size 变化时的 layout reuse、layout history reuse，以及 layout prewarm 复用/构造；`main` 测试覆盖 cached zoom relayout 只要求真实可见 range、已对齐和未对齐热窗口都不裁剪，并守卫相邻 layout prewarm 在后台路径执行；`pane` 测试覆盖 fallback media 多槽复用、split snapshot 继承热 cache、recent previous layout history、current layout 不重复写入 history、layout prewarm pane-local single-flight 和内容变化清空旧 virtual layout；`model_update` 测试覆盖 cached relayout 复用 `VecModel`、保留 selection/highlight 并裁剪 Rust-only raster media input；`thumbnail_pipeline` 测试覆盖旧 size thumbnail 不阻塞新 size 调度、row-token 调度不读取 Slint row model，以及非候选 row/token 在 zoom 后直接跳过；`fs::thumbnails` 测试覆盖未知扩展不进入 thumbnailer lookup。`cargo test app::virtual_view` / `app::geometry` / `app::pane` 覆盖预计算宽度单位接入后的 virtual layout、Dolphin compact geometry 和 pane snapshot/raster cache 路径。
...
- **Phase 5**: `PaneEntrySnapshot`（不含 `Image` 的轻量快照, `Arc` 零拷贝共享，并缓存 `name_width_units`）+ `VirtualViewSnapshotInput`（完全 owned 的纯函数输入）— 虚拟视图的条目过滤/切片/clone/location 标注全部在 `tokio::spawn_blocking` 中完成，UI 线程只做 generation staleness 检查 + Slint 模型写入 + 缩略图缓存装饰。Dolphin compact size-hint 所需的文件名估宽在 entry 进入 pane 时完成，virtual layout/cached zoom relayout 只按当前 font size 缩放宽度单位，后台 cache-miss layout 直接从 visible source 流式构造，不再额外分配整目录宽度中间表；cached layout、recent layout history、相邻 zoom 后台 prewarm 结果、snapshot result 和 pane cache commit 都通过 `Arc<ItemViewLayoutEngine>` 共享 compact layout，thumbnail size 不再阻止 layout reuse，cache-hit 签名检查优先使用 `Arc::ptr_eq`，compact 内部宽度/offset 数组同样用 `Arc<[f32]>`，搜索/过滤可见索引和 location group 缓存也通过 `Arc` slice 共享到 snapshot input。`virtual_generation` 独立于 `load_generation`，目录切换时自动推进；virtual refresh in-flight/pending bookkeeping 已收敛进 `VirtualViewRefreshState`，layout prewarm 是 pane-local single-flight 且不参与 Slint model commit，为后续完整 two-phase pending→commit view refresh 留出 pane-local 状态边界。`apply_virtual_view_result` 先在 `borrow_mut` 内写 state 再 drop 后写 Slint，避免 RefCell 跨线程风险。所有可见 pane 走同一条 slot-aware 虚拟视图管线。
