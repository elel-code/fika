...
- UI 线程不再为目录/滚动重建同步计算所有 zoom 档的可见 range；`VirtualViewSnapshotInput.range_hint` 在当前 UI 路径传 `None`，避免请求创建阶段扫描整目录并构造多份 compact layout。相邻 zoom 档 layout prewarm 只在 virtual result / zoom fast path 之后通过 `tokio::spawn_blocking` 后台执行，并带 pane-local generation 校验，不触碰 Slint model。
