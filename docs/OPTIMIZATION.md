...
12. `ItemViewInputState` 已开始输出 `ItemViewControllerAction`：item press 在 `item_view.rs` 内记录 pane-local drag source 并返回选择路径 action，blank release 返回清空选择或矩形选择 action；`main.rs` 只执行 action，不再直接写 input state 或匹配 blank gesture 内部细节。这仍不是 Dolphin 式完整 controller signal bus，但已经把 press/release 的手势状态和 UI 执行动作分开。
