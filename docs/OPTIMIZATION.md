...
**验证**：源码守卫测试确认 `PaneViewSyncScheduler` 的滚动路径仍无 timer，`icon_zoom_layout_changed` 同步刷新 layout，优先尝试 cached relayout，cache miss 进入后台 prepare，缩略图调度才走独立 coalesced timer。`virtual_view` 测试覆盖 `range_hint` 的对齐扩展；`main` 测试覆盖 cached zoom relayout 只要求真实可见 range、已对齐和未对齐热窗口都不裁剪，以及 zoom hint 按目标 rows 对齐；`pane` 测试覆盖 fallback media 多槽复用和 split snapshot 继承热 cache；`model_update` 测试覆盖 cached relayout 复用 `VecModel`、保留 selection/highlight 并裁剪 Rust-only raster media input；`thumbnail_pipeline` 测试覆盖旧 size thumbnail 不阻塞新 size 调度、row-token 调度不读取 Slint row model，以及非候选 row/token 在 zoom 后直接跳过；`fs::thumbnails` 测试覆盖未知扩展不进入 thumbnailer lookup。
