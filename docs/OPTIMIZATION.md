       ...
       └─ root.view_changed() → PaneRouting.view-changed(slot)
            └─ Rust: PaneViewSyncScheduler 同步重建当前 visible slice
                 └─ sync_virtual_entries_for_slot()  [src/main.rs]
                      ...
                      ├─ set pane-local raster/paint/media/metadata → Slint；bounds/highlight/fallback 保留在 Rust sidecar
...
- `PaneViewData` 承载 viewport、entry count、layout metrics、空状态、drop/content interactive、当前 tile raster base layer，以及 pane-local `paint` / `media` / `metadata` models；`ItemViewItemBounds`、highlight 和 fallback geometry 保留为 Rust-side projection/raster sidecar。
...
**收益**：虚拟切片、viewport clamp、目录 view-state 恢复、selection 更新不再重建 pane chrome row；raster/paint/media/metadata 都是 pane-local view row 数据，bounds/highlight/fallback 只保留在 Rust sidecar，避免全局 slot 0/1 sidecar 重新引入“主 pane”概念。
