...
| Rust item-view render plan | `item_view_renderer.rs` / `split_view.rs` / `split_pane.slint` | 主视图行列/滚动 metrics、tile x/y/width/text width、media/text rect、尺寸/字体 token 和 selection/fallback/thumbnail raster 输入不再由 Slint 每项公式或 hot row data 承载；可见底图由 Rust raster layer 提供，Slint 只叠加 text/metadata/drop target |
