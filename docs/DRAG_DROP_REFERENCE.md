...
- Pane viewport ownership wins while a same-window drag is active. If the
  retained Places DnD layer receives a drag move whose window position is inside
  any pane viewport, it must clear only its Places target, emit
  `places-dnd-defer-to-pane` on `FIKA_DEBUG_DND=1`, and let the pane retained
  hit-test keep or update the item drop target. A stale Places target must not
  be allowed to accept `can_drop` for pane coordinates.
