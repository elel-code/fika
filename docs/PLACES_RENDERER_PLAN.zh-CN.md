...
5. 在 opt-in flag 背后添加自定义行视觉绘制器。与当前 GPUI row 路径对比滚动和 DnD。
   当前实现提供 `FIKA_CUSTOM_PLACES_ROWS=1` 作为 opt-in 行视觉绘制器，
   绘制行背景、active/drop 状态、标签、trash 标记和插入指示器。它保留 GPUI
   图标、行事件传递、右键菜单、行 DnD 和 drag-start shell。默认路径仍是当前
   GPUI row renderer。
6. 仅当 retained row painter 行为完整且性能持平或更优时才切换默认值。否则保留
   Dolphin 对齐的 model/projection，行渲染继续使用 GPUI。
...
仅由 `FIKA_CUSTOM_PLACES_ROWS=1` 使用；默认 GPUI row renderer 不受影响。
运行时日志包含：
