...
- Service-menu actions follow Dolphin's user-configurable direction at a smaller scope: discovered actions are filtered by a persisted enable/disable policy, while the configuration popup is backed by the full current match set so disabled rows remain recoverable.
