...
Fika 是一个面向 Linux 桌面的 Rust 文件管理器。当前 UI 主线是
Fika 专用的 `winit + wgpu` shell，也就是 `fika-wgpu`；之前的 UI runtime
已经从源码树移除。
...
- `fika-wgpu` 是默认运行目标，也是当前源码树里唯一的文件管理器 UI。
...
src/
  ...
  bin/
    fika-wgpu.rs                 winit/wgpu shell 入口
    fika_wgpu/                   已拆出的 shell 模块
...
因为 `default-run` 已经是 `fika-wgpu`，也可以直接运行：
