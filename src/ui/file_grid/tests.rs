...
use super::surface::item_theme_icon_cache_refresh_request;
...
fn content_layers_split_base_visuals_from_image_visuals() {
    ...
    assert_eq!(
        items
            .iter()
            .filter_map(|item| item_theme_icon_cache_refresh_request(item, 1.0))
            .map(|request| request.source_path().as_ref().to_path_buf())
            .collect::<Vec<_>>(),
        vec![PathBuf::from("/tmp/app.svg")]
    );
