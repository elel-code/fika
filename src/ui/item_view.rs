...
pub(crate) fn sync_pane_view_from_item_view_scroll_handle(
    ...
    scroll_state.sync_view_from_handle_snapshot(pane_id, view, |view| {
...
pub(crate) fn finish_item_view_scrollbar_drag(
    scroll_state: &mut ItemViewScrollState,
    panes: &mut PaneController,
    pane_id: PaneId,
) -> bool {
    let Some(view) = item_view_scroll_snapshot_for_existing_pane(panes, pane_id) else {
        return scroll_state.finish_scrollbar_drag_without_view(pane_id);
    };
    scroll_state.finish_scrollbar_drag_syncing_view_snapshot(pane_id, view, |view| {
        apply_item_view_scroll_snapshot_to_pane(panes, pane_id, view);
    })
}

pub(crate) fn preserve_item_view_scroll_for_layout_change(
    scroll_state: &mut ItemViewScrollState,
    panes: &mut PaneController,
    pane_id: PaneId,
) {
    let view = item_view_scroll_snapshot_for_pane(panes, pane_id);
    scroll_state.preserve_layout_scroll_syncing_view_snapshot(pane_id, view, |view| {
        apply_item_view_scroll_snapshot_to_pane(panes, pane_id, view);
    });
}
...
pub(crate) fn sync_pane_view_after_item_view_bounds_update(
    scroll_state: &mut ItemViewScrollState,
    panes: &mut PaneController,
    pane_id: PaneId,
) -> bool {
    ...
    scroll_state.sync_view_after_bounds_update_snapshot(pane_id, view, |view| {
        apply_item_view_scroll_snapshot_to_pane(panes, pane_id, view);
    })
}
