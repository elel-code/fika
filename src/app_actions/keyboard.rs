    ...
    pub(crate) fn handle_main_keyboard_input(
        ...
        if dark_mode_toggle_requested_for_key_event(
            ...
            self.apply_action_outcome(
                event_loop,
                ShellActionOutcome::present_if(changed, "toggle-dark-mode"),
            );
            return;
        }
        if let Some(command) =
        ...
        if let Some(view_mode) = view_mode_for_key_event(event, shortcut) {
            ...
            self.apply_action_outcome(
                event_loop,
                ShellActionOutcome::present_if(changed, "switch-immediate"),
            );
            return;
        }
        ...
        if hidden_toggle_requested_for_key_event(event, shortcut) {
            ...
            self.apply_action_outcome(
                event_loop,
                ShellActionOutcome::present_if(changed, "toggle-hidden"),
            );
            return;
        }
