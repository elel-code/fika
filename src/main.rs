...
fn start_device_monitor(bridge: &AsyncBridge) {
    let async_tx = bridge.tx.clone();
    let notify_ui = bridge.ui_weak.clone();
...
async fn device_monitor_loop(
    ...
    let mut signal_stream = match signal_stream {
        Ok(stream) => {
            device_debug_log_main("watching UDisks2 signals");
            Some(stream)
        }
        Err(err) => {
            device_debug_log_main(&format!("{err}; using snapshot polling only"));
            None
        }
    };
            ...
            tokio::select! {
                signal = stream.next() => {
                    if signal.is_some() {
                        ...
                        device_debug_log_main("UDisks2 signal stream ended; using snapshot polling only");
                        signal_stream = None;
                    }
                }
                _ = poll.tick() => {
...
fn schedule_devices_changed(
    ...
    tokio::spawn(async move {
        ...
        device_debug_log_main(&format!("device monitor refresh reason={reason}"));
        send_async_event(async_tx, notify_ui, AsyncEvent::DevicesChanged);
    });
}

fn device_debug_log_main(message: &str) {
